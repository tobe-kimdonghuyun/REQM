...
if (nexacro._Browser == "Runtime")
{
	if (!nexacro._init_platform_runtime)
	{
		...
		nexacro._syshandler_onmousemove_forward = function (_window, elem, strButton, altKey, ctrlKey, shiftKey, windowX, windowY, screenX, screenY, metaKey)
		{
			...
			_window._cur_screen_pos.x = screenX;
			_window._cur_screen_pos.y = screenY;
		...
		nexacro._syshandler_oncontextmenu_forward = function (_window, elem, strButton, altKey, ctrlKey, shiftKey, windowX, windowY, screenX, screenY, metaKey)
		{
			...
			return _window._on_sys_contextmenu(elem, strButton, altKey, ctrlKey, shiftKey, windowX, windowY, screenX, screenY, clientX, clientY, metaKey);
		};
		...
		*/
        ...
        nexacro._syshandler_onnetworkstatuschange_forward = function (_window, is_online)
        {
            return _window._on_sys_onnetworkstatuschange(_window, "onnetworkstatuschange", is_online); //(handle, eventid, online)
        };
		...
		nexacro._initWindowEventHandler = function (_window, handle)
		{
			...
			if (win_handle)
			{
				...
				// For Network Status Change Detection Event
	            nexacro._observeSysEvent(win_handle, "offline", "onnetworkstatuschange", nexacro._syshandler_onnetworkstatuschange_forward);
	            nexacro._observeSysEvent(win_handle, "online", "onnetworkstatuschange", nexacro._syshandler_onnetworkstatuschange_forward);
		...
		nexacro._stopWindowEventHandler = function (_window)
		{
			...
			if (win_handle)
			{
				...
				// For Network Status Change Detection Event
	            nexacro._stopSysObserving(win_handle, "online", "onnetworkstatuschange", nexacro._syshandler_onnetworkstatuschange_forward);
	            nexacro._stopSysObserving(win_handle, "offline", "onnetworkstatuschange", nexacro._syshandler_onnetworkstatuschange_forward);
			}
		};
		...
		nexacro.__bindEventModalAsyncWindowLoadHandler = function (_window, env)
		{
			return function ()
			{
				if (_window)
		...
		nexacro._createWindowHandle = function (parent_win, _window, name, left, top, width, height, resizable, layered, taskbar, is_main, env)
		{
			...
			// 2015.04.22 neoarc 
			nexacro._setViewportScale(_window, env);
		...
		nexacro._createModalAsyncWindowHandle = function (parent_win, _window, name, left, top, width, height, resizable, layered, lockmode, env)	
		{
			var parent_handle = null;
			if (parent_win) parent_handle = parent_win.handle;
		...
		{
			if(!nexacro._getWindowHwndHandle(popup))
				return true;	
			return false;
		};

			nexacro._createOpenWindowHandle = function (parent_win, name, formurl, left, top, width, height, resizable, layered, taskbar, is_main, parentframe, frameopener, arr_arg, ext_options, openstyles, close_callback, env)
			{
		...
		nexacro._throttleAnimationFrame = function (callback) { return nexacro._emptyFn; };
		nexacro._queueAnimationFrame = function (callback) { return nexacro._emptyFn; };
		...
		*/
		...
		// 2015.04.22 neoarc
		nexacro._setViewportScale = function (_window, env)
		{
			if (!_window)
				return;
		...
		//nexacro.__setTypeDefinitionURL = nexacro._emptyFn;
