...
if (nexacro.Component)
{
	...
	_pComponent._on_focus = function (self_flag, evt_name, lose_focus, refer_lose_focus, new_focus, refer_new_focus)
	{
			...
			if (cur_focus_paths && cur_focus_paths.length)
			{
				...
				if (focuspath_index > -1)
				{
					if (focuspath_index == cur_focus_paths.length - 1)
					{
						...
						if (is_accessibility_mode)
						{
							if (this._getTabOrderFirst() == null && env?._a11ytype != 5)
								return;
						}
						else
						{
							if (this._getTabOrderFirst() == null)
							{
								...
								if (this._has_inputElement && input_elem)
								{
		...
		else
		{
			...
			if (is_accessibility_mode)
			{
				...
				{
					c = this._getTabOrderFirst(env?._p_enableaccessibility ? 15 : 4);
				}
			}
			else
			{
	...
	_pComponent._on_click = function (elem, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY, meta_key)
	{
		...
		if (this._p_visible && this._isEnable() && this._p_enableevent)
		{
			...
			if (this._is_subcontrol || (this.onclick && this.onclick.defaultprevented)) return;

			this.on_click_default_action();
		}
	};

	_pComponent.on_click_default_action = function (navtarget, navurl, navtargetproperties)
	{
		navtarget = navtarget ? navtarget : this._p_navtarget;
		navurl = navurl ? navurl : this._p_navurl;

		if (!navtarget || !navurl) return;

		const env = this._getEnvironment();
		const app = env?._getApplication();
		if (!app) return;

		navtargetproperties = navtargetproperties ? navtargetproperties : this._p_navtargetproperties;
		if (navtargetproperties && typeof navtargetproperties == "string")
		{
			try
			{
				navtargetproperties = JSON.parse(navtargetproperties);
			}
 			catch (error)
			{
				navtargetproperties = undefined;
			}
		}

		const navidobj = app._getNavidObject(navtarget);
		if (!navidobj) return;
		...
		{
			if (navidobj._p_navtype == "alwaysnew")
			{
				return;
			}
			let statedata = { navtarget: navtarget, navurl: navurl, navtargetproperties: navtargetproperties };
			var hash = 0, str = navtarget + navurl;
			...
			hash = hash.toString();
			nexacro.History._pushState_internal(hash, "", statedata);
		}
	};

	_pComponent.on_click_navigate = function (url, targetproperties)
	{
	};
	...
	_pComponent._on_contextmenu = function (button, altKey, ctrlKey, shiftKey, clientX, clientY, screenX, screenY, offsetX, offsetY, metaKey)
	{
		...
		const env = this._getEnvironment();
		const enabletouchevent = env ? env._p_enabletouchevent : nexacro._enabletouchevent;
		if (!nexacro._CheckShowContextPrevented(root_comp, this, button, altKey, ctrlKey, shiftKey, clientX, clientY, screenX, screenY, offsetX, offsetY, metaKey, enabletouchevent))
		{
	...
	{
		const env = this._getEnvironment();		
		const usecontextmenu = env ? env._p_usecontextmenu : "all";
		return nexacro._checkShowContextMenu(this, usecontextmenu);
	};
	...
	_pComponent._on_mousemove = function (elem, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY, meta_key)
	{
		...
		{
			this.on_mousemove_default_action(elem, fire_comp, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY, refer_comp, meta_key);
	...
	//기본동작  return 없고 + preventDefault() -X  : 한번에 스크롤가능할때까지 하고 상위로 버블됨, 상위에서 스크롤가능할때까지 하고 상위로 버블
	//        return false(==undefined) + preventDefault() - O : self 스크롤 안됨 상위 버블됨, 상위에서 스크롤
	//        return true + preventDefault() - X : self 스크롤안되고 버블 안됨
	//        return true + preventDefault() - O : self 스크롤안되고 버블 안됨    

	// -> 변경사항
	//  스크롤가능할때까지 스코롤 
	//        return 없고 + preventDefault() -X  : 한번에 스크롤가능할때까지 함, 다음 휠에 상위에서 스크롤가능할때까지 하고 상위로 버블
	//        return false(==undefined) + preventDefault() - O : self 스크롤 안됨 상위 버블됨, 상위에서 스크롤
	//        return true + preventDefault() - X : self 스크롤안되고 버블 안됨
	//        return true + preventDefault() - O : self 스크롤안되고 버블 안됨    
	_pComponent.on_mousewheel_default_action = function (wheeldeltax, wheeldeltay, shiftkey, usehorzwheel)
	{
		let scrolltype = this._getScrollType();
		if (scrolltype == "none") return false;
		if (usehorzwheel && !shiftkey)
		{
			switch (scrolltype)
			{
				case "horizontal": if (wheeldeltay && !wheeldeltax) { wheeldeltax = wheeldeltay, wheeldeltay = 0; } break;
				case "vertical": if (wheeldeltax && !wheeldeltay) { wheeldeltay = wheeldeltax, wheeldeltax = 0; } break;
			}
		}
		else
		{
			if (wheeldeltay && scrolltype == "horizontal") return false;
			if (wheeldeltax && scrolltype == "vertical") return false;
		}
		...
		let oldvalue, newvalue, scrolllimit;
		if (wheeldeltay)
		{
			oldvalue = this._vscroll_pos;
			newvalue = oldvalue - wheeldeltay;
			if (oldvalue <= 0 && newvalue < 0)
				return false;
			scrolllimit = control_elem.vscroll_limit;
		}
		else if (wheeldeltax)
		{
			oldvalue = this._hscroll_pos;
			newvalue = oldvalue - wheeldeltax;
			if (oldvalue <= 0 && newvalue < 0)
				return false;

			scrolllimit = control_elem.hscroll_limit;
		}

		if (newvalue > scrolllimit)
			newvalue = scrolllimit;

		return oldvalue !== newvalue;  //스크롤 위치가 변경됨 → true 반환
	};
	...
	_pComponent._on_bubble_mousewheel = function (elem, wheelDeltaX, wheelDeltaY, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, meta_key)
	{
		...
		if (event_bubbles === undefined) //this is fire_comp or subcontrol
		{
			...
			if (this._p_visible && this._isEnable())
			{
				...
				{
					if (!this.onmousewheel || (this.onmousewheel && !this.onmousewheel.defaultprevented))
					{
						//  scroll 처리 
						...
						if (ret)
						{
							let comp = this._getFromComponent(this);
							if (comp && !comp._canScroll)
								comp._canScroll = () => { return false };
							// 스크롤 안됨
						}
					}
					...
					if (!ctrl_key)
					{
						...
						{
							if (this._useNativeWheelScroll())
							{
								// 이중 스크롤 방지: 진행 중 제스처의 스크롤 owner 가 따로 있고(내 자손도 아님)
								// 이면 native scroll 을 시도하지 않고 버블만 한다. 버블이 owner 에 도달해 스크롤되면
								// preventDefault 되어 이 native 컴포넌트의 기본 스크롤도 막힌다(휠 1회 = 스크롤 1회).
								var wheel_owner = nexacro._getWheelGestureOwner();
								if (wheel_owner && wheel_owner !== this && !this._containsWheelGestureOwner(wheel_owner))
								{
									// native scroll 시도 없이 아래 버블 로직으로 진행
								}
								else if (this.on_mousewheel_default_action(wheelDeltaX, wheelDeltaY, shift_key)) //scroll 가능영역이 있는지 여부 체크
								{
									let comp = this._getFromComponent(this);
									if (comp)
										comp._canScroll = () => { return true }; //스크롤 해야함
								}
								else
								{
									let comp = this._getFromComponent(this);
									comp._isscrolllast = true;

									if (wheelDeltaY)
									{
										if (shift_key && !wheelDeltaX)
											this._scrollTo(this._hscroll_pos, this._vscroll_pos, undefined, undefined, "", "mousewheel_h");
										else
											this._scrollTo(this._hscroll_pos, this._vscroll_pos, undefined, undefined, "", "mousewheel_v");
									}
									if (wheelDeltaX)
										this._scrollTo(this._hscroll_pos, this._vscroll_pos, undefined, undefined, "", "mousewheel_h");

   								    var native_popupcontrol = this._getPopupControl();
									if (native_popupcontrol && native_popupcontrol._is_popup())
										return false;
									return;
								}
							}
							...
							{
								// 비네이티브: overscroll-behavior:contain 동작 + 제스처 owner 고정
								if (this._handleWheelScrollNonNative(wheelDeltaX, wheelDeltaY, shift_key) === false)
									return false;
								// true 면 부모로 체이닝 → 아래 버블 로직으로 진행
							}
						}
					...
					if (event_bubbles === false) event_bubbles = undefined;
					if (this.onmousewheel && this.onmousewheel.defaultprevented)
					{
						if (!this._isscrolllast)
						{
							return false;
						}
						this._isscrolllast = false;
						// last 나 first 가 아닌 경우만 scroll 중지 
						// 그 외에는 parent 로 버블처리					
					}
					...
					if (!this.onmousewheel || (this.onmousewheel && !this.onmousewheel.stoppropagation))
					{
						if (this.parent && !this.parent._is_application)
						{
							...
							if (this._is_subcontrol)
							{
								return this._p_parent._on_bubble_mousewheel(elem, wheelDeltaX, wheelDeltaY, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY, event_bubbles, null, refer_comp, meta_key);
							}
							else
							{
								return this._p_parent._on_bubble_mousewheel(elem, wheelDeltaX, wheelDeltaY, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY, false, this, refer_comp, meta_key);
							}
						}
					}
				}
				...
				if (event_bubbles !== true)
				{
					if (!this.onmousewheel || (this.onmousewheel && !this.onmousewheel.defaultprevented))
					{
						...
						if (!ctrl_key) 
						{
							if (this._is_scrollable)
							{
								var nativescroll = !fire_comp._is_scrollable && fire_comp._useNativeWheelScroll();
								if (!(fire_comp && fire_comp._is_scrollable && !fire_comp._useNativeWheelScroll()) && nativescroll && this._useNativeWheelScroll())  // engine scrollable 자식(Grid 등) bubble 은 native 경로 제외 → 아래 else 에서 _handleWheelScrollNonNative 직접 스크롤(native Form 포함)
								{
									if (this.on_mousewheel_default_action(wheelDeltaX, wheelDeltaY)) //scroll 가능영역이 있는지 여부 체크 							
									{
										this._is_wheel_scroll = true;
										return;
									}
									//스크롤할 것이 없다면 상위로 onmousewheel 발생
								}
								else
								{
									// 비네이티브: overscroll-behavior:contain 동작 + 제스처 owner 고정
	...
	// 작업 완료 후 삭제
	// _pComponent._on_bubble_mousewheel1 = function (elem, wheelDeltaX, wheelDeltaY, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, meta_key)
	// {
	// 	if (!this._is_alive) return;
	// 	var clientXY, canvas;
	// 	var pThis, ret;
	...
	_pComponent._setHScrollDefaultAction = function (wheelDelta)
	{
		...
		let scrolltype = this._getScrollType();
		if (scrolltype == "none" || scrolltype == "vertical")
			return false;
	...
	// 비네이티브(_useNativeWheelScroll()==false) 휠 스크롤 1스텝 시도.
	// 실제 스크롤 위치가 바뀌면 true, 경계에 도달해 더 못 움직이면 false.
	_pComponent._doWheelScrollNonNative = function (wheelDeltaX, wheelDeltaY, shift_key)
	{
		var scrolled = false;
		if (wheelDeltaY)
		{
			this._is_wheel_scroll = true;
			if (shift_key && !wheelDeltaX)
				scrolled = this._setHScrollDefaultAction(wheelDeltaY);
			else
				scrolled = this._setVScrollDefaultAction(wheelDeltaY);
		}

		if (!scrolled && wheelDeltaX && nexacro.OS == "Mac OS")
		{
			this._is_wheel_scroll = true;
			scrolled = this._setHScrollDefaultAction(wheelDeltaX);
		}

		return scrolled;
	};

	// 이번 휠이 스크롤하려는 축("vertical" | "horizontal")을 _doWheelScrollNonNative 와 동일 규칙으로 판별.
	_pComponent._getWheelScrollAxis = function (wheelDeltaX, wheelDeltaY, shift_key)
	{
		if (wheelDeltaY && shift_key && !wheelDeltaX)
			return "horizontal";
		if (wheelDeltaY)
			return "vertical";
		if (wheelDeltaX)
	...
	// 해당 축으로 스크롤 가능한 컨테이너인지(scrolltype 기준).
	_pComponent._canWheelAxisScroll = function (axis)
	{
		var scrolltype = this._getScrollType();
		if (axis == "vertical")
			return scrolltype == "vertical" || scrolltype == "both";
		return scrolltype == "horizontal" || scrolltype == "both";
	};

	// scrollpropagation(=overscroll-behavior 유사) 속성상 해당 축의 스크롤을 부모로 전파(체이닝)할 수 있는지.
	//   both       → 양축 전파(기본, overscroll-behavior:auto)
	//   none       → 전파 안 함(contain)
	//   vertical   → 세로만 전파, horizontal → 가로만 전파
	_pComponent._isWheelScrollPropagatable = function (axis)
	{
		var scrollpropagation = this._getScrollPropagation();
		if (!scrollpropagation || scrollpropagation == "both")
			return true;
		if (scrollpropagation == "none")
			return false;
		return scrollpropagation == axis;
	};

	// 비네이티브 휠 스크롤 처리 + overscroll-behavior:contain + 제스처 latch.
	// 브라우저 네이티브 scroll chaining 과 동일한 제스처(연속 휠 버스트) 단위 latch:
	//  - 제스처 중 owner 가 경계에 도달하면 남은 휠 이벤트는 조용히 흡수(멈춤, 체인 없음).
	//  - 새 제스처를 경계에서 시작하면 wheellastover/wheelfirstover 1회 발생 후 같은 휠에서
	//    부모로 체이닝되어 부모가 스크롤하고, 그 부모가 남은 제스처의 latch 대상이 된다.
	//   반환 false : 이번 휠은 여기서 소비됨(실제 스크롤, 경계 흡수, 또는 contain) → 버블 중지
	//   반환 true  : 이 컴포넌트에서 스크롤하지 않음 → 부모로 버블(체이닝)
	_pComponent._handleWheelScrollNonNative = function (wheelDeltaX, wheelDeltaY, shift_key)
	{
		var owner = nexacro._getWheelGestureOwner();

		if (owner && owner !== this && !this._containsWheelGestureOwner(owner))
			return true;

		var axis = this._getWheelScrollAxis(wheelDeltaX, wheelDeltaY, shift_key);
		var axis_scrollable = this._canWheelAxisScroll(axis);
		var at_limit = axis_scrollable && this._isWheelAxisAtLimit(axis, wheelDeltaX, wheelDeltaY, shift_key);

		if (owner === this && at_limit)
		{
			nexacro._setWheelGestureOwner(this);   // latch 유지
			return false;
		}

		if (!at_limit && this._doWheelScrollNonNative(wheelDeltaX, wheelDeltaY, shift_key))
		{	
			this._isscrolllast = false;
			nexacro._setWheelGestureOwner(this);
			return false;
		}
	
		if (at_limit && !nexacro._wheel_limitover_fired)
		{
			nexacro._wheel_limitover_fired = true;
			this._doWheelScrollNonNative(wheelDeltaX, wheelDeltaY, shift_key);
		}

		var chain_allowed = !axis_scrollable || this._isWheelScrollPropagatable(axis);

		// 경계 표시(_isscrolllast)
		if (axis_scrollable)
			this._isscrolllast = true;
	...
	// comp 가 this(자기 자신) 또는 this 의 자손인지 여부.
	// 제스처 owner gate 에서, owner 가 자손이면(자식이 먼저 스크롤하다 경계 → 부모 체이닝) 부모 스크롤을 허용하는 데 사용.
	_pComponent._containsWheelGestureOwner = function (comp)
	{
		while (comp)
		{
			if (comp === this)
	...
	// 이 축에서 delta 방향의 경계(맨끝/맨처음)에 이미 있는지 — 순수 위치 비교, 이벤트/스크롤 발생 없음.
	// delta 부호 규칙: wheelDelta = -e.deltaY → 아래/오른쪽 스크롤 = delta < 0 = pos 증가(limit 방향).
	_pComponent._isWheelAxisAtLimit = function (axis, wheelDeltaX, wheelDeltaY, shift_key)
	{
		var control_elem = this.getElement();
		if (!control_elem)
			return false;

		var delta = (axis === "horizontal") ? ((shift_key && wheelDeltaY && !wheelDeltaX) ? wheelDeltaY : wheelDeltaX) : wheelDeltaY;
		if (!delta)
			return false;

		if (axis === "horizontal")
			return delta < 0 ? this._hscroll_pos >= control_elem.hscroll_limit : this._hscroll_pos <= 0;
		return delta < 0 ? this._vscroll_pos >= control_elem.vscroll_limit : this._vscroll_pos <= 0;
	};
	...
	_pComponent._on_animationstart = function (elem, animationname, elapsedtime)
	{
		var event_bubbles, fire_comp, refer_comp;
		var ret = this._on_bubble_animationstart(elem, animationname, elapsedtime, event_bubbles, fire_comp, refer_comp, true); // user event bubble

		var pThis = this._getFromComponent(this);
		if (pThis && (!pThis.onanimationstart || (pThis.onanimationstart && !pThis.onanimationstart.defaultprevented)))
		{
			this.on_animationstart_default_action(elem, animationname, elapsedtime, refer_comp);
			ret = this._on_bubble_animationstart(elem, animationname, elapsedtime, event_bubbles, fire_comp, refer_comp, false); // system event bubble                        
		}
		return ret;
	};

	_pComponent._on_bubble_animationstart = function (elem, animationname, elapsedtime, event_bubbles, fire_comp, refer_comp, bubble_scope)
	{
		if (!this._is_alive) return;
		...
		if (event_bubbles === undefined)
		{
			...
			{			
				if (bubble_scope)
				{
					this.on_animationstart_basic_action(elem, animationname, elapsedtime, refer_comp);
					event_bubbles = this.on_fire_user_onanimationstart(animationname, elapsedtime, this, refer_comp);
				}
				else
				{
					event_bubbles = this.on_fire_sys_onanimationstart(animationname, elapsedtime, this, refer_comp);
				}
				if (event_bubbles === false) 
					event_bubbles = undefined;
			}
			if ((!this.onanimationstart || (this.onanimationstart && !this.onanimationstart.stoppropagation)) && event_bubbles !== true && this.parent && !this.parent._is_application)
			{
				if (this._is_subcontrol)
				{
					return this.parent._on_bubble_animationstart(elem, animationname, elapsedtime, event_bubbles, null, refer_comp, bubble_scope);
				}
				else
				{
					return this.parent._on_bubble_animationstart(elem, animationname, elapsedtime, false, this, refer_comp, bubble_scope);
				}
			}
		}
		else
		{
			if (this._p_visible && (this._isEnable() || this._isAccessibilityEnable()))
			{
				if (bubble_scope)
				{
					event_bubbles = this.on_fire_user_onanimationstart(animationname, elapsedtime, fire_comp, refer_comp);
				}
				else
				{
					event_bubbles = this.on_fire_sys_onanimationstart(animationname, elapsedtime, fire_comp, refer_comp);
				}
			}
			if ((!this.onanimationstart || (this.onanimationstart && !this.onanimationstart.stoppropagation)) && event_bubbles !== true && this.parent && !this.parent._is_application)
			{
				return this.parent._on_bubble_animationstart(elem, animationname, elapsedtime, false, fire_comp, refer_comp, bubble_scope);
			}
		}
	};

	_pComponent.on_animationstart_basic_action = function (elem, animationname, elapsedtime, refer_comp)
	{
	};

	_pComponent.on_animationstart_default_action = function (elem, animationname, elapsedtime, refer_comp)
	{
	};

	_pComponent._on_animationend = function (elem, animationname, elapsedtime)
	{
		var event_bubbles, fire_comp, refer_comp;
		var ret = this._on_bubble_animationend(elem, animationname, elapsedtime, event_bubbles, fire_comp, refer_comp, true); // user event bubble

		var pThis = this._getFromComponent(this);
		if (pThis && (!pThis.onanimationend || (pThis.onanimationend && !pThis.onanimationend.defaultprevented)))
		{
			this.on_animationend_default_action(elem, animationname, elapsedtime, refer_comp);
			ret = this._on_bubble_animationend(elem, animationname, elapsedtime, event_bubbles, fire_comp, refer_comp, false); // system event bubble                        
		}
		return ret;
	};

	_pComponent._on_bubble_animationend = function (elem, animationname, elapsedtime, event_bubbles, fire_comp, refer_comp, bubble_scope)
	{
		if (!this._is_alive) return;
		...
		if (event_bubbles === undefined)
		{
			...
			{			
				if (bubble_scope)
				{
					this.on_animationend_basic_action(elem, animationname, elapsedtime, refer_comp);
					event_bubbles = this.on_fire_user_onanimationend(animationname, elapsedtime, this, refer_comp);
				}
				else
				{
					event_bubbles = this.on_fire_sys_onanimationend(animationname, elapsedtime, this, refer_comp);
				}
				if (event_bubbles === false) 
					event_bubbles = undefined;
			}
			if ((!this.onanimationend || (this.onanimationend && !this.onanimationend.stoppropagation)) && event_bubbles !== true && this.parent && !this.parent._is_application)
			{
				if (this._is_subcontrol)
				{
					return this.parent._on_bubble_animationend(elem, animationname, elapsedtime, event_bubbles, null, refer_comp, bubble_scope);
				}
				else
				{
					return this.parent._on_bubble_animationend(elem, animationname, elapsedtime, false, this, refer_comp, bubble_scope);
				}
			}
		}
		else
		{
			if (this._p_visible && (this._isEnable() || this._isAccessibilityEnable()))
			{
				if (bubble_scope)
				{
					event_bubbles = this.on_fire_user_onanimationend(animationname, elapsedtime, fire_comp, refer_comp);
				}
				else
				{
					event_bubbles = this.on_fire_sys_onanimationend(animationname, elapsedtime, fire_comp, refer_comp);
				}
			}
			if ((!this.onanimationend || (this.onanimationend && !this.onanimationend.stoppropagation)) && event_bubbles !== true && this.parent && !this.parent._is_application)
			{
				return this.parent._on_bubble_animationend(elem, animationname, elapsedtime, false, fire_comp, refer_comp, bubble_scope);
			}
		}
	};

	_pComponent.on_animationend_basic_action = function (elem, animationname, elapsedtime, refer_comp)
	{
	};

	_pComponent.on_animationend_default_action = function (elem, animationname, elapsedtime, refer_comp)
	{
	};

	_pComponent._on_animationiteration = function (elem, animationname, elapsedtime)
	{
		var event_bubbles, fire_comp, refer_comp;
		var ret = this._on_bubble_animationiteration(elem, animationname, elapsedtime, event_bubbles, fire_comp, refer_comp, true); // user event bubble

		var pThis = this._getFromComponent(this);
		if (pThis && (!pThis.onanimationiteration || (pThis.onanimationiteration && !pThis.onanimationiteration.defaultprevented)))
		{
			this.on_animationiteration_default_action(elem, animationname, elapsedtime, refer_comp);
			ret = this._on_bubble_animationiteration(elem, animationname, elapsedtime, event_bubbles, fire_comp, refer_comp, false); // system event bubble                        
		}
		return ret;
	};

	_pComponent._on_bubble_animationiteration = function (elem, animationname, elapsedtime, event_bubbles, fire_comp, refer_comp, bubble_scope)
	{
		if (!this._is_alive) return;
		...
		if (event_bubbles === undefined)
		{
			...
			{			
				if (bubble_scope)
				{
					this.on_animationiteration_basic_action(elem, animationname, elapsedtime, refer_comp);
					event_bubbles = this.on_fire_user_onanimationiteration(animationname, elapsedtime, this, refer_comp);
				}
				else
				{
					event_bubbles = this.on_fire_sys_onanimationiteration(animationname, elapsedtime, this, refer_comp);
				}
				if (event_bubbles === false) 
					event_bubbles = undefined;
			}
			if ((!this.onanimationiteration || (this.onanimationiteration && !this.onanimationiteration.stoppropagation)) && event_bubbles !== true && this.parent && !this.parent._is_application)
			{
				if (this._is_subcontrol)
				{
					return this.parent._on_bubble_animationiteration(elem, animationname, elapsedtime, event_bubbles, null, refer_comp, bubble_scope);
				}
				else
				{
					return this.parent._on_bubble_animationiteration(elem, animationname, elapsedtime, false, this, refer_comp, bubble_scope);
				}
			}
		}
		else
		{
			if (this._p_visible && (this._isEnable() || this._isAccessibilityEnable()))
			{
				if (bubble_scope)
				{
					event_bubbles = this.on_fire_user_onanimationiteration(animationname, elapsedtime, fire_comp, refer_comp);
				}
				else
				{
					event_bubbles = this.on_fire_sys_onanimationiteration(animationname, elapsedtime, fire_comp, refer_comp);
				}
			}
			if ((!this.onanimationiteration || (this.onanimationiteration && !this.onanimationiteration.stoppropagation)) && event_bubbles !== true && this.parent && !this.parent._is_application)
			{
				return this.parent._on_bubble_animationiteration(elem, animationname, elapsedtime, false, fire_comp, refer_comp, bubble_scope);
			}
		}
	};

	_pComponent.on_animationiteration_basic_action = function (elem, animationname, elapsedtime, refer_comp)
	{
	};

	_pComponent.on_animationiteration_default_action = function (elem, animationname, elapsedtime, refer_comp)
	{
	};
	...
	_pComponent.on_tap_basic_action = function (elem, canvasX, canvasY, screenX, screenY, refer_comp)
	{
		...
		//delete fire onclick -> browser native onclick		
	};
	...
	_pComponent.on_fire_sys_native_onslidestart = function (elem, touch_manager, touchinfos, xaccvalue, yaccvalue, xdeltavalue, ydeltavalue, fire_comp, refer_comp) 
	{
		return false;
	};
	...
	_pComponent._on_bubble_slidestart = function (elem, touch_manager, touchinfos, xaccvalue, yaccvalue, xdeltavalue, ydeltavalue, event_bubbles, fire_comp, refer_comp, bubble_scope)
	{
		...
		// native 여부에 따라 fire_event_func 분기
		var fire_event_func;
		if (this._useNativeTouchScroll())
			fire_event_func = bubble_scope ? nexacro._emptyFn : this.on_fire_sys_native_onslidestart;
		else
			fire_event_func = bubble_scope ? this.on_fire_user_onslidestart : this.on_fire_sys_onslidestart;
		const env = this._getEnvironment();
		const enabletouchevent = env ? env._p_enabletouchevent : nexacro._enabletouchevent;
		if (event_bubbles === undefined)
		{
	...
	_pComponent.on_fire_sys_native_onslide = function (elem, touch_manager, touchinfos, xaccvalue, yaccvalue, xdeltavalue, ydeltavalue, fire_comp, refer_comp)
	{
		...
		// fix scrollmode
		let scroll_mode = 0; //	0:none, 1:vert, 2:horz, 3:both
		...
		if (scroll_mode > 0)
		{
			...
			return true;
			//if (this._useNativeTouchScroll()) return true;  // 브라우저 스크롤 하자 
		}
		else
		{
			let scrollpropagation = this._getScrollPropagation();
			if (scrollpropagation == "none") 
			{
				touch_manager._scroll_comp = this;
				touch_manager._scroll_mode = scroll_mode;
	...
	_pComponent._on_bubble_slide = function (elem, touch_manager, touchinfos, xaccvalue, yaccvalue, xdeltavalue, ydeltavalue, event_bubbles, fire_comp, refer_comp, bubble_scope)
	{
		...
		var fire_event_func;
		if (this._useNativeTouchScroll())
			fire_event_func = bubble_scope ? nexacro._emptyFn : this.on_fire_sys_native_onslide;
		else
			fire_event_func = bubble_scope ? this.on_fire_user_onslide : this.on_fire_sys_onslide;
		const env = this._getEnvironment();
		const enabletouchevent = env ? env._p_enabletouchevent : nexacro._enabletouchevent;
		if (event_bubbles === undefined)
		{
			...
			if (event_bubbles !== true && this.parent)
			{
				...
				else
				{
					...
					{
						if (env._allow_default_pinchzoom && xdeltavalue != 0 && Math.abs(xdeltavalue) > Math.abs(ydeltavalue))
						{
	...
	_pComponent.on_fire_sys_native_onslideend = function (elem, touch_manager, touchinfos, xaccvalue, yaccvalue, xdeltavalue, ydeltavalue, fire_comp, refer_comp)
	{
		var control_elem = this.getElement();
		if (control_elem && (this == touch_manager._scroll_comp && touch_manager._scroll_end))
		{
	...
	_pComponent._on_bubble_slideend = function (elem, touch_manager, touchinfos, xaccvalue, yaccvalue, xdeltavalue, ydeltavalue, event_bubbles, fire_comp, refer_comp, bubble_scope)
	{
		...
		// native scroll 사용하는 경우 별도 함수 정의
		var fire_event_func;
		if (this._useNativeTouchScroll())
			fire_event_func = bubble_scope ? nexacro._emptyFn : this.on_fire_sys_native_onslideend;
		else
			fire_event_func = bubble_scope ? this.on_fire_user_onslideend : this.on_fire_sys_onslideend;
	...
	_pComponent.on_slideend_basic_action = function (elem, touch_manager, touchinfos, xaccvalue, yaccvalue, xdeltavalue, ydeltavalue, refer_comp)
	{
		...
		if (!pThis._is_frame && !pThis._useNativeTouchScroll())
		{
			var is_fling = false;
	...
	_pComponent._on_bubble_accessibilitygesture = function (direction, event_bubbles, fire_comp, refer_comp, bubble_scope)
	{
		...
		if (event_bubbles === undefined) //this is fire_comp or subcontrol
		{
			...
			const env = this._getEnvironment();			
			if ((this._p_visible && this._isEnable()) || (this._p_visible && env?._p_enableaccessibility && env?._a11ytype == 5))
			{
				if (bubble_scope && !this._is_hotkey)
				{
	...
	_pComponent.on_fire_sys_ondragmove = function (src_comp, src_refer_comp, dragdata, userdata, datatype, filelist, button, alt_key, ctrl_key, shift_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, xdeltavalue, ydeltavalue, meta_key)
	{
		if (src_comp && src_comp._selectscrollmode && (src_comp._selectscrollmode == "scroll"))
		{
			...
			{
				src_comp._p_hscrollbar._setPos(src_comp._p_hscrollbar.pos - xdeltavalue, "dragmove");
			}

			if ((scroll_mode == 3 || scroll_mode == 1) && ydeltavalue != 0 && src_comp._p_vscrollbar && src_comp._p_vscrollbar._p_enable)
			{
				src_comp._p_vscrollbar._setPos(src_comp._p_vscrollbar.pos - ydeltavalue, "dragmove");
			}
		}
		if (this.ondragmove && this.ondragmove._has_handlers)
		{
	...
	_pComponent.on_fire_sys_onanimationstart = function (animationname, elapsedtime, from_comp, from_refer_comp)
	{
		if (this.onanimationstart && this.onanimationstart._has_handlers)
		{
			var evt = new nexacro.StyleAnimationEventInfo(this, "onanimationstart", animationname, elapsedtime, from_comp, from_refer_comp);
			return this.onanimationstart._fireSysEvent(this, evt);
		}
		return false;
	};

	_pComponent.on_fire_user_onanimationstart = function (animationname, elapsedtime, from_comp, from_refer_comp)
	{
		if (this.onanimationstart && this.onanimationstart._has_handlers)
		{
			var evt = new nexacro.StyleAnimationEventInfo(this, "onanimationstart", animationname, elapsedtime, from_comp, from_refer_comp);
			return this.onanimationstart._fireUserEvent(this, evt);
		}
		return false;
	};

	_pComponent.on_fire_sys_onanimationend = function (animationname, elapsedtime, from_comp, from_refer_comp)
	{
		if (this.onanimationend && this.onanimationend._has_handlers)
		{
			var evt = new nexacro.StyleAnimationEventInfo(this, "onanimationend", animationname, elapsedtime, from_comp, from_refer_comp);
			return this.onanimationend._fireSysEvent(this, evt);
		}
		return false;
	};

	_pComponent.on_fire_user_onanimationend = function (animationname, elapsedtime, from_comp, from_refer_comp)
	{
		if (this.onanimationend && this.onanimationend._has_handlers)
		{
			var evt = new nexacro.StyleAnimationEventInfo(this, "onanimationend", animationname, elapsedtime, from_comp, from_refer_comp);
			return this.onanimationend._fireUserEvent(this, evt);
		}
		return false;
	};

	_pComponent.on_fire_sys_onanimationiteration = function (animationname, elapsedtime, from_comp, from_refer_comp)
	{
		if (this.onanimationiteration && this.onanimationiteration._has_handlers)
		{
			var evt = new nexacro.StyleAnimationEventInfo(this, "onanimationiteration", animationname, elapsedtime, from_comp, from_refer_comp);
			return this.onanimationiteration._fireSysEvent(this, evt);
		}
		return false;
	};

	_pComponent.on_fire_user_onanimationiteration = function (animationname, elapsedtime, from_comp, from_refer_comp)
	{
		if (this.onanimationiteration && this.onanimationiteration._has_handlers)
		{
			var evt = new nexacro.StyleAnimationEventInfo(this, "onanimationiteration", animationname, elapsedtime, from_comp, from_refer_comp);
			return this.onanimationiteration._fireUserEvent(this, evt);
		}
		return false;
	};
	...
	{
		// 네이티브 스크롤 사용하는 경우 fling 로직 비활성화 (브라우저 네이티브에 위임)
		if (this._useNativeTouchScroll())
			return true;
		...
		var vscrollbar = this._p_vscrollbar;
		var dragscrolltype;
		var scrolltype = this._scrolltype || this._p_scrolltype;
		const env = this._getEnvironment();
		const enabletouchevent = env ? env._p_enabletouchevent : nexacro._enabletouchevent;
		if (enabletouchevent) 
		{
			dragscrolltype = this._p_dragscrolltype;
			if (scrolltype == "vertical" && dragscrolltype != "vert" && dragscrolltype != "none")
			{
				dragscrolltype = "vert";
			}
			else if (scrolltype == "horizontal" && dragscrolltype != "horz" && dragscrolltype != "none")
			{
				dragscrolltype = "horz";
			}
		...
		else 
		{
			switch (scrolltype) 
			{
				case "vertical": dragscrolltype = "vert"; break;
				case "horizontal": dragscrolltype = "horz"; break;
				case "none": dragscrolltype = "none"; break;
				default: dragscrolltype = "both"; break;
			}
		}
		var scroll_comp = touch_manager._scroll_comp;
	...
	_pComponent._on_extendedcommand = function (eventid, eventtype, deviceid, data, event_bubbles)														
	{
		...
		{
			const env = this._getEnvironment();
			const module = env?.getDeviceAdaptor(deviceid);
			if (module)
	...
	{		
		...
		if (event_bubbles === undefined)																												
		{
			...
			if ((!this.onextendedcommand || (this.onextendedcommand && !this.onextendedcommand.stoppropagation)) && (event_bubbles !== true))
			{
				...
				if (!p)																											
				{
					p = this._getEnvironment();	// parent form이 없으면 environment의 event를 fire 해준다.		
					if (bubble_scope)
					{
