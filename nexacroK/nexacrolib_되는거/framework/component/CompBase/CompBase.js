...
if (!nexacro.Component)
{
    ...
    nexacro.Component = function (id, left, top, width, height, right, bottom, minwidth, maxwidth, minheight, maxheight, parent)
    {
        ...
        if (parent)
        {
            ...
            this._env = this._refform?._getEnvironment();
        }
        ...
        if (horz >= 2 && vert >= 2)
        {
            ...
            if (!this._isWidthUnset() && (left == undefined || width == undefined || right == undefined))
                this._p_right = right;
            if (!this._isHeightUnset() && (top == undefined || height == undefined || bottom == undefined))
    ...
    _pComponent._initUniqueID = function ()
    {
        if (this._unique_id.length <= 0)
        {
            ...
            this._unique_id = _p && _p._unique_id ? (nexacro._filterIDRootNodeClassName(_p._unique_id, this._getEnvironment()?.appkey) + "." + this.id) : (this.id ? this.id : "");
        }
    };
    ...
    _pComponent.attachHandle = function (win, enablea11y, bContainer)
    {
        ...
        if (!this._is_created && this._is_create_commandstr)
        {
            ...
            if (!bContainer)
            {                
                this.on_attach_contents_handle(win, enablea11y);
    ...
    _pComponent.on_attach_contents_handle = function (win, enablea11y) { this._on_attach_accessibility_contents_handle(win, enablea11y); };
    _pComponent.on_after_created = function () { };

    // check prepare command
    _pComponent._isPrepare = function ()
    {
        var ocomp = this._getRootComponent(this) || this;

        return ocomp._is_prepare;
    };
    ...
    _pComponent.on_apply_layoutorder = function ()
    {
        ...
        else
        {
            ...
            if (control_elem)
            {
                if (control_elem.dest_handle || this._isPrepare())
                {
        ...
        if (control_elem)
        {
            ...
            if (form)
            {
                ...
                if (fittocontents == "none")
                {
                    ...
                    this._flexgrow = this._p_flexgrow;
                ...
                else
                {
                    if (layouttype == 1)
                    {
                        if (fittocontents == "width" || fittocontents == "both")
                        {
                            ...
                            control_elem.setElementFlexGrow(0);
                        }
                    }
                    ...
                    {
                        if (this._flexshrink != this._p_flexshrink)
                        {
                            this._flexshrink = this._p_flexshrink;
                        }

                        if (this._flexgrow != this._p_flexgrow)
                        {
                            this._flexgrow = this._p_flexgrow;
                        }
                    }
    ...
    {
        return this._p_scrollpropagation ? this._p_scrollpropagation : this._getEnvironment()?._p_scrollpropagation;
    };
    ...
    _pComponent.on_apply_font = function (font)
    {
        ...
        // em/rem 단위 padding 재계산 (font 변경 시 px 재적용)
        if (this._p_padding)
        {
            var raw = nexacro.PaddingObject(this._p_padding);
            if (raw._hasRelative)
            {
                var resolved = this._resolvePaddingEm(raw);
                this._padding = resolved;
                this._on_apply_padding(resolved);
            }
        }

        // em/rem 단위 border 재계산 (font 변경 시 px 재적용)
        if (this._p_border)
        {
            var raw_border = nexacro.BorderObject(this._p_border);
            if (raw_border._hasRelative)
            {
                var resolved_border = this._resolveBorderEm(raw_border);
                this._border = resolved_border;
                this.on_apply_border(resolved_border);
            }
        }

        // em/rem 단위 min/max 크기 재계산 (font 변경 시 px 재적용)
        var _needMinMaxUpdate = (nexacro._isEmRemUnit(this._p_minwidth  || "") ||
                                 nexacro._isEmRemUnit(this._p_maxwidth  || "") ||
                                 nexacro._isEmRemUnit(this._p_minheight || "") ||
                                 nexacro._isEmRemUnit(this._p_maxheight || ""));
        if (_needMinMaxUpdate)
        {
            if (nexacro._isEmRemUnit(this._p_minwidth  || "")) this._minwidth  = this._resolveEmPx(this._p_minwidth);
            if (nexacro._isEmRemUnit(this._p_maxwidth  || "")) this._maxwidth  = this._resolveEmPx(this._p_maxwidth);
            if (nexacro._isEmRemUnit(this._p_minheight || "")) this._minheight = this._resolveEmPx(this._p_minheight);
            if (nexacro._isEmRemUnit(this._p_maxheight || "")) this._maxheight = this._resolveEmPx(this._p_maxheight);
            this._update_layoutbasis();
        }
    };

    _pComponent._reapplyRemValues = function ()
    {
        if (!this._isEnableRedraw()) return;
        if (this._p_itemheight && nexacro._isEmRemUnit(this._p_itemheight))
        {
            this._itemheight = this._resolveEmPx(this._p_itemheight);
    ...
    _pComponent.set_border = function (val)
    {
        ...
        if (val)
        {
            if (this._border == null || !this._border._single || this._border.value != val)
            {
                ...
                if (border._hasRelative)
                    border = this._resolveBorderEm(border);
                this._border = border;
        ...
        if (val)
        {
            if (this._padding == null || this._padding.value != val)
            {
                ...
                if (padding._hasRelative)
                    padding = this._resolvePaddingEm(padding);
    ...
    _pComponent.move = function (left, top, width, height, right, bottom)
    {
        ...
        switch (arguments.length)
        {
            ...
            default:
                if (arguments.length <= 2) width  = this._p_width;
                if (arguments.length <= 3) height = this._p_height;
                if (arguments.length <= 4) right  = this._p_right;
                if (arguments.length <= 5) bottom = this._p_bottom;
    ...
    _pComponent._apply_setfocus = function (evt_name)
    {
        ...
        if (control_elem)
        {
            var selffocus = ((evt_name == "lbutton") ? false : this._getEnvironment()?._p_enableaccessibility);
    ...
    // 음수면 0으로 clamp
    _pComponent._clampToNonNegative = function (value)
    {
    ...
    {        
        ...
        if (form)
        {
            const env = form._getEnvironment();
            if (!env) return;
            manager = env._getLayoutManager();
            if (!manager) return;
        }

        var width = this._getComputedPropWidth();
        var height = this._getComputedPropHeight();
        var minwidth = this._getComputedPropMinWidth();
        var minheight = this._getComputedPropMinHeight();

        var parent_comp = this._getParentComponent();
        var parent_fittocontents = parent_comp ? parent_comp._p_fittocontents : "none";

        var fit_type = this._p_fittocontents;
        var fit_size;

        // 1. 컨텐츠 크기 가져오기 (필요한 경우)
        if (fit_type === "width" || fit_type === "height" || fit_type === "both")
        {
            fit_size = this._on_getFitSize();
        }

        // 2. fit 적용 (음수는 0으로 clamp)
        if (fit_type === "width" || fit_type === "both")
        {
            var fit_width = this._clampToNonNegative(fit_size[0]);

            // fittocontents 결과가 0이고, 원래 % width가 있으면 fallback
            if (fit_width <= 0 && width
                && typeof width === "string"
                && width.indexOf("%") > -1)
            {
                ...
                if (containerComp)
                {
                    var ref = containerComp._basis_width || containerComp._adjust_width;
                    if (ref > 0)
                    {
                        width = this._convToPixel(width, ref);
                    }
                }
            }
            else
        ...
        if (fit_type === "height" || fit_type === "both")
        {
            var fit_height = this._clampToNonNegative(fit_size[1]);

            // fittocontents 결과가 0이고, 원래 % height가 있으면 fallback
            if (fit_height <= 0 && height
                ...
                && height.indexOf("%") > -1)
            {
                ...
                if (containerComp)
                {
                    var ref = containerComp._basis_height || containerComp._adjust_height;
                    if (ref > 0)
                    {
                        height = this._convToPixel(height, ref);
                    }
                }
            }
            else
        ...
        */
        // 3. fit_type이 default(none)일 때만 position 기반 보정
        //    (width/height/both는 컨텐츠 기반이므로 별도 보정 불필요)

        if (layouttype != "table" && fit_type !== "width" && fit_type !== "height" && fit_type !== "both")
        {
            this._fit_width = this._getAdjustFitPositionWidth(width, parent_fittocontents);
            this._fit_height = this._getAdjustFitPositionHeight(height, parent_fittocontents);

            width = this._fit_width;
            height = this._fit_height;
        }
    ...
    _pComponent._getStringResourceProperty = function (v)
    {
        if (v == undefined)
            return;
        const env = this._getEnvironment(); 
        if (env && env._stringrc)
        {
            if (this._p_expr)
                v = env._getTextTemplateValue(this._p_expr);
            else if (v && v.indexOf && v.indexOf("TEXT(") > -1)
                v = env._getTextTemplateValue(v);
        }
        return v;
    };
    ...
    _pComponent._getEnvironment = function ()
    {
        if (!this._env)
        {
    ...
    _pComponent._makeCSSMapInfo = function ()
    {
        ...
        if (!env)
        {
            //trace("CompBase._makeCSSMapInfo() : env is not found");
            return;
        }

        var mapfortypename = env?._dimension_maps?.[typeselector];
        if (mapfortypename)
        {
        ...
        if (this._is_subcontrol && this._p_parent && this._p_parent._is_component)
        {
            ...
            {
                var searchmapdata;
                var mapforid = env._dimension_maps?.[idselector];
                if (mapforid)
                {
                    searchmapdata = [];
    ...
    */
    ...
    _pComponent._adjustPosition = function ()
    {
        ...
        var minwidth = this._getComputedPropMinWidth();
        var maxwidth = this._getComputedPropMaxWidth();
        var minheight = this._getComputedPropMinHeight();
        var maxheight = this._getComputedPropMaxHeight();
    ...
    _pComponent._setMinwidth = function (v)
    {
        ...
        if (this._p_minwidth != v)
        {
            ...
            {
                this._p_minwidth = v;
                this._minwidth = this._resolveEmPx(v);
            }
    ...
    _pComponent._setMaxwidth = function (v)
    {
        ...
        if (this._p_maxwidth != v)
        {
            ...
            {
                this._p_maxwidth = v;
                this._maxwidth = this._resolveEmPx(v);
            }
    ...
    _pComponent._setMinheight = function (v)
    {
        ...
        if (this._p_minheight != v)
        {
            ...
            {
                this._p_minheight = v;
                this._minheight = this._resolveEmPx(v);
            }
    ...
    _pComponent._setMaxheight = function (v)
    {
        ...
        if (this._p_maxheight != v)
        {
            ...
            {
                this._p_maxheight = v;
                this._maxheight = this._resolveEmPx(v);
            }
    ...
    _pComponent._request_update_size = function ()
    {
        if (!this._is_created || !this._is_alive || this._is_subcontrol || this._is_frame || this._is_form || this._is_innerform)
            return;
        
        this._getWindow()?._component_size_manager?.markDirty(this);
    };
        ...
        else
        {
            if (font._unit == "rem" || font._unit == "em")
            {
                ...
                if (font._unit == "rem")
                {
                    ...
                    if (comp == this)
                    {
                        ret_font = nexacro._convertToAbsUnitFontObj(font, nexacro._getSystemFont());
                        comp = null;
                    }
                }
                else
    ...
    _pComponent._getComputedFontSize = function ()
    {
        var comp = this;
        while (comp && comp._is_component)
        {
            var font = comp._getSettedFontObject() || comp._getCSSStyleValue("font");
            if (font)
            {
                if (font._unit == "px" || font._unit == "pt")
                    return font;

                if (font._unit == "rem")
                {
                    var root_px = comp._getMFERootFontObject();
                    return nexacro._convertToAbsUnitFontObj(font, root_px);
                }
                ...
                if (font._unit == "em")
                {
                    var par_px;
                    if (comp._isMain())
                        par_px = nexacro._getSystemFont();
                    else
                    {
                        var par = comp._is_group ? comp._group_panel : comp._p_parent;
                        par_px = (par && par._is_component)
                            ? par._getComputedFontSize()
                            : nexacro._getSystemFont();
                    }
                    return nexacro._convertToAbsUnitFontObj(font, par_px);
                }

                return font;
            }

            if (comp._isMain() || comp._is_popup_frame)
                return nexacro._getSystemFont();

            comp = comp._is_group ? comp._group_panel : comp._p_parent;
        }

        return nexacro._getSystemFont();
    };

    // em/rem 단위를 px로 변환하기 위한 기준값 계산 (공통)
    _pComponent._getEmPxContext = function ()
    {
        var em_font = this._getComputedFontSize();
        var em_px = em_font ? em_font._size : 1;
        if (em_font && em_font._unit == "pt")
            em_px = em_px * (100 / 75);

        var rem_font = this._getMFERootFontObject();
        var rem_px = rem_font ? rem_font._size : 1;
        if (rem_font && rem_font._unit == "pt")
            rem_px = rem_px * (100 / 75);

        return { em: em_px, rem: rem_px };
    };

    _pComponent._getMFERootFontObject = function ()
    {
        if (nexacro.ismodule)
        {
            // MFE 환경: context를 유지하여 현재 context의 root FontObject 가져오기
            var win = this._getWindow();
            var root_app = win && win.handle ? nexacro.__MFEAPI._getRootApp(win.handle) : null;
            if (root_app)
                return root_app._getComputedFontSize();

            // fallback: _is_main인 상위 컴포넌트 직접 탐색
            var comp = this;
            while (comp && comp._is_component)
            {
                if (comp._isMain())
                {
                    var main_font = comp._getSettedFontObject() || comp._getCSSStyleValue("font");
                    if (main_font && main_font._unit == "rem")
                        return nexacro._convertToAbsUnitFontObj(main_font, nexacro._getSystemFont());
                    return comp._getComputedFontSize();
                }
                comp = comp._is_group ? comp._group_panel : comp._p_parent;
            }
        }
        var root = this._getRootObject();
        if (root)
        {
            var root_font = root._getSettedFontObject() || root._getCSSStyleValue("font");
            if (root_font && root_font._unit == "rem")
                return nexacro._convertToAbsUnitFontObj(root_font, nexacro._getSystemFont());
            return root._getComputedFontSize();
        }
        return nexacro._getSystemFont();
    };

    _pComponent._resolveEmPx = function (v)
    {
        if (nexacro._isEmRemUnit(v))
            return parseFloat(this._resolveEmString(String(v)));
        return parseFloat(v);
    };

    _pComponent._resolveSizeStr = function (str)
    {
        var self = this;
        return String(str).split(/\s+/).map(function (p) 
        {
            return nexacro._isEmRemUnit(p) ? String(self._resolveEmPx(p)) : p;
        }).join(" ");
    };

    // 값 문자열 내 모든 em/rem 토큰을 px 문자열로 치환 (공통)
    // 사용 예: "1em 2rem solid red" → "18px 24px solid red"
    _pComponent._resolveEmString = function (val)
    {
        if (!val || !/\d(rem|em)/i.test(val)) return val;
        var ctx = this._getEmPxContext();
        var self = this;
        return val.replace(/(\d*\.?\d+)(rem|em)\b/gi, function (match)
        {
            var px_ref = (match.indexOf("rem") >= 0) ? ctx.rem : ctx.em;
            return self._convToPixel(match, px_ref) + "px";
        });
    };

    // padding 전용 thin wrapper
    _pComponent._resolvePaddingEm = function (padding)
    {
        var r = nexacro.PaddingObject(this._resolveEmString(padding.value));
        return { top: r.top, right: r.right, bottom: r.bottom, left: r.left,
                 value: padding.value, _hasRelative: true };
    };

    // border 전용 thin wrapper
    _pComponent._resolveBorderEm = function (border)
    {
        var r = new nexacro._BorderObject(this._resolveEmString(border.value));
        r.value = border.value;
        r._hasRelative = true;
        return r;
    };

    // (%, em, rem) to px 처리
    _pComponent._getFontRefSize = function (refer_font)
    {
    ...
    _pComponent._calcArrangePosition = function ()
    {
            ...
            if (form && form._is_created_contents)   // 초기 loading중일 때 .left 등 설정 안함.
            {
                ...
                if (!comp)
                {
                    if (distance.indexOf("rem") > -1 || distance.indexOf("em") > -1)
                    {
                        ...
                        {
                            refer_font = this._getComputedFontSize();
                        }
        ...
        const adjustSizeArrangeInfo = (obj, type) => 
        {
            let comp_size;
            distance = obj.distance;
            comp = this._findComponentForArrange(obj.compid);
            if (!comp)
            {
                if (distance.indexOf("rem") > -1 || distance.indexOf("em") > -1)
                {
                    ...
                    comp_size = this._getFontRefSize(refer_font);
                }
                else
                {
                    if (parent_comp._p_fittocontents == type || parent_comp._p_fittocontents == "both")
                    {
                        comp_size = type == "width" ? parent_comp._basis_width : parent_comp._basis_height;
                    }
                    else
                    {
                        comp = form;
                        comp_size = type == "width" ? comp._getClientWidth() : comp._getClientHeight();
                    }
                }
            }
            else
            {
                comp_size = type == "width" ? comp._adjust_width : comp._adjust_height;
            }

            return this._convToPixel(distance, comp_size);
        };
        ...
        else
        {
            if (info && (obj = info.width))
            {
                if (form && form._is_created_contents)
                {
                    this._width = adjustSizeArrangeInfo(obj, "width");
                }
            }
            else
            {
                const val = this._getComputedPropWidth(false);
                obj = this._parseArrangeVal(this._isWidthUnset() ? val : null); // unset 일때 em, rem, % 처리를 위함, unset이 아닐때는 기존 방식 유지
                if(!obj)
                {
                    if (val != null)
                        this._width = isNaN(parsePosition = parseFloat(val)) ? this._width : parsePosition;
                    else
                        this._width = null;
                }
                else
        ...
        else
        {
            if (info && (obj = info.height))
            {
                if (form && form._is_created_contents)
                {
                    this._height = adjustSizeArrangeInfo(obj, "height");
                }
            }
            else
            {
                const val = this._getComputedPropHeight(false);
                obj = this._parseArrangeVal(this._isHeightUnset() ? val : null); // unset 일때 em, rem, % 처리를 위함, unset이 아닐때는 기존 방식 유지
                if(!obj)
                {
                    if (val != null)
                        this._height = isNaN(parsePosition = parseFloat(val)) ? this._height : parsePosition;
                    else
                        this._height = null;
                }
                else
    ...
    _pComponent._makeExprFn = function (expr)
    {
        ...
        return nexacro._createExprFunc(str, this, ["comp"]);
    };
    ...
    _pComponent._isMainFrameBody = function ()
    {
        ...
        return (this._isMainFrame() && nexacro.isglobalmodule)
    };
    ...
    _pComponent._isAllowDefaultPinchZoom = function ()
    {   
        const env = this._getEnvironment();
        return env?._allow_default_pinchzoom || false;
    };
    ...
    */
    ...
    _pComponent._isWidthUnset = function ()
    {
        return this._p_width === "unset";
    };

    _pComponent._isHeightUnset = function ()
    {
        return this._p_height === "unset";
    };

    _pComponent._getComputedPropWidth = function (bResolveEmPx = true)
    {
        let val = this._p_width;
        if (this._isWidthUnset())
        {
            val = this._getCSSStyleValue("width") || (this._getEnvironment()?._getRegisterClassWidth(this._type_name) ?? "100");
        }

        if (nexacro._isPercentageUnit(val))
            return val;

        return bResolveEmPx ? this._resolveEmPx(val) : val;
    };

    _pComponent._getComputedPropMinWidth = function (bResolveEmPx = true)
    {
        const val = this._p_minwidth ?? this._getCSSStyleValue("minWidth");
        
        return (val && bResolveEmPx) ? this._resolveEmPx(val) : val;
    };

    _pComponent._getComputedPropMaxWidth = function (bResolveEmPx = true)
    {
        const val = this._p_maxwidth ?? this._getCSSStyleValue("maxWidth");

        return (val && bResolveEmPx) ? this._resolveEmPx(val) : val;
    };

    _pComponent._getComputedPropHeight = function (bResolveEmPx = true)
    {
        let val = this._p_height;
        if (this._isHeightUnset())
        {
            val = this._getCSSStyleValue("height") || (this._getEnvironment()?._getRegisterClassHeight(this._type_name) ?? "100");
        }
        if (nexacro._isPercentageUnit(val))
            return val;
    ...
    _pComponent._getComputedPropMinHeight = function (bResolveEmPx = true)
    {
        const val = this._p_minheight ?? this._getCSSStyleValue("minHeight");

        return (val && bResolveEmPx) ? this._resolveEmPx(val) : val;
    };

    _pComponent._getComputedPropMaxHeight = function (bResolveEmPx = true)
    {
        const val = this._p_maxheight ?? this._getCSSStyleValue("maxHeight");
    ...
    _pComponent._getFixedOffsetValue = function ()
    {
        ...
        {
            val = this._getComputedPropWidth();
        ...
        {
            val = this._getComputedPropHeight();
            if (typeof (val) == "string" && val.indexOf("%") >= 0)
            {
                if (parseFloat(val) <= 100)
                    b = 0;
    ...
    _pComponent._isRtl = function (comp)
    {
        ...
        return this._getEnvironment()?._rtl || false;  //chcek
    };  
    ...
    {
        // editable_control flag 확인 용도로 변경
        return this._is_editable_control;

        // 이전 코드에서 taborder 검색시 하위 component의 type 별 검색을 위한 코드로 현재 N이상 버전 에서는 사용하지 않아 주석 처리
    ...
    _pComponent._getLayoutBasis = function ()
    {
        ...
        var form = this._is_group ? this._group_panel : this._is_fluid_layout ? this : this._p_parent;
        var layouttype = form._getLayoutType(manager);
        /*
        manager.getLayoutProperty(form, "type");
        */
        ...
        var width = this._fit_width > -1 ? this._fit_width : this._getComputedPropWidth();
        var height = this._fit_height > -1 ? this._fit_height : this._getComputedPropHeight();
        ...
        var minwidth = withUnit(this._getComputedPropMinWidth());
        var maxwidth = withUnit(this._getComputedPropMaxWidth());
        var minheight = withUnit(this._getComputedPropMinHeight());
        var maxheight = withUnit(this._getComputedPropMaxHeight());
    ...
    _pComponent._getAdjustFitPositionHeight = function (height, parent_fittocontents)
    {
        ...
        var needsConversion = (parent_fittocontents === "both" || parent_fittocontents === "height")
            && typeof height === "string"
            && height.indexOf("%") > -1;

        if (!needsConversion) return height;

        var myFit = this._p_fittocontents;

        // 자기가 fittocontents이면 % height는 일단 무시
        // 단, 자식 컨텐츠 높이가 0이면 % 를 fallback으로 사용
        if (myFit === "height" || myFit === "both")
        {
            return -1;
        }
    ...
    _pComponent._getAdjustFitPositionWidth = function (width, parent_fittocontents)
    {
        ...
        var needsConversion = (parent_fittocontents === "both" || parent_fittocontents === "width")
            && typeof width === "string"
            && width.indexOf("%") > -1;

        if (!needsConversion) return width;

        // 자기가 fittocontents이면 % width는 무시 → 컨텐츠 기반으로 결정
        // (결과가 0이면 _getSizeInfo에서 % fallback 처리)
        var myFit = this._p_fittocontents;
        if (myFit === "width" || myFit === "both")
        {
            return -1;
        }

        // 부모가 fittocontents이고 자기는 fittocontents가 아닌 경우 → % 변환 스킵
        return width;
    };

    _pComponent._isFontBasedUnit = function (distance)
    {
        return /(?:^|[\d.])r?em$/.test(distance);
    };
    ...
    _pComponent._calcBasisSize = function ()
    {
        ...
        // fittocontents 축은 basis를 초기화 — 확정된 실제 크기로 나중에 세팅됨
        if (fittocontents == "width" || fittocontents == "both")
            this._basis_width = undefined;
        if (fittocontents == "height" || fittocontents == "both")
            this._basis_height = undefined;

        this._calcBasisSizeForAxis(
            this._p_width,
            (fittocontents == "width" || fittocontents == "both") ? null : this._p_right,
            containerComp._basis_width,
            "_basis_width",
            "_adjust_left"
        );
        ...
        this._calcBasisSizeForAxis(
            this._p_height,
            (fittocontents == "height" || fittocontents == "both") ? null : this._p_bottom,
            containerComp._basis_height,
            "_basis_height",
            "_adjust_top"
        );
    };
    _pComponent._calcBasisSizeForAxis = function (size, end_offset, parent_basis, basis_prop, adjust_offset_prop)
    {
        // 1. size (width / height) 처리
        if (size)
        {
            if (typeof size === "string" && size.indexOf("%") > -1)
            {
                if (parent_basis)
                {
                    this[basis_prop] = Math.max(0, this._convToPixel(size, parent_basis));
                }
            }
            else
            {
                this[basis_prop] = Math.max(0, +size);
            }
        }

        // 2. end_offset (right / bottom) 처리
        // 부모 basis가 없으면 처리 불가
        if (!end_offset || !parent_basis) return;

        var calculated;
        if (typeof end_offset === "string" && end_offset.indexOf("%") > -1)
        {
            calculated = parent_basis - this._convToPixel(end_offset, parent_basis);
        }
        else
        {
            var offset = this[adjust_offset_prop];
            var adjust_offset = (offset > 0) ? offset : 0;
            calculated = parent_basis - (+end_offset) - adjust_offset;
        }

        // 음수면 부모 기준으로 clamp - 잘못된 값이 다른 계산으로 전파되는 것 방지
        //this[basis_prop] = Math.max(parent_basis, calculated);
        this[basis_prop] = calculated;
    };
    ...
    {
        // true: 자기자신에서 focus를 멈춤
        if (this._hasContainer() && this._getEnvironment()?._p_enableaccessibility && this._isAccessibilityEnable() == true && this._isAccessibilityRoleHeading())
            return true;
    ...
    {
        const env = this._getEnvironment();
        const datatyperule = env ? env._p_datatyperule : "1.0";
        if (datatyperule == "1.0" || nexacro._isNull(v))
            return v;
    ...
    {
        const env = this._getEnvironment();
        v = v !== undefined ? this._p_usesoftkeyboard : env ? env._p_usesoftkeyboard : true;
        this._use_softkeyboard = env ? env._p_enabletouchsoftkeyboard ? (v ? 2 : 0) : 1 : nexacro._enabletouchsoftkeyboard ? (v ? 2 : 0) : 1;
        return v;
    };
    ...
    _pComponent._setLayoutContainerProps = function ()
    {
        ...
        {
            const env = this._getEnvironment();
            const manager = env._getLayoutManager();
    ...
    _pComponent._setLayoutContentsProps = function ()
    {
        ...
        if (control_elem)
        {
            ...
            if (this._isFlexGrow())
            {
                if (this._p_flexgrow != undefined)
                    control_elem.setElementFlexGrow(this._p_flexgrow);
                if (this._p_flexshrink != undefined)
                    control_elem.setElementFlexShrink(this._p_flexshrink);
            }
    ...
    _pPopupControl._popupBy = function (from_comp, left, top, width, height, center)
    {
        ...
        if (nexacro._setzoomfactor)
        {
            left = left != 0 ? left * nexacro._setzoomfactor : left;
            top = top != 0 ? top * nexacro._setzoomfactor : top;
        }
        var _zoomfactor = nexacro._setzoomfactor ? nexacro._setzoomfactor : 1;
        if (center)
        {
            if (from_comp)
                p = nexacro._getElementPositionInFrame(from_comp.getElement());

            left = (left * _zoomfactor) + p.x;
            top = (top * _zoomfactor) + p.y;
        }
        else
        {
            if (from_comp)
                p = nexacro._getPopupElementPositionInFrame(from_comp.getElement(), left, top*_zoomfactor, width, height, true);
    ...
    __pWaitControl._on_loadimage = function (imgurl, w, h)
    {
        ...
        if (img_elem && w > 0 && h > 0)
        {
            ...
            img_elem.setElementImage(nexacro.UrlObject(imgurl, this));
        }
    };
    ...
    __pWaitControl.setImage = function (imageurl)
    {
        ...
        if (img_elem)
        {
            var imageurl = nexacro._getURIValue(imageurl);
            imageurl = this._getEnvironment()?._getImageLocation(imageurl, this._getRefFormBaseUrl());
            var imagesize = nexacro._getImageSize_extended(imageurl, this._on_loadimage, this);
            if (imagesize)
            {
                ...
                img_elem.setElementImage(nexacro.UrlObject(imageurl, this));
            }
        }
    };
...
if (!nexacro.HTMLComponent)
{
    ...
    _pHTMLComponent.on_apply_content = async function ()
    {
        ...
        if (control_elem && env)
        {
            const htmlTemplate = env.loadHTMLTemplate(this._p_content);
