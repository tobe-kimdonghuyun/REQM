...
if (!nexacro.Element)
{
    ...
    _pElement._removeElementHandle = function ()
    {
     ...
     //   if (linked_control && linked_control._is_main) // mainframe  check 필요 
    ...
    _pElement._isRtl = function (bPosition)
    {
        ...
        if (elem)
            env = elem.linkedcontrol?._getEnvironment();
        ...
        while (elem)
        {
            ...
            if (!env && elem && elem.linkedcontrol)
                env = elem.linkedcontrol._getEnvironment();
        }
    ...
    _pElement._forceApplyPosTop = function (top)
    {
        if (this.top != top)
        {
            var handle = this.handle;
            if (handle)
                nexacro.__setDOMStyle_Pos(handle.style, null, top);
        }
    };
    ...
    _pElement._forceApplyPosLeft = function ()
    {
        if (this.left)
        {
            var handle = this.handle;
            if (handle)
                nexacro.__setDOMStyle_Pos(handle.style, this.left, null);
        }
    };
    ...
    _pElement.setElementRtl = function (rtl)
    {
        if (this.rtl != rtl)
        {
            ...
            if (handle)
            {
                nexacro.__setDOMNode_Dir(handle, this.rtl);
            }
        }
    };
    ...
    */
    ...
    _pElement._refreshCommonStyleProps = function (handle_style, no_size)
    {
        ...
        if (!no_size && !(control && control._is_frame && (control._isMainFrameBody() && !control._isAllowDefaultPinchZoom())))   //if it is not mainframe
        {
    ...
    _pElement._getCommonStyleStr = function (no_size)
    {
        ...
        else if (this._use_sticky)
        {
            str += nexacro.__getHTMLStyle_Sticky();
        }
    ...
    _pElement.setElementAccessibilityLabelledBy = nexacro._emptyFn;
    ...
    _pTextBoxElement._sub_node = null;  // for textOverflow (ellipsis)
    ...
    _pTextBoxElement._createTextElementHandle = function (_doc)
    {
        ...
        var textoverflow = this._getTextOverflow();

        if (textoverflow)
        {
            var sub_handle = _doc.createElement("div");
            nexacro.__setDOMNode_ClassName(sub_handle, "nexatextitem");
            nexacro.__setDOMNode_Id(sub_handle, "", this.name + ":subtext");
            this._sub_node = this._box_node = sub_handle;

            nexacro.__setDOMDataset_TextOverflow(sub_handle, this.textOverflow || this._textoverflow_info);
            //sub_handle.style.textOverflow = "ellipsis";
            //sub_handle.style.whiteSpace = "nowrap";
            sub_handle.style.width = this.width + "px";
            nexacro.__appendDOMNode(handle, sub_handle);
        }
    ...
    _pTextBoxElement._getComputedStyle = function (prop, noflush, boxnode)
    {
        var style;
    ...
    _pTextBoxElement._createElementHandle = function (owner_elem, _doc)
    {
        if (this.text)
        {
            ...
            if (this.verticalAlign)
                nexacro.__setDOMStyle_verticalAlign(handle_style, this.verticalAlign);
            ...
            if (this._use_style_padding)
                nexacro.__setDOMStyle_PaddingObject(box_style, this.padding);
    ...
    _pTextBoxElement._isWordWrapNone = function ()
    {
        const val = this.wordWrap || this._wordwrap_info;
        return (val === "none" || val === "" || val == null);
    };

    _pTextBoxElement._updateTextWordWrapAndOverflow = function ()
    {
        var text_node = this._box_node;
        if (text_node)
        {
            this._destroyElementHandle();
            this._createElementHandle(this.owner_elem);

            text_node = this._box_node;

            if (this._isWordWrapNone())
                nexacro.__setDOMDataset_TextOverflow(text_node, this.textOverflow || this._textoverflow_info);
            else
                nexacro.__setDOMNodeStyle_WordWrap(this.handle.style, this.wordWrap);
        }
    };

    _pTextBoxElement.setElementTextOverflow = function (textoverflow)
    {
        ...
        if (this.textOverflow != textoverflow)
        {
            ...
            if (this.handle)
            {
                this._updateTextWordWrapAndOverflow();
            }
        }
    };

    _pTextBoxElement.setElementCSSMapTextOverflowInfo = function (textoverflow_info)
    {
        if (this._textoverflow_info != textoverflow_info)
        {
            this._textoverflow_info = textoverflow_info;
            if (this.handle)
            {
                this._updateTextWordWrapAndOverflow();
            }
    ...
    _pTextBoxElement._getTextOverflow = function ()
    {
        var textoverflow = "";
        if (this._isWordWrapNone())
            textoverflow = nexacro.__getHTMLDataset_TextOverflow(this.textOverflow || this._textoverflow_info);
    ...
    _pTextBoxElement.createCommand = function ()
    {
        ...
        if (owner_elem && !this._created)
        {
            ...
            if (this.text)
            {
                ...
                if (this._use_style_padding)
                    box_style += nexacro.__getHTMLStyle_PaddingObject(this.padding);
                ...
                if (textoverflow)
                    //str += "<div id='" + this.name + ":subtext' class='nexatextitem' " + textoverflow + " style='white-space:nowrap; text-overflow: ellipsis; width:" + this.width + "px'>";
                    str += "<div id='" + this.name + ":subtext' class='nexatextitem' " + textoverflow + " style='width:" + this.width + "px'>";
    ...
    _pTextBoxElement.attachHandle = function (win)
    {
        if (this.name && !this._created)
        {
            ...
            if (handle)
            {
                ...
                var text_subnode = win._doc.getElementById(this.name + ":subtext");
                if (text_subnode)
                    this._sub_node = this._box_node = text_subnode;
    ...
    _pTextBoxElement.setElementPadding = function (padding)
    {
        ...
        if (this.padding != padding)
        {
            ...
            if (box_node)
            {
                nexacro.__setDOMStyle_PaddingObject(box_node.style, padding);
            }
        }
    };
    ...
    _pTextBoxElement.setElementVerticalAlign = function (verticalalign)
    {
        if (this.verticalAlign != verticalalign)
        {
            ...
            if (handle)
            {
                nexacro.__setDOMStyle_verticalAlign(handle.style, verticalalign);
            }
        }
    };
    ...
    _pTextBoxFlexElement._applyPadding = function ()
    {
        ...
        var right = this.padd_left;
        var bottom = this.padd_bottom;
        var left = this.padd_right;

        var v_align = this._getComputedStyle("align-items");

        // padding을 조절하여 text영역을 확보함. (성능개선)
        handle.style.paddingInline = left + "px " + right + "px";

        if (v_align == "flex-start")
        {
            handle.style.paddingBlock = top + "px " + 0 + "px";
        }
        else if (v_align == "flex-end")
        {
            handle.style.paddingBlock = 0 + "px " + bottom + "px";
        }
        else
        {
            var val = top - bottom;
            ...
            handle.style.paddingInline = left + "px " + right + "px";
            handle.style.paddingBlock = top + "px " + bottom + "px";
        }
    };
    ...
    _pTextBoxVirtualFlexElement._applyPadding = function ()
    {
        var handle = this.handle;
        if (!handle)
            return;
        ...
        var top = this.padd_top;
        var right = this.padd_right
        var bottom = this.padd_bottom;
        var left = this.padd_left;

        var v_align = this._getComputedStyle("align-items");
        var w = this.width;
        var h = this.height;

        // padding이 컨트롤 크기보다 큰 경우 처리
        var gap = (left + right) - (w < 0 ? 0 : w);
        if (gap > 0)
        {
            right -= gap;
    ...
    */
        ...
        if (owner_elem && !this._created)
        {
            ...
            if (_type)
            {
                ...
                else if (_type == 3)
                {
                    if (this.textwidth)
                    {
                        text_style += nexacro.__getHTMLStyle_TextWidth(this.textwidth);
                    }
                    if (this.textPadding)
    ...
    _pIconTextElement.setElementTextWidth = function (textwidth)
    {
        if (this.textwidth != textwidth)
        {
            ...
            if (text_node)
            {
                ...
                if (_type == 3)
                {
                    if (nexacro._isEmRemUnit(textwidth))
                        nexacro.__setDOMStyle_TextWidth(text_node.style, textwidth);
                    else
                    {
    ...
    if (nexacro._OS == "Android")
    {
        ...
        _pInputElement._refreshCommonStyleProps = function (handle_style)
        {
            ...
            var linked_control = this.linkedcontrol;
            if (!(linked_control && linked_control._is_frame && (linked_control._isMainFrameBody() && !linked_control._isAllowDefaultPinchZoom())))   //if it is not mainframe
            {
                if (this.width && this.height)
                {
    ...
    _pInputElement.setElementInputType = function (type, bImedisable, use_password)
    {
    ...
    _pInputElement.setElementBlur = function ()
    {
        ...
        if (input_handle)
        {
            const _doc = this._getRootWindowHandle();
            var pThis;

            if (_doc)
            {
                // RP 104753  caret을 삭제하려는 원인은 16년도에 작업되어서 caret을 지우려는 node 가 동일하면 수행(focusin event 시점에 caret을 날려서 caret 안나오는 문제)
                if (this.inputtype != "date" && input_handle === _doc.activeElement)
                {
        ...
        if (input_handle)
        {
            ...
            if (_doc)
            {
                if (range == len)
                {
                    ...
                    {
                        const env = this.parent_elem.linkedcontrol._getEnvironment();
                        if (env?._p_enableaccessibility)
                        {
                            setTimeout(function ()
                            {
    ...
    _pInputElement._go_next_focus = function ()
    {
        ...
        if (input_handle && comp)
        {
            ...
            if (newfocus_comp && newfocus_comp[0])
            {
                ...
                if (!newfocus_comp[0]._p_autoselect && newfocus_comp[0]._isEditableComponent())
    ...
    _pTextAreaElement._scrollpropagation = "both";
    _pTextAreaElement._scrolltype = "both";
    // TODO
    ...
    _pTextAreaElement._createElementHandle = function (owner_elem, _doc)
    {
        ...
        if (this._scrollpropagation != "both")
        {
            nexacro.__setDOMStyle_OverscrollBehavior(handle_style, this._scrollpropagation);
        }
    ...
    _pTextAreaElement.createCommand = function ()
    {
        ...
        if (owner_elem && !this._created)
        {
            ...
            if (this._scrollpropagation != "both")
            {
                handle_style += nexacro.__getHTMLStyle_OverscrollBehavior(this._scrollpropagation);
            }
    ...
    _pTextAreaElement.setElementScrollPropagation = function (scrollpropagation)
    {
        if (this._scrollpropagation != scrollpropagation)
        {
            this._scrollpropagation = scrollpropagation;
            let handle = this.handle;
            if (handle)
            {
                nexacro.__setDOMStyle_OverscrollBehavior(handle.style, this._scrollpropagation);
            }
        }
    };
    ...
    */
    ...
    _pTextAreaElement._on_sys_scroll = function (evt)
    {
        ...
        if (input_handle)
        {
            ...
            if (pThis)
            {
                let parent_elem = pThis.parent_elem;
                let comp = parent_elem.linkedcontrol;

                let evtid = "";
                let scrolltype = parent_elem._scrolltype;
                let bScroll = false;
                let bWheel = comp._is_wheel_scroll;

                let container = pThis.owner_elem;
                if (container && parent_elem)
                {
                    let hscroll_pos = parent_elem.scroll_left;
                    let vscroll_pos = parent_elem.scroll_top;

                    let scrollleft = input_handle.scrollLeft;
                    let scrolltop = input_handle.scrollTop;
                    if (hscroll_pos != scrollleft)
                    {
                        container._cached_scrollLeft = scrollleft;
                        container._scroll_left = scrollleft;
                        ...
                        if (scrolltype != "none" && scrolltype != "vertical" && !comp._is_tracking)
                        {
                            ...
                            {
                                hscroll_pos = scrollleft;
                                bScroll = true;
                            }

                        }
                        evtid = bWheel ? "mousewheel_h" : "horizontal";
                    }

                    if (vscroll_pos != scrolltop)
                    {
                        container._cached_scrolltop = scrolltop;
                        container._scroll_top = scrolltop;
                        if (scrolltype != "none" && scrolltype != "horizontal" && !comp._is_tracking)
                        {
                            vscroll_pos = scrolltop;
                            bScroll = true;
                        }
                        evtid = bWheel ? "mousewheel_v" : "vertical";
                    }

                    if (bScroll)
                    {
                        comp._scrollTo(hscroll_pos, vscroll_pos, true, true, undefined, evtid, bWheel);
                    }
                }
            }
        }
    ...
    _pTextAreaElement._on_sys_paste = function (evt)
    {
        ...
        if (input_handle && !input_handle._linked_element._use_event_beforeinput && data)
        {
            ...
            */
            ...
            /**
             * キャレット位置がこのメソッドの?理中は正しい位置にあるが後で?制的に末尾に設定されてしまうため、
             * タイマ??由で再度同じ位置に設定する（タイマ?で?延されないと末尾になる）
             */
            setTimeout(function ()
            {
    ...
    _pControlElement.transform = null;
    _pControlElement.transformOrigin = null;
    _pControlElement.animation = null;
    _pControlElement.animationPlayState = null;
    _pControlElement.transition = null;
    ...
    _pControlElement._use_native_scroll = false;
    _pControlElement._use_native_scroll_size = false;

    _pControlElement._scrolltype = "both";
    _pControlElement._scrollpropagation = "both";
        ...
        if (!this._is_popup)
        {
            ...
            if (owner_elem && owner_elem.handle)
            {
                if (!this.handle)
                {
                    ...
                    if (this.linkedcontrol && this.linkedcontrol._is_abstract) return;

                    //iOS 
                    if (nexacro._OS == "iOS" && this.linkedcontrol &&!this.linkedcontrol._isAllowDefaultPinchZoom())
                        nexacro.__setDOMStyle_TouchAction(handle_style, "manipulation");
    ...
    _pControlElement._getHTMLCustomDataStr = function ()
    {
        return "";
    };
        ...
        if (!this._is_popup)
        {
            ...
            if (owner_elem && !this.handle)
            {
                ...
                if (linkedcontrol._p_semantictag)
                    str = "<" + linkedcontrol._p_semantictag + " id='" + this.name + "' class='" + this._getElementClassName(linkedcontrol._p_semantictag) + "'" + nexacro.__getHTMLAttr_Dir(this.rtl) + " tabindex ='" + this.tabindex + "' ";
                else
                    str = "<div id='" + this.name + "' class='" + this._getElementClassName() + "'" + nexacro.__getHTMLAttr_Dir(this.rtl) + " tabindex ='" + this.tabindex + "' ";

                if (!nexacro._isNull(this.tooltiptext))
                {
                    let tooltiptext = this.tooltiptext + "" ;
                    str += " title = '" + tooltiptext.replaceAll("'", "&apos;") + "' ";
                }

                str += this._getHTMLCustomDataStr();

                var style_str = this._getControlStyleStr();
                var status_str = this._getControlStatusStr();
                if (nexacro._OS == "iOS" && !linkedcontrol._isAllowDefaultPinchZoom())
                    style_str += nexacro.__getHTMLStyle_TouchAction("manipulation"); //disable the double-tap
    ...
    _pControlElement.setLinkedControl = function (control)
    {
        if (!this.linkedcontrol && control)
        {
            ...
            if (control._use_sticky != undefined)
                this._use_sticky = control._use_sticky;
        }
    };
    ...
    _pControlElement._parseLayoutPosition = function ()
    {
        ...
        if (strpos)
        {
            ...
            else
            {
                ...
                {
                    const env = control._getEnvironment();
                    var manager = env._getLayoutManager();
                    if (manager)
                    {
                        // div 상황에 따라 getForm 말고 다른걸로 처리
                        var tabletemplatearea = manager.getLayoutProperty(control._is_group ? control._group_panel : control._getForm(), "tabletemplatearea");
    ...
    _pControlElement._getComputedStyleColor = function (noflush)
    {
        var handle = this.handle;
        if (handle)
        {
    ...
    _pControlElement.setElementTransform = function (transform)
    {
        if (this.transform != transform)
        {
            ...
            if (handle)
            {
                nexacro.__setDOMStyle_TransformObject(handle.style, transform);
            }
        }
    };

    _pControlElement.setElementTransformOrigin = function (transformorigin)
    {
        if (this.transformOrigin != transformorigin)
        {
            this.transformOrigin = transformorigin;
            const handle = this.handle;
            if (handle)
            {
                nexacro.__setDOMStyle_TransformOriginObject(handle.style, transformorigin);
            }
        }
    };

    _pControlElement.setElementAnimation = function (animation)
    {
        if (this.animation != animation)
        {
            ...
            if (handle)
            {
                nexacro.__setDOMStyle_AnimationObject(handle.style, animation);
            }
        }
    };

    _pControlElement.setElementAnimationPlayState = function (animationplaystate)
    {
        if (this.animationPlayState != animationplaystate)
        {
            this.animationPlayState = animationplaystate;
            const handle = this.handle;
            if (handle)
            {
                nexacro.__setDOMStyle_AnimationPlayStateObject(handle.style, animationplaystate);
            }
        }
    };

    _pControlElement.setElementTransition = function (transition)
    {
        if (this.transition != transition)
        {
            ...
            if (handle)
            {
                nexacro.__setDOMStyle_TransitionObject(handle.style, transition);
            }
        }
    };
    /*
    ...
    _pControlElement._refreshControl = function (handle, handle_style)
    {
        ...
        if (this.transform)
        {
            nexacro.__setDOMStyle_TransformObject(handle_style, this.transform);
        }
        if (this.transformOrigin)
        {
            nexacro.__setDOMStyle_TransformOriginObject(handle_style, this.transformOrigin);
        }
        if (this.animation)
        {
            nexacro.__setDOMStyle_AnimationObject(handle_style, this.animation);
        }
        if (this.animationPlayState)
        {
            nexacro.__setDOMStyle_AnimationPlayStateObject(handle_style, this.animationPlayState);
        }
        if (this.transition)
        {
            nexacro.__setDOMStyle_TransitionObject(handle_style, this.transition);
        }
    ...
    _pControlElement._getControlStyleStr = function ()
    {
        ...
        if (this.transform)
            str += nexacro.__getHTMLStyle_TransformObject(this.transform);

        if (this.transformOrigin)
            str += nexacro.__getHTMLStyle_TransformOriginObject(this.transformOrigin);

        if (this.animation)
            str += nexacro.__getHTMLStyle_AnimationObject(this.animation);

        if (this.animationPlayState)
            str += nexacro.__getHTMLStyle_AnimationPlayStateObject(this.animationPlayState);

        if (this.transition)
            str += nexacro.__getHTMLStyle_TransitionObject(this.transition);
    ...
    */
    ...
    _pScrollableControlElement.setLinkedControl = function (control)
    {
        if (!this.linkedcontrol && control)
        {
            ...
            if (!this.parent_elem)
            {
                if (!nexacro.isglobalmodule && this.linkedcontrol.__containerhandle)
                {
                    control._unique_id = this.linkedcontrol.__containerhandle.id + "." + control._unique_id;
                    //const appkey = control._getEnvironment()?.appkey;
                    //this._p_name = nexacro._prefixIDRootNodeClassName(control._unique_id, appkey);
    ...
    _pScrollableControlElement.create = function (win)
    {
        ...
        let control = this.linkedcontrol;
        if (!this._is_popup)
        {
            if (this.parent_elem)
            {
            ...
            else if (win.root == this.linkedcontrol)
            {
                ...
                if (!this.handle)
                {
                    ...
                    {
                        if (!control._isFluidContainer(true) || !nexacro._is_native_fluidlayout)
                            nexacro.__setDOMStyle_Pos(handle_style, this.left, this.top);
                    }
                    else
                    {
        ...
        {
            this._use_translate_scroll = control._use_translate_scroll_force;
        }
        else
        {
        ...
        this._use_native_scroll_size = control._use_native_scroll_size;
        ...
        if (this.handle)
        {
            ...
            if (!this._client_elem.handle)
                this._client_elem.create(win, step_count);
            if (step_count > 0)
            {
                this._step_containers = [];
                for (var i = 0; i < step_count; i++)
                {
                    var step_client = new nexacro._ContainerElement(this._client_elem, this._use_translate_scroll, "step" + i);
                    step_client._use_container_move = control._use_container_move;
                    step_client._use_native_scroll = control._use_native_scroll;
                    step_client._use_native_scroll_size = control._use_native_scroll_size;
    ...
    _pScrollableControlElement.initElementInfo = function ()
    {
        ...
        if (control._is_scrollable)
        {
            ...
            //this._scrolltype = "";  -> setElementScrollbarSize 함수에서 초기화
            //this._scrollpropagation = true;
            ...
            this.setElementScrollbarSize(control._getHScrollBarSize(), control._getVScrollBarSize(),
                control._getHScrollBarType(), control._getVScrollBarType(),
                control._getScrollType());//, control._getScrollPropagation());
        ...
        else if (control._is_expandable)
        {
            ...
            // init function
            if (control._use_expandbar !== false)
                this.setElementExpandbarSize(control._getExpandBarSize(), control._getExpandBarType(), control._getExpandDirType(), control._getExpandVisible(), control._getExpandArrange());
        }
        ...
        if (client_elemt)
        {
            ...
            client_elemt._use_native_scroll_size = this._use_native_scroll_size;
        }
    };
    ...
    _pScrollableControlElement.setElementStepCount = function (count)
    {
        if (this._step_count != count)
        {
            ...
            if (this.handle && count > 0)
            {
                ...
                for (i = 0; i < count; i++)
                {
                    step_client = new nexacro._ContainerElement(this._client_elem, this._use_translate_scroll, "step" + i);
                    //step_client.setElementPosition(0, 0);
                    //step_client.setElementPosition(i * this.client_width, 0);
    ...
    {
        if (this._use_native_scroll_size)// && width == undefined && height == undefined)
        {
            ...
            if (this._client_elem)
            {
                ...
                {
                    this.container_maxwidth = this._client_elem.getElementScrollWidth();
                    this.container_maxheight = this._client_elem.getElementScrollHeight();
                }
            }

            let step_count = this._step_count;
            let step_containers = this._step_containers;
            if (step_count > 0 && step_containers)
            {
                let client_elem = this.getContainerElement(this._step_index);
                if (client_elem)
                {
                    if (width == undefined && height == undefined)
                        this.container_maxheight = client_elem.getElementScrollHeight();
                    client_elem.setElementScrollMaxSize(this.client_width, this.container_maxheight);
                }
                /*
                let container_maxheight = height;
                let step_client_elem;
                for (let i = 0; i < step_count; i++)
                {
                    step_client_elem = step_containers[i];
                    container_maxheight = this.container_maxheight; //height ? height : step_client_elem.getElementScrollHeight();
                    step_client_elem.setElementScrollMaxSize(this.client_width, container_maxheight);
                }*/
            }

            if (this._client_elem)
            {
                this._client_elem.setElementScrollMaxSize(this.container_maxwidth, this.container_maxheight);

            }

            this._updateClientRect();
        }
        else if (this.container_maxwidth != width || this.container_maxheight != height)
        {
            this.container_maxwidth = width;
            this.container_maxheight = height;
    ...
    _pScrollableControlElement.setElementScrollbarSize = function (hscroll_size, vscroll_size, hscrollbartype, vscrollbartype, scrolltype)//, scrollpropagation)
    {
        ...
        if (this._scrolltype != scrolltype)
        {
            ...
            let container = this._client_elem;
            if (container)
                container._setElementScrollType(scrolltype);
            ret = true;
        }
    ...
    {
        if (this._use_native_scroll && (this._scrolltype != scrolltype))
        {
            this._scrolltype = scrolltype;
    ...
    _pScrollableControlElement.setElementScrollPropagation = function (scrollpropagation)
    {
        if (this._use_native_scroll && (this._scrollpropagation != scrollpropagation))
        {
            this._scrollpropagation = scrollpropagation;

            let container = this.getContainerElement();
            if (container)
                container._setElementScrollPropagation(scrollpropagation);
        }
    };
    ...
    _pScrollableControlElement.setElementSpinbarSize = function (incspinbarsize, decspinbarsize, hspinbartype, vspinbartype, scrolltype, spindirtype, spinarrange)
    {
        ...
        if (hspinbartype == "none" && vspinbartype == "none")
        {
            this._spinbarsize = this._incspinbarsize = this._decspinbarsize = 0;
        }
                ...
                if (step_count > 0 && step_containers)
                {
                    for (i = 0; i < step_count; i++)
                    {
                        ...
                        if (i == step_index)// && this._use_translate_scroll)
                            step_client_elem.setElementSize(zclient_width, this._zclient_height);
                    }
                    /*
    ...
    */
    ...
    _pScrollableControlElement._on_updateClientRect = function ()
    {
        ...
        */
        ...
        var expandbartype = this._expandbartype;
        var expanddirtype = this._expanddirtype;
        var expandarrange = this._expandarrange;
        ...
        if (nexacro._isAlmostGreater(container_maxwidth, zclient_width))
        {
            if (this._is_expand)
            {
                if ((zclient_width > 0) && (expandbartype != "auto" || (container_maxwidth - zclient_width) > overflow_diff))
                {
                    if (expanddirtype == "horz" || expanddirtype == "auto")
                    {
                        if (expandarrange.startsWith("lead"))
                        {
                            client_left += this._expandbarwidth;
                        }                    
            ...
            else if (this._is_scrollspin)
            {
                if (zclient_width > 0 && (container_maxwidth - zclient_width) > overflow_diff)
                {
                    if (/*(scrolltype != "none" && scrolltype != "vertical") &&*/ hspinbartype != "none")
                    {
                        if (spinarrange == "")
                        {
                            client_left += this._decspinbarsize;
                        }
        ...
        else
        {
            if (this._is_expand)
            {
                if (expandbartype == "fixed" || expandbartype == "const")
                {
                    if (zclient_width > 0)
                    {
                        if (expanddirtype == "horz" || expanddirtype == "auto")
        ...
        if (nexacro._isAlmostGreater(container_maxheight, zclient_height) && (has_contents_size ? nexacro._isAlmostGreater(contents_maxheight, zclient_height) : true))
        {
            ...
            if (this._is_expand)
            {
                ...
                {
                    if (this._expanddirtype == "vert" /*|| this._expanddirtype == "auto" && expandbartype != "none"*/)
                    {
                        if (expandarrange.startsWith("lead"))
                        {
                            client_top += this._expandbarheight;
                        }                    

                        client_height -= this._expandbarheight;
                        zclient_height = client_height / zoomfactor;
                        vscroll_limit = container_maxheight - zclient_height; //  vscroll_limit += this._expandbarheight;
                    }
                }
            }
            else if (this._is_scrollspin)
                ...
                //  if (/*(scrolltype != "none" && scrolltype != "horizontal") &&*/ vspinbartype != "none")
                    {
                        if (spinarrange == "")
                        {
                            client_top += this._decspinbarsize;
                        }
                        else if (spinarrange.startsWith("lead"))
                        {
                        ...
                        zclient_height = client_height / zoomfactor;
                        vscroll_limit = container_maxheight - zclient_height; //  vscroll_limit += this._spinbarsize;
                    }
                }
            }
            else
                ...
                if (this._is_expand)
                {
                    if ((zclient_width > 0) && (expandbartype != "auto" || (container_maxwidth - zclient_width) > overflow_diff))
                    {
                        if (expanddirtype == "horz" || expanddirtype == "auto")
                        {
                            ...
                            client_width -= this._expandbarwidth;
                            zclient_width = client_width / zoomfactor;
                            hscroll_limit = container_maxwidth - zclient_width; //  hscroll_limit += this._expandbarwidth;
                        }
                    }
                }
                else if (this._is_scrollspin)
        ...
        else
        {
            if (this._is_expand)
            {
                if (expandbartype == "fixed" || expandbartype == "const")
                {
                    if (zclient_height > 0)
                    {
                        if (expanddirtype == "vert" /*|| expanddirtype == "auto"*/)
                        {
                            if (expandarrange.startsWith("lead"))
                            {
                                client_top += this._expandbarheight;
    ...
    _pScrollableControlElement._on_updateClientRectExpand = function ()
    {
        ...
        switch(this._expandbartype)
        {
            ...
            case "fixed" :
            {
                ...
                if ((this._expanddirtype == "vert") && (this._expandbartype != "auto" || this.container_maxheight > this.zclient_height))
                {
                    client_height -= this._expandbarheight;
                    zclient_height = client_height / zoomfactor;
                    vscroll_limit  = this.container_maxheight - zclient_height; // + this._expandbarheight;
                }
                if ((this._expanddirtype == "horz") && (this._expandbartype != "auto" || this.container_maxwidth  > this.zclient_width))
                {
                    client_width -= this._expandbarwidth;
                    zclient_width = client_width / zoomfactor;
                    hscroll_limit = this.container_maxwidth - zclient_width; // + this._expandbarwidth;
                }
                ...
                if (this.hscroll_limit != hscroll_limit)
                {
                    ...
                    if (this.scroll_left > hscroll_limit && hscroll_limit >= 0)
                    {
                        this.setElementHScrollPos(hscroll_limit);
                    }
                }
                ...
                if (this.vscroll_limit != vscroll_limit)
                {
                    ...
                    if (this.scroll_top > vscroll_limit && vscroll_limit >= 0)
                    {
                        this.setElementVScrollPos(vscroll_limit);
                    }
                }

                this.zclient_width  = zclient_width;
                this.zclient_height = zclient_height;

                break;
            }
            case "const" :
            {
                if (this._expanddirtype == "vert")
                {
                    client_height -= this._expandbarheight;
                    zclient_height = client_height / zoomfactor;
                }
                if (this._expanddirtype == "horz")
                {
                    client_width -= this._expandbarwidth;
                    zclient_width = client_width / zoomfactor;
                }
    ...
    */
    ...
    _pFrameControlElement.create = function (win)
    {
        ...
        if (!this.handle)
        {
            if (this.parent_elem == null)
            {
                ...
                {
                    const rect = win.handle == document.body ? {left:0, top:0, width:nexacro._getWindowHandleClientWidth(win.handle), height:nexacro._getWindowHandleClientHeight(win.handle)} : nexacro._getContentRect(win.handle);
                   // const rect = nexacro._getContentRect(win.handle)
                    this.left = rect.paddingLeft || 0;
                    this.top = rect.paddingTop || 0;
                ...
                nexacro.__setHTMLAttr_Property(handle, "data-screen", env._getCurrentScreenType());                                
                handle._environment = env;
                ...
                if (nexacro._isDesktop() && (!env || !env._allow_default_pinchzoom))
                {
                    if (!nexacro._ishandleWindow(win.handle))
                    {
            ...
            else
            {
                ...
                if (owner_elem && owner_elem.handle && !this.handle)
                {
                    ...
                    {
                        if (nexacro._isDesktop() && (!env || !env._allow_default_pinchzoom))  // mobile에서 fixed 사용하면 안됨
                            nexacro.__setDOMStyle_Fixed(handle_style);
                        // else
                        //     nexacro.__setDOMStyle_Absolute(handle_style);
                    }
    ...
    _pModalOverlayElement.create = function (win)
    {
        ...
        if (this.parent_elem && !this.handle)
        {
            ...
            let width = nexacro.isglobalmodule ? win.clientWidth : nexacro._getWindowHandleClientWidth(win.handle);
            let height = nexacro.isglobalmodule ? win.clientHeight : nexacro._getWindowHandleClientHeight(win.handle);
    ...
    nexacro._ContainerElement = function (parent_elem, use_translate_scroll, type, use_onscroll_inner, use_onscroll_size_tempdiv)
    {
        this._p_parent = parent_elem;
        this.parent_elem = parent_elem;
        ...
        this._use_onscroll_size_tempdiv = (use_onscroll_size_tempdiv === false) ? false : true;
    };
    ...
    __pContainerElement._use_onscroll_size_tempdiv = true;
    ...
    __pContainerElement._scrolltype = "both";
    __pContainerElement._scrollpropagation = "both";
    ...
    var _bind_container_scroll_handler = function (elem)
    {
        return function (evt)
        {
            ...
            if (target._init_scroll_pos)
            {
                target._init_scroll_pos = undefined;
                return;
            }
            ...
            {
                // zoom 일떄 scrollTop 값이 정수로 설정해도 실수로 변경되는 경우가 있음
                if (((Math.abs(elem._scroll_top - scrolltop) >= 1) || (Math.abs(elem._scroll_left - scrollleft) >= 1)) && elem._use_onscroll_inner != true)
                {
                    if (nexacro._use_requestani)
                    {
                        const win = nexacro._findWindow(window);
                        nexacro._requestAnimationFrame(win, function () { elem._callback_func_onscroll(evt); });
                    }
                    else
                ...
                {
                    elem._scroll_left = scrollleft;
                    elem._scroll_top = scrolltop;
    ...
    __pContainerElement._callback_func_onscroll = function (evt)
    {
        ...
        if (container)
        {
            ...
            {
                nexacro._ContainerElement.prototype._syshandler_onscroll_forward.call(container, evt);
            }
        }
    };

    __pContainerElement.create = function (win, steproot = false)
    {
        var owner_elem = this.parent_elem;
        if (owner_elem && owner_elem.handle && !this.handle)
        {
            ...
            let stepcontainer = false;
            if (this._use_native_scroll)
            {
                //추후 변경
                nexacro.__setDOMNode_Id(handle, "", this.name);
                nexacro.__setDOMStyle_ScrollbarWidth(handle.style, "none");
                let strextclass = "nexanativescroll";
                if (steproot)
                {
                    strextclass += " nexasteprootcontainer";
                    if (this._scrolltype == "none" || this._scrolltype == "vertical")
                        nexacro.__setDOMStyle_Overflow(handle.style, this._scrolltype);
                }
                else if (this.type.indexOf("step") >= 0)
                {
                    strextclass += " nexastepcontainer";
                    stepcontainer = true;
                }
                else
                {
                ...
                nexacro.__setDOMNode_ClassName(handle, this._getElementNexaClassName("nexacontainer", strextclass));
            ...
            if (stepcontainer)
                nexacro.__setDOMStyle_Height(handle_style, this.height);
            else
                nexacro.__setDOMStyle_Size(handle_style, this.width, this.height);
            ...
            if (owner_elem.dest_handle.hasChildNodes() && stepcontainer == false && (nexacro._isNull(this.type) || this.type.length < 1)) 
            {
    ...
    __pContainerElement.createCommandStart = function ()
    {
        ...
        if (owner_elem)
        {
            ...
            if (this._use_native_scroll)
            {
                strextraclass += "nexanativescroll" + (this.type.indexOf("step") >= 0 ? " nexastepcontainer" : "");
            }

            str += " class='" + this._getElementNexaClassName("nexacontainer", strextraclass) + "'";

            str += " id='" + this.name + "' ";
            var style_str = this._getCommonStyleStr();
            if (this._use_native_scroll)    
            {
                //  style_str += nexacro.__getHTMLStyle_Overflow(this._scrolltype); 
                style_str += nexacro.__getHTMLStyle_Overflow(this._scrolltype) + "scrollbar-width:none;"
                style_str += (this._scrollpropagation != "both") ? nexacro.__getHTMLStyle_OverscrollBehavior(this._scrollpropagation) : "";
            }
    ...
    __pContainerElement._getCommonStyleStr = function ()
    {
        var str = nexacro.Element.prototype._getCommonStyleStr.call(this);
        ...
        switch (this._layouttype)
        {
            ...
            case "vertical":
                ...
                var gap_style = nexacro.__getHTMLStyle_LayoutGap(this._layouthorizontalgap, this._layoutverticalgap);
                if (gap_style)
                    str += gap_style;
                if (this._layoutspacing)
            ...
            case "table":
                str += nexacro.__getHTMLStyle_CompLayoutType(this._layouttype);
                gap_style = nexacro.__getHTMLStyle_LayoutGap(this._layouthorizontalgap, this._layoutverticalgap);
                if (gap_style)
                    str += gap_style;
                if (this._layoutspacing)
                    str += nexacro.__getHTMLStyle_LayoutSpacing(this._layoutspacing);
    ...
    __pContainerElement.attachHandle = function (win)
    {
        if (this.name)
        {
            ...
            if (handle)
            {
                ...
                if (!this._use_translate_scroll && !(this._use_native_scroll && this._use_native_scroll_size))
                {
                    if (this._use_onscroll_size_tempdiv)
                    {
        ...
        if (handle)
        {
            ...
            switch (this._layouttype)
            {
                ...
                case "vertical":
                    ...
                    if (this._layoutspacing)
                        nexacro.__setDOMStyle_LayoutSpacing(dest_handle.style, this._layoutspacing);
        ...
        if (this._scroll_top != vpos || this.parent._reset_scrollpos || this._use_container_move)
        {
            ...
            else if (this._use_container_move)
            {
                ...
                if (handle)
                {
                    ...
                    if (this._cached_scrollTop != vpos || handle.scrollTop != vpos)
                        nexacro.__setDOMNode_VScrollPos(handle, vpos);
                }
            }
        }
    ...
    __pContainerElement._setElementHScrollPos = function (hpos)
    {
        if (this._scroll_left != hpos || this.parent._reset_scrollpos || this._use_container_move)
        {
            ...
            else if (this._use_container_move)
            {
                ...
                {
                    //아래 api 호출후 onscroll이벤트가 발생하기 전에 이 api를 다시 호출해야 하는 상황이 있다.
                    if (this._cached_scrollLeft != hpos || handle.scrollLeft != hpos)
                        nexacro.__setDOMNode_HScrollPos(handle, hpos, this._isRtl());
                }
            }
        }
    };

    __pContainerElement._setElementScrollPropagation = function (scrollpropagation)
    {
        if (this._scrollpropagation != scrollpropagation)
        {
            this._scrollpropagation = scrollpropagation;
            if (this.handle)
            {
                nexacro.__setDOMStyle_OverscrollBehavior(this.handle.style, scrollpropagation);
            }
        }
    };
    ...
    __pContainerElement._setElementScrollType = function (scrolltype)
    {
        if (this._scrolltype != scrolltype)
        {
            ...
            if (this.handle)
            {
                nexacro.__setDOMStyle_Overflow(this.handle.style, scrolltype);
            }
        }
    };
    ...
    __pContainerElement.setElementScrollMaxSize = function (width, height)
    {
        ...
        {
            if (this.handle && !(this._use_native_scroll && this._use_native_scroll_size))
            {
                if (this._use_onscroll_size_tempdiv)
                {
                    var tempdiv = this._tempdiv;
    ...
    {
        return (this.handle && this._use_native_scroll_size) ? this.handle.scrollLeft : this._scroll_left;
    };
    __pContainerElement.getElementScrollTop = function ()
    {
        return (this.handle && this._use_native_scroll_size) ? this.handle.scrollTop : this._scroll_top;
    };

    __pContainerElement.getElementScrollWidth = function ()
    {
        return (this.handle && this._use_native_scroll_size) ? this.handle.scrollWidth : this._scroll_maxwidth;
    };
    __pContainerElement.getElementScrollHeight = function ()
    {
        return (this.handle && this._use_native_scroll_size) ? this.handle.scrollHeight : this._scroll_maxheight;
    };
    ...
    __pContainerElement.setElementSize = function (width, height)
    {
        if (this.width != width || this.height != height)
        {
            this.width = width;
            ...
            if (handle)
            {
                if (this.type.indexOf("step") >= 0)
                    nexacro.__setDOMStyle_Height(handle.style, height);
                else
                    nexacro.__setDOMStyle_Size(handle.style, width, height);
    ...
    __pContainerElement._syshandler_onscroll_forward = function (evt)
    {
        ...
        if (container)
        {
            ...
            container._cached_scrollLeft = Math.floor(this._getScrollLeft(target));
            container._cached_scrollTop = Math.floor(this._getScrollTop(target));
            container._scroll_maxwidth = target.scrollWidth;
            container._scroll_maxheight = target.scrollHeight;
            ...
            if (this._isRtl()) // RTL인 경우 scrollLeft는 음수값이어서 0 으로 보정되어버리므로 양수값으로 처리
            {
                hscroll_pos = -hscroll_pos;
            }
            ...
            if (linkedcontrol)
            {
                let win = linkedcontrol._getWindow();
                let iswheel = linkedcontrol._is_wheel_scroll;
                let evtkind;
                // if (win )//&& performance.now() - win._lastwheeltime < 150)                
                // {                    
                //     if (win._lastwheeltime.startscrollpos + win._lastwheeltime.endscrollpos < container._cached_scrollTop && win._lastwheeltime.endscrollpos > container._cached_scrollTop)
                //         iswheel = true;         
                // }

                let scrolltype = linkedcontrol._getScrollType();
                if (elem_scroll_left != container._cached_scrollLeft)
                {
                    if (scrolltype != "none" && scrolltype != "vertical") // (linkedcontrol.scrollbartype != "none" && linkedcontrol.scrollbartype != "vertical"))
                    {
                        bhscroll = true;
                        evtkind = iswheel ? "mousewheel_h" : "slide";
                    }
                }

                if (elem_scroll_top != container._cached_scrollTop)
                {
                    if (scrolltype != "none" && scrolltype != "horizontal")// (linkedcontrol.scrollbartype != "none" && linkedcontrol.scrollbartype != "horizontal"))
                    {
                        bvscroll = true;
                        evtkind = iswheel ? "mousewheel_v" : "slide";
                    }
                }

                if (bhscroll || bvscroll)
                {
                    linkedcontrol._scrollTo(bhscroll ? hscroll_pos : null, bvscroll ? vscroll_pos : null, undefined, true, "", evtkind, true);
                }
            }
        }
    };

    __pContainerElement._getScrollLeft = function (node)
    {
        return node.scrollLeft;
    };

    __pContainerElement._getScrollTop = function (node)
    {
    ...
    {
        const nativeclassname = this._getElementNativeClassName();
        return this._use_native_scroll ? nativeclassname + this._getElementNexaClassName("nexacontainer", "nexanativescroll") : nativeclassname + this._getElementNexaClassName("nexacontainer");
    ...
    __pWebBrowserPluginElement.create = function (win)
    {
        ...
        if (owner_elem && owner_elem.handle && !this.handle)
        {
            ...
            var handle = this._createInternalFrameElementHandle(_doc, this.left, this.top, this.width, this.height, this._params, env);
...
//	        if (this.left || this.top)
    ...
    __pWebBrowserPluginElement._go_prev_focus = function (evt)
    {
        ...
        // tabkey focus
        var newfocus_comp = form._searchPrevTabFocus(form._last_focused, undefined, undefined, 0);
    ...
    __pWebBrowserPluginElement._createInternalFrameElementHandle = function (_doc, left, top, width, height, params, env)
    {
    ...
    {
        const env = this._linked_element.parent_elem.linkedcontrol._getEnvironment();

        if (env._communication_waiting) nexacro._stopSysEvent(evt);
    ...
    {
        const next_outfocus_elem = this._next_outfocus_message_elem;
        if (next_outfocus_elem)
        {
            // leavemessage 설정 여부에 따라 hidden 하여 스크린리더의 음성 출력 제어
            let is_hidden = (typeof message === "string" && message.length > 0) ? false : true;
            next_outfocus_elem.setElementAccessibilityStatHidden(is_hidden);
    ...
    */
    ...
    __pWebViewPluginElement._createInternalFrameElementHandle = function (_doc, left, top, width, height, params, env)
    {
        var handle = _doc.createElement("iframe");
    ...
    __pWebViewPluginElement._removeInternalFrameDOMParam = function (_doc, target_handle, name)
    {
        nexacro.__removeHTMLAttr_Property(target_handle, name);
    };
    ...
    __pWebViewPluginElement.setElementParam = function (name, value)
    {
        if (this.handle)
        {
            ...
            {
                this._removeInternalFrameDOMParam(_doc, this.handle, name);
            }
        }
        else
        {
    ...
    __pWebViewPluginElement._setContent = function (content)
    {
        if (this.handle)
        {
            this.setElementParam("src");
            this.setElementParam("srcdoc", content);
        }
    };
    ...
    */
    ...
    __pVideoPlayerPluginElement.destroy = function ()
    {
        ...
        {            
            var handle = this.handle;            
            nexacro._stopSysObserving(handle, "canplaythrough", "oncanplaythrough", this._video_eventhandler_canplaythrough);
            nexacro._stopSysObserving(handle, "durationchange", "ondurationchange", this._video_eventhandler_durationchange);

            //nexacro._observeSysEvent(handle, "emptied", "onemptied", this._video_eventhandler_emptied);
            nexacro._stopSysObserving(handle, "ended", "onended", this._video_eventhandler_changedstatus);
            nexacro._stopSysObserving(handle, "error", "onerror", this._video_eventhandler_error);

            //nexacro._observeSysEvent(handle, "loadeddata", "onloadeddata", this._video_eventhandler_loadeddata);
            //nexacro._stopSysObserving(handle, "loadedmetadata", "onloadedmetadata", this._video_eventhandler_loadedmetadata);
            //nexacro._observeSysEvent(handle, "loadstart", "onloadstart", this._video_eventhandler_loadstart);

            nexacro._stopSysObserving(handle, "pause", "onpause", this._video_eventhandler_changedstatus);
            nexacro._stopSysObserving(handle, "play", "onplay", this._video_eventhandler_changedstatus);
            //nexacro._observeSysEvent(handle, "playing", "onplaying", this._video_eventhandler_playing);
            nexacro._stopSysObserving(handle, "progress", "onprogress", this._video_eventhandler_progress);
            //nexacro._observeSysEvent(handle, "ratechange", "onratechange", this._video_eventhandler_ratechange);

            //nexacro._stopSysObserving(handle, "seeked", "onseeked", this._video_eventhandler_seeked);
            //nexacro._stopSysObserving(handle, "seeking", "onseeking", this._video_eventhandler_seeking);
            //nexacro._observeSysEvent(handle, "stalled", "onstalled", this._video_eventhandler_stalled);
            //nexacro._stopSysObserving(handle, "suspend", "onsuspend", this._video_eventhandler_suspend);

            nexacro._stopSysObserving(handle, "timeupdate", "ontimeupdate", this._video_eventhandler_timeupdate);
        }
        ...
        if (handle)
        {
            ...
            if (imagesource && imagesource instanceof nexacro.CanvasElement)
            {
                ...
                if (url)
                {
                    ...
                    imgObj.src = url;  
                    imgObj.handle = nexacro._getImageObject_extended(url, pattern.onloadcallback, pattern);  //check trans Nexaro K (imagefullurl, text)
            ...
            else if (imagesource && imagesource instanceof nexacro.Image)
            {
                ...
                if (imagesource.src)
                {
                    imagesource.handle = nexacro._getImageObject_extended(imagesource.src, pattern.onloadcallback, pattern);  //check trans Nexaro K (imagefullurl, text)
                }
                else
                {
    ...
    */
    ...
    nexacro.GridScrollableControlElement = function (parent_elem)
    {
        ...
        this._client_elem._getScrollLeft = function (node)
        {
            var prev = this.parent._getContainerScrollCenterPos();
            var move = node.scrollLeft - prev;

            node._init_scroll_pos = true;
            node.scrollLeft = prev;

            return this.parent._getScrollLeft() + move;
        };

        this._client_elem._getScrollTop = function (node)
        {
            var prev = this.parent._getContainerScrollCenterPos();
            var move = node.scrollTop - prev;

            node._init_scroll_pos = true;
            node.scrollTop = prev;

            return this.parent._getScrollTop() + move;
        };
    };
    ...
    _pGridScrollableControlElement._getContainerScrollCenterPos = function ()
    {
        var grid = this.linkedcontrol;
        return Math.round((grid._div_max_height - this.height) / 2);
    };
    ...
    _pGridScrollableControlElement._on_attachHandle = function (win)
    {
        ...
        if (this._client_elem.handle)
        {
            this._client_elem.handle._init_scroll_pos = true;
            this._client_elem.handle.scrollLeft = this._client_elem.handle.scrollTop = this._getContainerScrollCenterPos();
        }
    };
    ...
    _pGridScrollableControlElement._setContainerMaxHeight = function (height)
    {
        ...
        if (vert_elems)
        {
            ...
            var grid = this.linkedcontrol;
            this._client_elem.setElementScrollMaxSize(grid._div_max_height, grid._div_max_height);

            if (this._client_elem.handle)
            {
                this._client_elem.handle._init_scroll_pos = true;
                this._client_elem.handle.scrollTop = this._getContainerScrollCenterPos();
            }
        }
    };
    ...
    _pGridScrollableControlElement._setContainerMaxWidth = function (width)
    {
        ...
        if (horz_elems)
        {
            ...
            {
                this._client_elem.handle._init_scroll_pos = true;
                this._client_elem.handle.scrollLeft = this._getContainerScrollCenterPos();
            }
        }
    };
    ...
    _pGridScrollableControlElement._getScrollTop = function ()
    {
        var v_element = this._target_vscroll_elements;
        if (v_element)
        {
            ...
            return v_element.scroll_top;
        }
    };

    _pGridScrollableControlElement._getScrollLeft = function ()
    {
        var h_element = this._target_hscroll_elements;
        if (h_element)
        {
    ...
    _pGridBandControlElement._use_sticky = true;
    ...
    * position 이동  : 터치시 화면반응속도 중간, 스크롤속도 중간, 화면출렁거림 거의없음 (노드량이 많으면 느려질 수 있음)
    ...
    nexacro.GridRowControlElement = function (parent_elem, left_gap)
    {
        ...
        const env = parent_elem?.linkedcontrol?._getEnvironment() || undefined;

        this._client_body_element = new nexacro._ContainerElement(this, use_inner_move, "body");

        // 모바일 접근성에서는 scroll 동작이 필요함
        this._client_body_element._no_focus_scroll = (env?._p_enableaccessibility && env?._a11ytype > 3) ? false : true;
    ...
    _pGridRowControlElement.createCommandStart = function ()
    {
        ...
        if (owner_elem && !this.handle)
        {
            ...
            var str = "<div id='" + this.name + "' class='" + this._getElementClassName() + "'" + nexacro.__getHTMLAttr_Dir(this.rtl);
            if (!nexacro._isNull(this.tooltiptext))
    ...
    nexacro.TabBarItemSetControlElement = function (parent_elem)
    {
        ...
        this._scroll_container_element._use_native_scroll = true;
        this._scroll_container_element._use_native_scroll_size = true;
    ...
    _pTabBarItemSetControlElement.setElementScrollMaxSize = function (width, height)
    {
        if (this._scroll_container_element)
        {
            this.container_maxwidth = this._scroll_container_element.getElementScrollWidth();
            this.container_maxheight = this._scroll_container_element.getElementScrollHeight();

            this._scroll_container_element.setElementScrollMaxSize(this.container_maxwidth, this.container_maxheight);
        }
    ...
    //=======================================================================================
    // nexacro.IconTextControlElement : Icon / Text / IconText 를 위한 Atomic ControlElement
    //=======================================================================================
    nexacro.IconTextControlElement = function (parent_elem)
    {
        this._p_parent = parent_elem;
        this.parent_elem = parent_elem;
    };

    var _pIconTextControlElement = nexacro._createPrototype(nexacro.ControlElement, nexacro.IconTextControlElement);
    nexacro.IconTextControlElement.prototype = _pIconTextControlElement;

    _pIconTextControlElement._type_name = "IconTextControlElement";
    ...
    const ContentType = Object.freeze({
        ...
        TEXT: Symbol("TEXT"),
        ICON: Symbol("ICON"),
        ICON_TEXT: Symbol("ICON_TEXT"),
        ICON_TEXT_OPTIMIZED: Symbol("ICON_TEXT_OPTIMIZED")
    });

    const ContentSuffix = Object.freeze({
        FLEXITEM: ":flexitem",
        TEXT: ":text",
        ICON: ":icon",
    });
    ...
    //==== defaule values =========//
    // basic element attrubutes
    _pIconTextControlElement.text = "";
    _pIconTextControlElement.icon = null;
    _pIconTextControlElement.iconwidth = "";
    _pIconTextControlElement.iconheight = "";
    _pIconTextControlElement.iconPos = "";
    _pIconTextControlElement.textAlign = null;
    _pIconTextControlElement.verticalAlign = null;
    _pIconTextControlElement.textwidth = 0;
    _pIconTextControlElement.userSelect = null;

    _pIconTextControlElement.wordWrap = null;
    _pIconTextControlElement._wordwrap_info = null;
    _pIconTextControlElement.textOverflow = null;
    _pIconTextControlElement._textoverflow_info = null;

    _pIconTextControlElement.iconAnimation = null;
    _pIconTextControlElement.iconAnimationPlayState = null;
    _pIconTextControlElement.iconTransform = null;
    _pIconTextControlElement.iconTransformOrigin = null;
    _pIconTextControlElement.iconTransition = null;

    _pIconTextControlElement.textAnimation = null;
    _pIconTextControlElement.textAnimationPlayState = null;
    _pIconTextControlElement.textTransform = null;
    _pIconTextControlElement.textTransformOrigin = null;
    _pIconTextControlElement.textTransition = null;

    // internal use
    _pIconTextControlElement._content_type = ContentType.EMPTY;
    _pIconTextControlElement._flexitem_node = null; // icon node, text node 간 정렬용
    _pIconTextControlElement._icon_node = null;
    _pIconTextControlElement._text_node = null;
    _pIconTextControlElement._use_decoration = false;

    //==== default flags =========//
    _pIconTextControlElement._use_newline = true;
    //==== end of initial =========//

    _pIconTextControlElement._getElementClassName = function (tagname)
    {
        this._setAddSelector();

        return nexacro.ControlElement.prototype._getElementClassName.call(this, tagname);
    };

    _pIconTextControlElement.create = function (win)
    {
        let _doc, owner_elem, handle;
        let handle_style;
        ...
        if (owner_elem && owner_elem.handle)
        {
            if (!this.handle)
            {
                const control = this.linkedcontrol;
                this.owner_elem = owner_elem;
                _doc = win._doc || owner_elem._getRootWindowHandle();
                ...
                this._setAddSelector();
                nexacro.__setDOMNode_ClassName(handle, this._getElementClassName("div"));
                nexacro.__setDOMNode_Dir(handle, this.rtl);

                handle_style = handle.style;

                if (!control || !control._isFluidContainer(true) || !nexacro._is_native_fluidlayout)
                    nexacro.__setDOMStyle_Pos(handle_style, this.left, this.top);
                ...
                if (this.linkedcontrol && this.linkedcontrol._is_abstract)
                    return;

                // create contents
                this._createContents(handle);

                //iOS 
                //browser scroll 을 사용하도록 
                if (nexacro._OS == "iOS" && !control._isAllowDefaultPinchZoom())
                    nexacro.__setDOMStyle_TouchAction(handle_style, "manipulation");
                ...
                {
                    const owner_handle = (this._is_nc_element) ? owner_elem.handle : owner_elem.dest_handle;
                    if (control && control._firstchild)
                        nexacro.__insertDOMNode(owner_handle, handle, owner_handle.firstChild);
                    else
                        nexacro.__appendDOMNode(owner_handle, handle);
    ...
    _pIconTextControlElement.createCommandStart = function ()
    {
        const owner_elem = (this._is_nc_element) ? this.parent_elem : this.parent_elem.getContainerElement(this.position_step);
        if (owner_elem && !this.handle)
        {
            let str = "";
            let style_str = this._getControlStyleStr();
            let status_str = this._getControlStatusStr();
            const flexdirection = this._getFlexDirection();

            this.owner_elem = owner_elem;
            this._setAddSelector(flexdirection);
            ...
            if (this.textAlign)
            {
                style_str += nexacro.__getHTMLStyle_textAlign_IconText(this.textAlign);
                style_str += nexacro.__getHTMLStyle_textAlign(this.textAlign);
            }
            if (this.userSelect)
                style_str += nexacro.__getHTMLStyle_UserSelect(this.userSelect);
            if (this.verticalAlign)
                style_str += nexacro.__getHTMLStyle_verticalAlign_IconText(this.verticalAlign);
            if (this.padding)
                style_str += nexacro.__getHTMLStyle_PaddingObject(this.padding);

            if (nexacro._OS == "iOS")
            {
                var linkedcontrol = this.linkedcontrol;
                if (linkedcontrol && !linkedcontrol._isAllowDefaultPinchZoom())
                    style_str += nexacro.__getHTMLStyle_TouchAction("manipulation"); //disable the double-tap
            }
            ...
            if (this._getContentsType() === ContentType.ICON_TEXT)
            {
                str += "<div id='" + this.name + ContentSuffix.FLEXITEM + "' class='" + this._getFlexItemClassName(flexdirection) + "'>";
                str += this._createContentsCommand();
                str += "</div>";
            }
            else // ICON, TEXT, ICON_TEXT_OPTIMIZED
            {
                str += this._createContentsCommand();
            }
            return str;
    ...
    _pIconTextControlElement.createCommandEnd = function ()
    {
        return "</div>";
    };

    _pIconTextControlElement.attachHandle = function (win)
    {
        if (this.name)
        {
            const handle = this.handle || win._doc.getElementById(this.name);
            if (handle)
            {
                handle._linked_element = this;
                this.handle = this.dest_handle = handle;
                ...
                // for edge
                const edge_elem = this._edge_elem;
                if (edge_elem)
                    edge_elem.attach_handle_fromparent(handle);

                // for contents
                const flexitem_node = win._doc.getElementById(this.name + ContentSuffix.FLEXITEM);
                if (flexitem_node)
                {
                    flexitem_node._linked_element = this;
                    this._flexitem_node = flexitem_node;
                }
                const icon_node = win._doc.getElementById(this.name + ContentSuffix.ICON);
                if (icon_node)
                {
                    icon_node._linked_element = this;
                    this._icon_node = icon_node;
                }
                const text_node = win._doc.getElementById(this.name + ContentSuffix.TEXT);
                if (text_node)
                {
                    text_node._linked_element = this;
                    this._text_node = text_node;
                }
                this._setAccessibility_notify();
                this._on_attachHandle(win);
    ...
    _pIconTextControlElement._createContentsCommand = function ()
    {
        const owner_elem = this.parent_elem.getContainerElement(this.position_step);
        if (owner_elem && !this.handle)
        {
            this.owner_elem = owner_elem;

            const contentsType = this._content_type = this._getContentsType();
            if (contentsType != ContentType.EMPTY)
            {
                let command = "";
                if (contentsType != ContentType.TEXT)
                {
                    let icon_style = "";
                    if (this.icon)
                    {
                        icon_style += nexacro.__getHTMLStyle_ContentUrl(this.icon._sysurl);
                        icon_style += nexacro.__getHTMLStyle_ContentSize(this.iconwidth, this.iconheight);
                    }
                    if (this.iconAnimation)
                        icon_style += nexacro.__getHTMLStyle_AnimationObject(this.iconAnimation);
                    if (this.iconAnimationPlayState)
                        icon_style += nexacro.__getHTMLStyle_AnimationPlayStateObject(this.iconAnimationPlayState);
                    if (this.iconTransform)
                        icon_style += nexacro.__getHTMLStyle_TransformObject(this.iconTransform);
                    if (this.iconTransformOrigin)
                        icon_style += nexacro.__getHTMLStyle_TransformOriginObject(this.iconTransformOrigin);
                    if (this.iconTransition)
                        icon_style += nexacro.__getHTMLStyle_TransitionObject(this.iconTransition);

                    command += "<i id='" + this.name + ContentSuffix.ICON + "' " + nexacro.__getDOMNode_Alt("");
                    command += (icon_style) ? (" style='" + icon_style + "'") : "";
                    command += " class='" + this._getElementNexaClassName("nexaiconitem", undefined, "i") + "'></i>";
                }
                if (contentsType != ContentType.ICON)
                {
                    let text_style = "";
                    let value_str = "";
                    const dataAttrs = [];
                    ...
                    if (this._use_decoration)
                        ...
                        value_str = nexacro.__getHTMLAttr_Text(this.text);
                    ...
                    if (this.textAnimation)
                        text_style += nexacro.__getHTMLStyle_AnimationObject(this.textAnimation);
                    if (this.textAnimationPlayState)
                        text_style += nexacro.__getHTMLStyle_AnimationPlayStateObject(this.textAnimationPlayState);
                    if (this.textTransform)
                        text_style += nexacro.__getHTMLStyle_TransformObject(this.textTransform);
                    if (this.textTransformOrigin)
                        text_style += nexacro.__getHTMLStyle_TransformOriginObject(this.textTransformOrigin);
                    if (this.textTransition)
                        text_style += nexacro.__getHTMLStyle_TransitionObject(this.textTransition);

                    if (this._isWordWrapNone())
                        dataAttrs.push(nexacro.__getHTMLDataset_TextOverflow(this.textOverflow || this._textoverflow_info));

                    const attrString = dataAttrs.filter(Boolean).join(" ");

                    command += "<div id='" + this.name + ContentSuffix.TEXT + "' class='nexatextitem'";
                    command += attrString ? (" " + attrString) : "";
                    command += (text_style) ? (" style='" + text_style + "'>") : ">";
                    command += value_str;
                    command += "</div>";
                }
                return command;
    ...
    _pIconTextControlElement._createContents = function (handle)
    {
        const contentsType = this._content_type = this._getContentsType();
        if (contentsType != ContentType.EMPTY)
        {
            if (handle)
            {
                ...
                let text_node = null, icon_node = null, flexitem_node = null;

                if (contentsType === ContentType.ICON_TEXT) // icon + text (not optimized) 이면 flexitem 노드 생성
                {
                    flexitem_node = doc.createElement("div");

                    nexacro.__setDOMNode_Id(flexitem_node, this.name, ContentSuffix.FLEXITEM);
                    nexacro.__setDOMNode_ClassName(flexitem_node, this._getFlexItemClassName());
                    this._flexitem_node = flexitem_node;

                    nexacro.__appendDOMNode(handle, flexitem_node);

                    handle = flexitem_node; // switch handle to flexitem_node
                }

                if (contentsType != ContentType.TEXT)
                {
                    icon_node = doc.createElement("i");

                    nexacro.__setDOMNode_Id(icon_node, this.name, ContentSuffix.ICON);
                    nexacro.__setDOMNode_ClassName(icon_node, this._getElementNexaClassName("nexaiconitem", undefined, "i"));
                    nexacro.__setDOMNode_Alt(icon_node, "");

                    if (this.icon)
                    {
                        nexacro.__setDOMStyle_ContentUrl(icon_node.style, this.icon._sysurl);
                        nexacro.__setDOMStyle_ContentSize(icon_node.style, this.iconwidth, this.iconheight);
                    }
                    if (this.iconAnimation)
                        nexacro.__setDOMStyle_AnimationObject(icon_node.style, this.iconAnimation);
                    if (this.iconAnimationPlayState)
                        nexacro.__setDOMStyle_AnimationPlayStateObject(icon_node.style, this.iconAnimationPlayState);
                    if (this.iconTransform)
                        nexacro.__setDOMStyle_TransformObject(icon_node.style, this.iconTransform);
                    if (this.iconTransformOrigin)
                        nexacro.__setDOMStyle_TransformOriginObject(icon_node.style, this.iconTransformOrigin);
                    if (this.iconTransition)
                        nexacro.__setDOMStyle_TransitionObject(icon_node.style, this.iconTransition);

                    this._icon_node = icon_node;

                    nexacro.__appendDOMNode(handle, icon_node);
                }
                if (contentsType != ContentType.ICON)
                {
                    text_node = doc.createElement("div");

                    nexacro.__setDOMNode_Id(text_node, this.name, ContentSuffix.TEXT);
                    nexacro.__setDOMNode_ClassName(text_node, this._getElementNexaClassName("nexatextitem"));
                    if (this.textPadding)
                        nexacro.__setDOMStyle_PaddingObject(text_node.style, this.textPadding);
                    if (this.textwidth)
                        nexacro.__setDOMStyle_TextWidth(text_node.style, parseInt(this.textwidth));
                    if (this._use_decoration)
                        nexacro.__setDOMNode_DecorateText(text_node, this.text);
                    else
                        nexacro.__setDOMNode_Text(text_node, this.text, this.wordWrap || this._wordwrap_info);
                    ...
                    if (this.textAnimation)
                        nexacro.__setDOMStyle_AnimationObject(text_node.style, this.textAnimation);
                    if (this.textAnimationPlayState)
                        nexacro.__setDOMStyle_AnimationPlayStateObject(text_node.style, this.textAnimationPlayState);
                    if (this.textTransform)
                        nexacro.__setDOMStyle_TransformObject(text_node.style, this.textTransform);
                    if (this.textTransformOrigin)
                        nexacro.__setDOMStyle_TransformOriginObject(text_node.style, this.textTransformOrigin);
                    if (this.textTransition)
                        nexacro.__setDOMStyle_TransitionObject(text_node.style, this.textTransition);

                    this._text_node = text_node;

                    nexacro.__appendDOMNode(handle, text_node);
                }

                this._updateContentsAll(handle.style);
            }
        }
    };

    _pIconTextControlElement._destroyContents = function ()
    {
        if (this._flexitem_node && this._flexitem_node.parentNode)
        {
            this._flexitem_node.remove();
        }
        if (this._icon_node && this._icon_node.parentNode)
        {
            this._icon_node.remove();
        }
        if (this._text_node && this._text_node.parentNode)
        {
            this._text_node.remove();
        }

        this._flexitem_node = null;
        this._icon_node = null;
        this._text_node = null;
    }

    _pIconTextControlElement._recreateContents = function ()
    {
        if (!this.handle)
            return;

        const contentsType = this._getContentsType();
        if (this._content_type !== contentsType)
        {
            this._destroyContents();
            this._createContents(this.handle);
        }
    };

    _pIconTextControlElement._getContentsType = function ()
    {
        // 0: empty, 1: text only, 2: icon only, 3: icon + text, 4: icon + text (optimized)
        var { icon, text, iconPos, verticalAlign, textAlign } = this;

        if (!icon)
            icon = this.linkedcontrol?._getCSSStyleValue("icon"); // get from CSS if not set property

        if (icon && icon._sysurl)
        {
            if (!text)
            ...
            if (text)
            {
                if ((!iconPos || iconPos === "left" || iconPos === "right") && (!verticalAlign || verticalAlign !== "middle"))
                    return ContentType.ICON_TEXT;
            }
            return ContentType.ICON_TEXT_OPTIMIZED;
        }
        return text ? ContentType.TEXT : ContentType.EMPTY;
    };

    _pIconTextControlElement._isWordWrapNone = function ()
    {
        const val = this.wordWrap || this._wordwrap_info;
        return (val === "none" || val === "" || val == null);
    };

    _pIconTextControlElement._getFlexItemClassName = function (flexdirection)
    {
        const _flexdirection = flexdirection || this._getFlexDirection();

        return "nexaflexitem" + (_flexdirection.startsWith("column") ? " column" : "");
    };

    _pIconTextControlElement._getFlexDirection = function ()
    {
    ...
    _pIconTextControlElement._setAddSelector = function (flexdirection)
    {
        const _flexdirection = flexdirection || this._getFlexDirection();

        this._addselector = "nexaicontext" + (_flexdirection.startsWith("column") ? " column" : "") + (_flexdirection.endsWith("reverse") ? " reverse" : "");
    };

    _pIconTextControlElement._updateFlexDirection = function (flexdirection)
    {
        const _flexdirection = flexdirection || this._getFlexDirection();
        const handle = this.handle;
        if (handle)
        {
            handle.classList.toggle("column", _flexdirection.startsWith("column"));
            handle.classList.toggle("reverse", _flexdirection.endsWith("reverse"));
        }
    };

    _pIconTextControlElement._updateContentsIconPos = function (handle_style, iconPos, flexdirection)
    {
        if (iconPos)
        {
            this._updateFlexDirection(flexdirection);

            const flexitem_node = this._flexitem_node;
            if (flexitem_node)
            {
                nexacro.__setDOMNode_ClassName(flexitem_node, this._getFlexItemClassName(flexdirection));
            }
        }
    };

    _pIconTextControlElement._updateContentsVerticalAlign = function (handle_style, verticalAlign)
    {
        if (verticalAlign)
        {
            nexacro.__setDOMStyle_verticalAlign_IconText(handle_style, verticalAlign);
        }
    };

    _pIconTextControlElement._updateContentsTextAlign = function (handle_style, textAlign)
    {
        if (textAlign)
        {
            nexacro.__setDOMStyle_textAlign_IconText(handle_style, textAlign);
            nexacro.__setDOMStyle_textAlign(handle_style, textAlign);
        }
    };

    _pIconTextControlElement._updateContentsPadding = function (handle_style, padding)
    {
        if (padding)
        {
            nexacro.__setDOMStyle_PaddingObject(handle_style, padding);
        }
    };

    _pIconTextControlElement._updateContentsAll = function (handle_style)
    {
        const { iconPos, textAlign, verticalAlign, padding } = this;
        const flexdirection = this._getFlexDirection();

        this._updateContentsIconPos(handle_style, iconPos, flexdirection);
        this._updateContentsVerticalAlign(handle_style, verticalAlign);
        this._updateContentsTextAlign(handle_style, textAlign);
        this._updateContentsPadding(handle_style, padding);

        this._updateTextWordWrapAndOverflow();
    };

    _pIconTextControlElement._updateTextWordWrapAndOverflow = function ()
    {
        const text_node = this._text_node;
        if (text_node)
        {
            if (this._isWordWrapNone())
                nexacro.__setDOMDataset_TextOverflow(text_node, this.textOverflow || this._textoverflow_info);
    ...
    _pIconTextControlElement._adjustPaddingForNegativeClientSize = function (width, height)
    {
        const handle = this.handle;
        if (handle)
        {
            // end 값을 먼저 보정하고 start 값을 보정한다.
            const padding = (this.padding || this._padding_info);
            if (padding)
            {
                let inlineStart = padding.left || 0;
                let inlineEnd = padding.right || 0;
                let blockStart = padding.top || 0;
                let blockEnd = padding.bottom || 0;

                // width 음수이면 inline 패딩 조정
                if (width < 0)
                {
                    let need = -width;

                    // end 먼저 줄이기
                    let reduce = Math.min(inlineEnd, need);
                    inlineEnd -= reduce;
                    need -= reduce;

                    if (need > 0)
                    {
                        reduce = Math.min(inlineStart, need);
                        inlineStart -= reduce;
                        need -= reduce;
                    }
                    handle.style.paddingInlineStart = inlineStart + "px";
                    handle.style.paddingInlineEnd = inlineEnd + "px";
                }
                else
                {
                    handle.style.paddingInline = "";
                }

                // height 음수이면 block 패딩 조정
                if (height < 0)
                {
                    let need = -height;

                    // end 먼저 줄이기
                    let reduce = Math.min(blockEnd, need);
                    blockEnd -= reduce;
                    need -= reduce;

                    if (need > 0)
                    {
                        reduce = Math.min(blockStart, need);
                        blockStart -= reduce;
                        need -= reduce;
                    }
                    handle.style.paddingBlockStart = blockStart + "px";
                    handle.style.paddingBlockEnd = blockEnd + "px";
                }
                else
                {
                    handle.style.paddingBlock = "";
                }
            }
        }
    };

    _pIconTextControlElement._refreshControl = function (handle, handle_style)
    {
        nexacro.ControlElement.prototype._refreshControl.call(this, handle, handle_style);

        if (this.userSelect)
            nexacro.__setDOMNodeStyle_UserSelect(handle_style, this.userSelect);

        this._adjustPaddingForNegativeClientSize(this.client_width, this.client_height);
    }

    _pIconTextControlElement.destroy = function ()
    {
        this._destroyContents();

        nexacro.ControlElement.prototype.destroy.call(this);
    };

    _pIconTextControlElement._on_change_clientSize = function (width, height)
    {
        this._adjustPaddingForNegativeClientSize(width, height);
    };

    _pIconTextControlElement.setElementPadding = function (padding)
    {
        if (this.padding != padding)
        {
            this.padding = padding;

            this._updateClientRect(); // client 영역을 고려한 padding 보정 처리 하기 위해 호출

            const handle = this.handle;
            if (handle)
                this._updateContentsPadding(handle.style, padding);
        }
    };

    _pIconTextControlElement.setElementUserSelect = function (userSelect)
    {
        if (userSelect == null)
            userSelect = "auto";

        const select = ["text", "none", "all", "auto"];
        if (select.indexOf(userSelect) < 0)
            return;

        if (this.userSelect != userSelect)
        {
            this.userSelect = userSelect;

            const handle = this.handle;
            if (handle)
                nexacro.__setDOMNodeStyle_UserSelect(handle.style, userSelect);
        }
    };

    _pIconTextControlElement.setElementTextAlign = function (textalign)
    {
        const align = ["left", "center", "right"];
        if (align.indexOf(textalign) < 0)
            textalign = "center";
        ...
        if (this.textAlign != textalign)
        {
            ...
            const handle = this.handle;
            if (handle)
                this._updateContentsTextAlign(handle.style, textalign);
        }
    };

    _pIconTextControlElement.setElementVerticalAlign = function (verticalalign)
    {
        const align = ["top", "middle", "bottom"];
        if (align.indexOf(verticalalign) < 0)
        ...
        if (this.verticalAlign != verticalalign)
        {
            ...
            const handle = this.handle;
            if (handle)
                this._updateContentsVerticalAlign(handle.style, verticalalign);
        }
    };

    _pIconTextControlElement.setElementTextPadding = function (textPadding)
    {
        if (this.textPadding != textPadding)
        {
            this.textPadding = textPadding;
    ...
    _pIconTextControlElement.setElementTextWidth = function (textwidth)
    {
        if (this.textwidth != textwidth)
        {
            this.textwidth = textwidth;
            ...
            if (text_node)
            {
                ...
                {
                    const _textwidth = parseInt(textwidth);
                    if (_textwidth > 0)
                        nexacro.__setDOMStyle_TextWidth(text_node.style, _textwidth);
                    else
                        nexacro.__clearDOMStyle_TextWidth(text_node.style);
    ...
    _pIconTextControlElement.setElementText = function (text)
    {
        if (this.text !== text || this._use_decoration == true)
        {
            this.text = text;
            ...
            if (this.handle)
            {
                ...
                if (text_node)
                {
                    nexacro.__setDOMNode_Text(text_node, text);
                }
            }
        }
    };

    _pIconTextControlElement.setElementDecorateText = function (text)
    {
        if (this.text != text || this._use_decoration == false)
        {
            this.text = text;
    ...
    _pIconTextControlElement.setElementWordWrap = function (wordwrap)
    {
        // this prop is css set --> use class selector  -> css inline property (map & css)
        if (this.wordWrap != wordwrap)
        {
    ...
    _pIconTextControlElement.setElementCSSMapWordWrapInfo = function (wordwrap_info)
    {
        if (this._wordwrap_info != wordwrap_info)
        {
            this._wordwrap_info = wordwrap_info;
            if (this.handle)
            {
                this._updateTextWordWrapAndOverflow();
            }
    ...
    _pIconTextControlElement.setElementTextOverflow = function (textoverflow)
    {
        // this prop is css set --> use class selector  -> css inline property (map & css)
        if (this.textOverflow != textoverflow)
        {
    ...
    _pIconTextControlElement.setElementCSSMapTextOverflowInfo = function (textoverflow_info)
    {
        if (this._textoverflow_info != textoverflow_info)
        {
            this._textoverflow_info = textoverflow_info;
    ...
    _pIconTextControlElement.setElementIcon = function (icon)
    {
        if (this.icon != icon)
        {
            this.icon = icon;
            ...
            if (this.handle)
            {
                ...
                const icon_node = this._icon_node;
                if (icon_node)
                {
                    const icon_url = icon ? (icon.value == "none" ? undefined : icon._sysurl) : undefined;
                    nexacro.__setDOMStyle_ContentUrl(icon_node.style, icon_url);
                }
            }
        }
    };

    _pIconTextControlElement.setElementIconWidth = function (iconwidth)
    {
        if (this.iconwidth != iconwidth)
        {
            ...
            if (this.handle)
            {
                ...
                if (icon_node)
                {
                    nexacro.__setDOMStyle_ContentWidth(icon_node.style, iconwidth);
                }
            }
        }
    };

    _pIconTextControlElement.setElementIconHeight = function (iconheight)
    {
        if (this.iconheight != iconheight)
        {
            ...
            if (this.handle)
            {
                ...
                if (icon_node)
                {
                    nexacro.__setDOMStyle_ContentHeight(icon_node.style, iconheight);
                }
            }
        }
    };

    _pIconTextControlElement.setElementIconPos = function (icon_pos)
    {
        ...
        if (this.iconPos != icon_pos)
        {
            ...
            if (icon_node)
            {
                const handle = this.handle;
                if (handle)
                    this._updateContentsIconPos(handle.style, icon_pos);
            }
        }
    };

    _pControlElement.setElementIconTransform = function (transform)
    {
        if (this.iconTransform != transform)
        {
            this.iconTransform = transform;
            const icon_node = this._icon_node;
            if (icon_node)
            {
                nexacro.__setDOMStyle_TransformObject(icon_node.style, transform);
            }
        }
    };

    _pControlElement.setElementIconTransformOrigin = function (transformorigin)
    {
        if (this.iconTransformOrigin != transformorigin)
        {
            this.iconTransformOrigin = transformorigin;
            const icon_node = this._icon_node;
            if (icon_node)
            {
                nexacro.__setDOMStyle_TransformOriginObject(icon_node.style, transformorigin);
            }
        }
    };

    _pControlElement.setElementIconAnimation = function (animation)
    {
        if (this.iconAnimation != animation)
        {
            this.iconAnimation = animation;
            const icon_node = this._icon_node;
            if (icon_node)
            {
                nexacro.__setDOMStyle_AnimationObject(icon_node.style, animation);
            }
        }
    };

    _pControlElement.setElementIconAnimationPlayState = function (animationplaystate)
    {
        if (this.iconAnimationPlayState != animationplaystate)
        {
            this.iconAnimationPlayState = animationplaystate;
            const icon_node = this._icon_node;
            if (icon_node)
            {
                nexacro.__setDOMStyle_AnimationPlayStateObject(icon_node.style, animationplaystate);
            }
        }
    };

    _pControlElement.setElementIconTransition = function (transition)
    {
        if (this.iconTransition != transition)
        {
            this.iconTransition = transition;
            const icon_node = this._icon_node;
            if (icon_node)
            {
                nexacro.__setDOMStyle_TransitionObject(icon_node.style, transition);
            }
        }
    };

    _pControlElement.setElementTextTransform = function (transform)
    {
        if (this.textTransform != transform)
        {
            this.textTransform = transform;
            const text_node = this._text_node;
            if (text_node)
            {
                nexacro.__setDOMStyle_TransformObject(text_node.style, transform);
            }
        }
    };

    _pControlElement.setElementTextTransformOrigin = function (transformorigin)
    {
        if (this.textTransformOrigin != transformorigin)
        {
            this.textTransformOrigin = transformorigin;
            const text_node = this._text_node;
            if (text_node)
            {
                nexacro.__setDOMStyle_TransformOriginObject(text_node.style, transformorigin);
            }
        }
    };

    _pControlElement.setElementTextAnimation = function (animation)
    {
        if (this.textAnimation != animation)
        {
            this.textAnimation = animation;
            const text_node = this._text_node;
            if (text_node)
            {
                nexacro.__setDOMStyle_AnimationObject(text_node.style, animation);
            }
        }
    };

    _pControlElement.setElementTextAnimationPlayState = function (animationplaystate)
    {
        if (this.textAnimationPlayState != animationplaystate)
        {
            this.textAnimationPlayState = animationplaystate;
            const text_node = this._text_node;
            if (text_node)
            {
                nexacro.__setDOMStyle_AnimationPlayStateObject(text_node.style, animationplaystate);
            }
        }
    };

    _pControlElement.setElementTextTransition = function (transition)
    {
        if (this.textTransition != transition)
        {
            this.textTransition = transition;
            const text_node = this._text_node;
            if (text_node)
            {
                nexacro.__setDOMStyle_TransitionObject(text_node.style, transition);
            }
        }
    };
    ...
    _pHTMLControlElement.customData = undefined;

    const loadScripts = async (scripts, mountNode, comp) =>
    {
        let errortype = "";
        let errormessage = "";
        const loadScript = (script, targetNode, id) => 
        {
            return new Promise((resolve, reject) => 
            {
                ...
                if (script.src)
                {
                    ...
                    targetNode.removeChild(newScript);
                }
                else
                {
                    try 
        ...
        for (const script of scripts) 
        {
            ...
            try 
            {
                await loadScript(script, mountNode, comp.id);
            }
            catch (e)
            {
                if (comp)
                    ret = comp.on_fire_onerror(comp, "onerror", errormessage, errortype); //obj, id, errormsg, errortype
...
/*
    _pHTMLControlElement.createCommand = function ()
    {
        if (!this.customData)
            this.customData = {};
        if (attrvalue)
            this.customData[attrname] = attrvalue;
        else
            delete this.customData[attrname];

        if (this.handle)
            nexacro.__setDOMDataset_Custom(this.handle, attrname, attrvalue);        
    };
*/
    _pHTMLControlElement._getHTMLCustomDataStr = function ()
    {
        if (this.customData)
            return nexacro.__getHTMLDataset_Custom(this.customData);
        return "";
    };

    _pHTMLControlElement.setElementCustomData = function (attrname, attrvalue)
    {
        if (!this.customData)
            this.customData = {};
        if (attrvalue)
    ...
    _pHTMLControlElement.setElementHTML = async function (html)
    {
        ...
        if (doc)
        {
            ...
            const comp = this.linkedcontrol;
            if (scripts)
            {
                await loadScripts(scripts, node, comp);
            }
            return true;
        }
        return false;
    ...
    _pHTMLControlElement._getElementById = function (id)
    {
    ...
    _pHTMLControlElement.setElementScrollIntoView = function (node, option)
    {
        nexacro.__setDOMNode_ScrollintoView(node, option);
    };

    _pHTMLControlElement._createContentNode = function (tagname, attributes, innerHTML)
    {
        var attrs = "";
        if (attributes)
        {
            ...
            {
                if (Object.prototype.hasOwnProperty.call(attributes,attr) && attributes[attr] !== null && attributes[attr] !== undefined)
                {
