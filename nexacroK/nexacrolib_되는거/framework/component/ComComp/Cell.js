...
if (!nexacro.CellInfo)
{
    ...
    _pCellInfo._property_map_switch = [
        ...
        ["switchbuttonsize", "switchbuttonsize", true, null, false],
    ...
    _pCellInfo._property_map_multicombo = [
        ...
        ["multicomboselectallcheckboxtext", "selectallcheckboxtext", true, "Select All", false],
        ["multicombodisplayselectalltext", "displayselectalltext", true, "", false]
    ];
    ...
    */
    ...
    _pCellInfo._getDisplayText_text = function (rowidx)
    {
        ...
        if (this._view)
            v = this._view._getEnvironment()?._getTextTemplateValue(v);
...
if (!nexacro._CellControl)
{
    ...
    _pCellControl.on_attach_contents_handle = function (win, enablea11y)
    {
        var subcells = this.subcells;
        var subcells_len = subcells.length;

        for (var i = 0; i < subcells_len; i++)
            subcells[i].attachHandle(win, enablea11y);

        var sub_ctrl = this._subComp;
        if (sub_ctrl)
            sub_ctrl.attachHandle(win, enablea11y);

        var sort_ctrl = this._sortCtrl;
        if (sort_ctrl)
            sort_ctrl.attachHandle(win, enablea11y);

        var filter_ctrl = this._filterCtrl;
        if (filter_ctrl)
            filter_ctrl.attachHandle(win, enablea11y);

        var expand_ctrl = this._expandCtrl;
        if (expand_ctrl)
            expand_ctrl.attachHandle(win, enablea11y);

        var text_elem = this._text_elem;
        if (text_elem)
            text_elem.attachHandle(win, enablea11y);
    ...
    {
        if (self_flag || this._getEnvironment()?._p_enableaccessibility)
    ...
    */
    ...
    {
        const enableaccessibility = this._getEnvironment()?._p_enableaccessibility;
    ...
    _pCellControl._destroyEditor = function ()
    {
        ...
        if (this._editor)
        {
            ...
            //  editor도 접근성에선 삭제
            if (this._getEnvironment()?._p_enableaccessibility && this._tempdestroyeditor)
            {
                this._tempdestroyeditor_list.push(this._tempdestroyeditor);
            }
    ...
    _pCellControl._createEditor = function (onlyone_control)
    {
        ...
        this._static_create = false; // 조건 아닌경우 초기화 하여 새로 만들도록 수정
...
*/
    ...
    {
        if (!this._displaymode || this._getEnvironment()?._p_enableaccessibility)
    ...
    _pCellCheckboxBase._on_getFitSize = function (w, h)
    {
        ...
        if (control_elem)
        {
            ...
            if (icon)
            {
                ...
                {
                    icon_size = { width: icon?.width, height: icon?.height };
                    if (icon_size.width == null || icon_size.height == null)
                    {
                        if (icon.url)
                        {
                            var iconurl = nexacro._getURIValue(icon.url);
                            const env = this._getEnvironment();
                            iconurl = env?._getImageLocation(iconurl, this._view ? this._view._getRefFormBaseUrl() : undefined)
                            icon_size = nexacro._getImageSize_extended(iconurl, this._on_icon_onload, this) || { width: 0, height: 0 };
                        }
                        else
                        {
    ...
    */
    ...
    _pCellCustom.on_attach_contents_handle = function (win, enablea11y)
    {
        var control = this._control;
        if (control)
        {
            control.attachHandle(win, enablea11y);
        }
    };
    ...
    _pCellTree._load_image = function (img_ctrl, image)
    {
        ...
        if (this._control_element && val)
        {
            if (val != "none")
            {
                ...
                if (!env) 
                    return;
                val = env._getImageLocation(val, this._getRefFormBaseUrl());                
                img_ctrl._load_url = val;

                var size = nexacro._getImageSize_extended(val, this._on_loadImage, this);
