...
if (!nexacro._isHybrid() && !nexacro._macOSWebView)
    {
        ...
        _pXPushController.setXPushLayoutURL = function (layouturl)
        {
            ...
            if (form)
            {
                var env = form._p_parent._getEnvironment();
                if (url.indexOf("%") < 0)
                    layouturl = env._getServiceLocation(url, form._getRefFormBaseUrl());
            }
    ...
    _pXPushController.connectXPush = function (userid, sessionid, projectid)
    {
        ...
        if (form)
        {
            ...
            if (layouturl.indexOf("%") < 0)
                layouturl = env._getServiceLocation(layouturl, form._getRefFormBaseUrl());
        }
...
if (!nexacro.XPush)
{
    ...
    _pXPush._on_xpush = function (recv)
    {
        ...
        for (var i = 0; i < this._commandlist.length; i++)
        {
            ...
            if (cc.messagetype == recv.type)
            {
                ...
                if ((!cc.useactiveformcallback) || (cc.useactiveformcallback && (cc.objform === this.parent._getRootObject().getActiveForm()))) // chk
                {
                    var callbackFn;
    ...
    {
        if (!nexacro.Device._isHybrid() && !nexacro._macOSWebView)
        {
            return true;
        }
