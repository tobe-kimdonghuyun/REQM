...
if (nexacro.Component)
{
    ...
    */
        ...
        if (this._p_left != null || (this._p_left == null && this._p_right == null)) 
        {
            ...
            if (typeof this._p_left == "string")
            {
                ...
                else if (this._isEndWithWord(this._p_left, "em"))
                {
                    ...
                    {
                        refer_font = this._getRootObject()._getReferenceAbsoluteFont();
                        unit = "rem";
                    }
                    else
                    {
                    ...
                    parent_size = this._getFontRefSize(refer_font);
                }
                else
                {
                    parent_size = 1;
    ...
    _pComponent.updateUniqueID = function () 
    {
        if (!(this._p_parent instanceof nexacro.DesignForm)) 
        {
            const element = document.getElementById(this._unique_id);

            const id = this.id ? this.id : this._p_name;
            this._unique_id = this._p_parent && this._p_parent._unique_id ? (this._p_parent._unique_id + '.' + id) : (id ? id : '');

            if (element) 
            {
                element.id = this._unique_id;
            }
        }
    };
