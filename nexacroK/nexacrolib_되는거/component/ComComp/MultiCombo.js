...
if (!nexacro.MultiCombo)
{
    ...
    _pMultiCombo._itemheight = undefined;
    ...
    _pMultiCombo._tagboxitemhorizontalgap = undefined;
    _pMultiCombo._p_tagboxitemverticalgap = undefined;
    _pMultiCombo._tagboxitemverticalgap = undefined;
    ...
    _pMultiCombo._selectallcheckboxheight = undefined;
    ...
    _pMultiCombo._p_displayselectalltext = "";
    ...
    _pMultiCombo.on_attach_contents_handle = function (win, enablea11y)
    {
        if (this._p_dropbutton)
        {
            this._p_dropbutton.attachHandle(win, enablea11y);
            this._p_dropbutton._setAccessibilityStatHidden(true);
        }

        if (this._p_multicombotagbox)
        {
            this._p_multicombotagbox.attachHandle(win, enablea11y);
            this._p_multicombotagbox._setAccessibilityStatHidden(true);
        }

        if (this._p_multicombotext)
        {
            this._p_multicombotext.attachHandle(win, enablea11y);
            this._p_multicombotext._setReadonlyView(true);
            this._p_multicombotext._setAccessibilityStatHidden(true);
        }
    ...
    _pMultiCombo.on_change_bindSource = function (propid, ds, row, col)
    {
        if (propid == "value")
        {
            ...
            if(env)
            {
                valueseparator = env._p_multivalueseparator || ",";
            }
            let textseparator = this._p_textseparator;
            let values = val ? val.split(valueseparator) : val;
            let index, text, value, i;

            if (ds)
            {
                let info = this._select_multi;
                let len = info.length;
                let range = [];
    ...
    _pMultiCombo._getDlgCode = function (keycode, altKey, ctrlKey, shiftKey)
    {
        if (keycode == nexacro.Event.KEY_UP || keycode == nexacro.Event.KEY_DOWN) 
        {
            ...
            const env = this._getEnvironment();
            if ((ctrlKey || altKey) || (env && env._p_enableaccessibility && !env._p_accessibilitycontentsearchkey)) 
            {
    ...
    _pMultiCombo.on_update_position = function (resize_flag, move_flag, update)
    {
        ...
        if (control_elem)
        {
            ...
            let abstract_comp = this._getAbstractParent();
            if (abstract_comp)
    ...
    _pMultiCombo.set_text = function (v)
    {
        ...
        let ds = this._innerdataset;
        let datacolumn = this._p_datacolumn == "" ? this._p_codecolumn : this._p_datacolumn;
        if (ds)
    ...
    _pMultiCombo.set_value = function (v)
    {
        ...
        else
        {
            ...
            {
                let ds = this._innerdataset;
                let codecolumn = this._p_codecolumn || this._p_datacolumn;
                if (ds)
                {
    ...
    {
        var v = this._p_itemheight;
        if (nexacro._isEmRemUnit(v))
            this._itemheight = this._resolveEmPx(v);
        else
            this._itemheight = (v !== undefined) ? +v : undefined;
        if (this._p_multicombolist)
        {
            var checkboxset = this._p_multicombolist.checkboxset;
            if (checkboxset)
            {
                checkboxset.set_itemheight(this._itemheight);
            }
        }
    };
    ...
    */
    ...
    _pMultiCombo.set_selectallcheckboxheight = function (v)
    {
        if (v !== undefined)
        {
            if (nexacro._isEmRemUnit(v))
            {
                this._selectallcheckboxheight = this._resolveEmPx(v);
            }
            else
            {
                if (isNaN(v = +v)) return;
                this._selectallcheckboxheight = undefined;
            }
        }
    ...
    _pMultiCombo._getSelectAllCheckboxHeight = function ()
    {
        return (this._selectallcheckboxheight !== undefined) ? this._selectallcheckboxheight : this._p_selectallcheckboxheight;
    };
    ...
    _pMultiCombo._getTagboxItemHGap = function ()
    {
        return (this._tagboxitemhorizontalgap !== undefined) ? this._tagboxitemhorizontalgap : this._p_tagboxitemhorizontalgap;
    };

    _pMultiCombo.on_apply_tagboxitemhorizontalgap = function (/*v*/)
    {
        var pv = this._p_tagboxitemhorizontalgap;
        if (nexacro._isEmRemUnit(pv))
            this._tagboxitemhorizontalgap = this._resolveEmPx(pv);
        else
            this._tagboxitemhorizontalgap = undefined;
        if (this._tagmode)
        {
            var multicombotagbox = this._p_multicombotagbox;
            if (multicombotagbox)
    ...
    _pMultiCombo._getTagboxItemVGap = function ()
    {
        return (this._tagboxitemverticalgap !== undefined) ? this._tagboxitemverticalgap : this._p_tagboxitemverticalgap;
    };

    _pMultiCombo.on_apply_tagboxitemverticalgap = function (/*v*/)
    {
        var pv = this._p_tagboxitemverticalgap;
        if (nexacro._isEmRemUnit(pv))
            this._tagboxitemverticalgap = this._resolveEmPx(pv);
        else
            this._tagboxitemverticalgap = undefined;
        if (this._tagmode)
        {
            var multicombotagbox = this._p_multicombotagbox;
            if (multicombotagbox)
    ...
    _pMultiCombo.on_apply_font = function (font)
    {
        ...
        this._reapplyRemValues();
    };

    _pMultiCombo._reapplyRemValues = function ()
    {
        if (!this._isEnableRedraw()) return;
        if (nexacro._isEmRemUnit(this._p_itemheight))
            this.on_apply_itemheight(this._p_itemheight);
        var v2 = this._p_selectallcheckboxheight;
        if (v2 !== undefined && nexacro._isEmRemUnit(v2))
            this._selectallcheckboxheight = this._resolveEmPx(v2);
        if (nexacro._isEmRemUnit(this._p_tagboxitemhorizontalgap))
            this.on_apply_tagboxitemhorizontalgap();
        if (nexacro._isEmRemUnit(this._p_tagboxitemverticalgap))
            this.on_apply_tagboxitemverticalgap();
    };
    ...
    _pMultiCombo.set_displayselectalltext = function (v)
    {
        v = nexacro._toString(v);
        if (this._p_displayselectalltext != v)
        {
            this._p_displayselectalltext = v;
            this.redraw();
        }
    };
    ...
    _pMultiCombo._on_edit_onkeydown = function (obj, e)
    {
        ...
        var text = "";
        var rowcnt = ds.getRowCount();
      //  var enableaccessibility = this._getEnvironment()?._p_enableaccessibility;
        var filter_last;
        var sel = this._select_multi;
        var i;
        ...
        if (ds)
        {
            ...
            switch (type)
            {
                ...
                case "caseifilterlike":
                    ...
                    trans_val = nexacro._escapeWhitespace(parse_val);
    ...
    _pMultiCombo._createFilteredDataset = function ()
    {
        ...
        if (!fds)
        {
            fds = this._filtereddataset = new nexacro.DatasetBase("filter_" + this.id, this);
        }
    ...
    _pMultiCombo._recheckValue = function ()
    {
        ...
        if (this._select_multi && this._select_multi.items.length > 0)
        {
            ...
            var i = this._select_multi.items.length;
            ...
            if (multicombolist)
            {
                ...
                if (checkboxset)
                {
                    ...
                    if (!this._isFiredOnInput)
                    {
                        if (this._tagmode)
                        {
                            let tagboxedit = this.multicombotagbox.tagboxedit;
                            if (tagboxedit)
                                tagboxedit._apply_setfocus("focus");
                        }
                        else
                        {
                            let multicomboedit = multicombolist.multicomboedit;
                            if (multicomboedit)
    ...
    _pMultiCombo._changeIndex = function (v, change_by_script, isNotFireEvent)
    {
        if (!isNotFireEvent)
        {
            let idx, val, txt, preidx, pretext, prevalue, postidx, posttext, postvalue, i;
            const env = this._getEnvironment();
            let valueseparator = ",";
            if(env)
            {
    ...
    _pMultiCombo._removeindex = function (v, change_by_script, isNotFireEvent)
    {
        if (!isNotFireEvent)
        {
            let idx, val, txt, preidx, pretext, prevalue, postidx, posttext, postvalue, i, rows;
            const env = this._getEnvironment(); 
            let valueseparator = ",";
            if(env)
            {
    ...
    _pMultiCombo._setMultiComboTextValue = function ()
    {
        ...
        if (multicombotext)
        {
            ...
            if (this._p_displayselectalltext && ds && selectedItems.length > 0 && selectedItems.length == ds.getRowCount())
            {
                this._setTextValue(this._p_displayselectalltext);
            }
            else if (this._p_text)
            {
    ...
    _pMultiComboEditControl._apply_setfocus = function (evt_name, self_flag)
    {
        ...
        if (input_elem)
        {
            if (!this._onlydisplay)
            {
                ...
                //if (this._getEnvironment()?._p_enableaccessibility && evt_name != "tabkey" && evt_name != "shifttabkey" && !this._isComponentKeydownAction())
                ...
                let text = input_elem.getElementText();
                ...
                if (!this._p_autoselect)
                {
                    if (this._caret_pos == -1)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                if (evt_name == "focus" || evt_name == "lbuttondown")
    ...
    _pMultiComboEditControl.on_keydown_basic_action = function (keycode, alt_key, ctrl_key, shift_key, refer_comp, meta_key)
    {
        ...
        let input_elem = this._input_element;
        if (input_elem)
        {
            ...
            if (this._undostack)
            {
                let pos = input_elem.getElementCaretPos();
                if (pos && pos != -1)
                {
    ...
    _pMultiComboCheckBoxSetControl._use_native_scroll = true;
    ...
    _pMultiComboCheckBoxSetControl.on_attach_contents_handle = function (win, enablea11y)
    {
        var items = this._getContentsItem();
        var item_len = items.length;
        for (var i = 0; i < item_len; i++)
        {
            items[i].attachHandle(win, enablea11y);
        }
    ...
    _pMultiComboListItemControl._redrawListBoxContents = function (bScrollChange)
    {
        ...
        if (rowcount)
        {
            ...
            if (this._getEnvironment()?._p_enableaccessibility && (nexacro._isMobile && nexacro._isMobile()))
            {
    ...
    _pMultiComboListItemControl._on_getFitSize = function ()
    {
        ...
        if (control_elem)
        {
            ...
            var _tw = (this._textwidth !== undefined) ? this._textwidth : this._p_textwidth;
            ...
            if (checkboxset)
            {
                ...
                if (selectallcheckbox)
                {
                    ...
                    if (this._p_parent._p_selectallcheckboxheight != undefined)
                        selectall_height = this._p_parent._getSelectAllCheckboxHeight();
                    else
                ...
                var win_left = this._isAllowDefaultPinchZoom() ? nexacro._getWindowOffsetPosition(win).left : rootframe_elem_pos.x;
        ...
        if (multicombo && multicombolist)
        {
            ...
            if (checkboxset)
            {
                ...
                if (win)
                {
                    ...
                    if (nexacro.isglobalmodule && nexacro._OS == "iOS" && nexacro._Browser == "MobileSafari" && iswindow)
                    {
    ...
    _pMultiComboTagBoxControl.on_attach_contents_handle = function (win, enablea11y)
    {
        if (this._multicombotags)
        {
            var selectedItems = this._p_parent._select_multi;
            ...
            for (var i = 0; i < selectedItems.length; i++)
            {
                this._multicombotags[selectedItems.items[i]].attachHandle(win, enablea11y);
        ...
        if (this.tagboxedit)
        {
            this.tagboxedit.attachHandle(win, enablea11y);
        }
    };
    ...
    _pMultiComboTagBoxControl._recalcLayout = function ()
    {
        if (this._is_created_contents)
        {
            ...
            if (multicombotags)
            {
                ...
                var gap_h = multicombo._p_tagboxitemhorizontalgap != undefined ? multicombo._getTagboxItemHGap() : 5;
                var gap_v = multicombo._p_tagboxitemverticalgap != undefined ? multicombo._getTagboxItemVGap() : 5;
    ...
    _pTagBoxControl.on_attach_contents_handle = function (win, enablea11y)
    {
        if (this.tagtext)
        {
            this.tagtext.attachHandle(win, enablea11y);
            this.tagtext._setAccessibilityStatHidden(true);
        }

        if (this.tagbutton)
        {
            this.tagbutton.attachHandle(win, enablea11y);
    ...
    _pMultiComboListControl._createSelectAllCheckBoxControl = function (ds)
    {
        ...
        if (!selectallcheckbox)
        {
            ...
            // selectallcheckbox._setEventHandler("onitemclick", this._p_parent._on_list_onitemclick, this._p_parent)
        }
    };
