...
if (!nexacro._GraphicsLib)
{
    ...
    nexacro._GraphicsLib.objCopyProperties = function (tarobject, srcobject)
    {
        if (tarobject && srcobject)
        {
            ...
            for (p in srcobject)
            {
                if (Object.prototype.hasOwnProperty.call(srcobject,p))
                {
                    value = srcobject[p];
                    tarobject[p] = value;
                }
...
if (!nexacro._GraphicsHashMap)
{
    ...
    _pGraphicsHashMap._debug = function (useConsole) 
    {
...
if (!nexacro._GraphicsRectsManager)
{
    ...
    _pGraphicsRectsManager._debug = function () 
    {
        ...
        console.log("nexacro._GraphicsRectsManager:", "size:", rects.length);
...
if (!nexacro.GraphicsImage)
{
        ...
        if (this._p_image != v)
        {
            ...
            if (v)
            {
                ...
                else
                {
                    ...
                    var imageurl = nexacro._getURIValue(rurl);
                    imageurl = this._getEnvironment()?._getImageLocation(imageurl, this._getRefFormBaseUrl());
                    var image_size = nexacro._getImageSize_extended(imageurl, nexacro._emptyFn, this);
                    if (image_size)
                    {
                        w = image_size.width;
                        h = image_size.height;
    ...
    _pGraphicsImage._getRealImageUrl = function (url)
    {
        ...
        return this._getEnvironment()?._getImageLocation(url, baseurl);
    };
...
if (!nexacro.Graphics)
{
    ...
    _pGraphicsControl.on_created_contents = function (win)
    {
        ...
        if (layer)
        {
            ...
            if (nexacro._AccessibilityStyles.length == 0 && this._getEnvironment()?._p_enableaccessibility)
            {
    ...
    _pGraphicsControl.on_attach_contents_handle = function (win/*, enablea11y*/)
    {
        var layer = this._layer;
        if (layer)
        {
