    ...
    */
    ...
    _pPopupMenu.on_fire_sys_onkeydown = function (keycode, alt_key, ctrl_key, shift_key, fire_comp, refer_comp)
    {
        ...
        switch (keycode)
        {
                    ...
                    if (pThis._popupitemindex > -1)
                    {
                        if (!popupvisible)
                        {
                            ...
                            if (parent instanceof nexacro.PopupMenu)
                            {
                                ...
                                rootComp._select_menuitem(rootComp._getItemIndex(-1), rootComp._popupitemindex, true);//, 0, true);
                        }
                        else
                        {
                            this._fire_on_Popupmenu(keycode, alt_key, ctrl_key, shift_key, fire_comp, refer_comp);
                    ...
                    if (pThis._popupitemindex > -1)
                    {
                        ...
                        else
                        {
                            ...
                            if (item)
                            {
                                ...
                                //     item._changeStatus("selected",false)
                            }
                            //this.closePopup();
            ...
            case E.KEY_ESC:
                ...
                {
                    //                    this._closePopup();
                    if (refer_comp == fire_comp || refer_comp.parent == fire_comp) //&& fire_comp instanceof nexacro.PopupMenu)
                    {
                        this._closePopup();
                    }
                    else
    ...
    _pPopupMenu.on_notify_menuitem_onmouseenter = function (obj/*, e*/)
    {
        ...
        if (!nexacro._enableaccessibility)
        {
            ...
            /*
            if (item[this._popupitemindex])
                item[this._popupitemindex]._on_apply_mouseover(true);
           */
            ...
            if (pitem && pitem[this.parent._previtemindex])
            {
               // pitem[this.parent._previtemindex]._changeUserStatus("selected", true);
                //pitem[this.parent._previtemindex]._on_apply_mouseover(true);
            }
            */
    ...
    _pPopupMenu._calcNavigationbutton = function (/*obj*/)
    {
    ...
    _pPopupMenu.trackPopupByComponent = function (obj, x, y, align, bcapture)
    {
        ...
        //var p = nexacro._getPopupElementPositionInFrame(obj.getElement(), x, y);
        var win_left = p.x;
        var win_top = p.y;
        ...
        x = +x + win_left;// - form._getScrollLeft();
        y = +y + win_top;// - form._getScrollTop();
            ...
            if (ds && this.levelcolumn && this.captioncolumn && this.idcolumn)
            {
                ...
                for (var rowlength = datarow; rowlength < len; rowlength++)
                {
                    ...
                    if (level == this.level)
                    {
                        ...
                        if (icon && !nexacro._toBoolean(checkimg)) // && this._iconcontrol)
    ...
    _pPopupMenu._adjustPopupPosition = function (x, y, align, win_position)
    {
        ...
        /* 
         if (win_position)
         {
           //  popup_left = popup_left * (2 - wheelZoom);//+ win_position.x;
    ...
    _pPopupMenu._reCalcSize = function (/*w,h*/)
    {
        ...
        if (ds && this.captioncolumn)
        {
            ...
            if (bchkimg)
                chkimgwidth = item_h;
            var gap = 0, icontextpadding = 0;//this._hotkeytextgap;
            if (!this._is_subcontrol)
            {
