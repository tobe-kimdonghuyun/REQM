...
if (!nexacro.ChartGaugeSeriesControl)
{
    ...
    _pChartGaugeSeriesControl._drawCircle = function ()
    {
        ...
        function drawCircleGauge(pThis, j)
        {
            if (items[j]._isShow)
            {
                var angle = items[j].angle;
                var drawvalue = items[j].value;
                ...
                items[j].startAngle = startAngle;
                items[j].endAngle = endAngle;
                items[j].angle = angle;
                items[j].rStartAngle = rStartAngle;
                items[j].rEndAngle = rEndAngle;
                ...
                slice._seriesItem = items[j];
                slice._series = pThis;

                items[j]._slice = slice;
                ...
                if (isselectitem)
                {
                    ...
                    {
                        selectfillstyle = items[j].color;
                    }
                    slice.set_strokepen(selectlinestyle);
                    if (pThis._chart && pThis._chart._usepattern)
                    {
                        var _idx = pThis._chart.getPatternIndex(j);
                        var _astyle = pThis._chart.getPattern(_idx);
                        slice.set_fillstyle(_astyle.selectfillstyle);
                    }
                    else
                    {
    ...
    _pChartGaugeSeriesControl._drawBarItemText = function (left, right, bottom, top, width, height, item)
    {
        ...
        function positionstart(targetItemText, l, r, b, t, w, h, itemtextGap, rotateaxis)
        {
            var textTop = 0;
            var textLeft = 0;
            ...
            else
            {
                ...
                textLeft = l + (w / 2);
                textTop = b - itemtextGap;
            }

            targetItemText.set_x(textLeft);
            targetItemText.set_y(textTop);
        }
        function positionmiddle(targetItemText, l, r, b, t, w, h, itemtextGap, rotateaxis)
        {
            targetItemText.set_verticalAlign("middle");
            targetItemText.set_textAlign("center");
            var textTop = 0;
            ...
            {
                textTop = t + (h / 2);
                textLeft = l + (w / 2) + itemtextGap;
            }
            else
            {
                textLeft = l + (w / 2);
                textTop = t + (h / 2) - itemtextGap;
            }

            targetItemText.set_x(textLeft);
            targetItemText.set_y(textTop);
        }
        function positionend(targetItemText, l, r, b, t, w, h, itemtextGap, rotateaxis)
        {
            var textTop = 0;
            var textLeft = 0;
    ...
    _pChartGaugeSeriesControl._drawLabels = function ()
    {
        ...
        function drawLabel(item, startangle, index, targetThis)
        {
            ...
            var halfAngle = 0,
                ...
                chart = targetThis._chart,
                itemtextposition = targetThis.itemtextposition,
                boardborderWidth = targetThis._chart._boardBorderWidth ? targetThis._chart._boardBorderWidth : 1.5,
                boardborderHeight = targetThis._chart._boardBorderHeight ? targetThis._chart._boardBorderHeight : 1.5,
                maxRadius = Math.min((targetThis._chart._boardWidth - (boardborderWidth * 2)), (targetThis._chart._boardHeight - (boardborderHeight))) / 2,
                ...
                radius,
                itemtextgap_h = targetThis._itemtextgap_h,
                itemtextgap_v = targetThis._itemtextgap_v,
            ...
            if (nexacro._GraphicsLib.isEmpty(targetThis._chart._gaugeradius) || nexacro._GraphicsLib.isEmpty(targetThis._chart._gaugeinnerradius)) // radius가 undefined임.
            {
                radius = maxRadius * 0.8;
            }
            else
            {
                radius = targetThis._chart._gaugeradius - (targetThis._chart._gaugeradius - targetThis._chart._gaugeinnerradius) / 2;
            }
