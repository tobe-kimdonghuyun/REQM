...
if (!nexacro.ChartPieSeriesControl)
{
    ...
    _pChartPieSeriesControl._drawPie = function ()
    {
        ...
        {
            var _idx = this._chart.getPatternIndex(this._configIndex);
            var _astyle = this._chart.getPattern(_idx);
            linestyle = _astyle.normallinestyle;
            selectlinestyle = _astyle.selectlinestyle;
        }
        ...
        function drawSlice(targetThis, targetIdx)
        {
            if (items[targetIdx]._isShow)
            {
                var angle = items[targetIdx].angle;
                var curAngle = items[targetIdx].angle * 180 / Math.PI;
                var valuelineblurangle = targetThis._valuelineblurangle || 0;
                ...
                {
                    items[targetIdx].isskip = true;
                    items[targetIdx].nextskip = true;

                    var _valuelineblurangle = targetThis._valuelineblurangle || 0;
                    ...
                    if (valuelineblurangle <= curAngle)
                    {
                        if (targetIdx == 0)
                        {
                            nextAngle = items[targetIdx + 1].angle * 180 / Math.PI;
                            prevAngle = items[items.length - 1].angle * 180 / Math.PI;

                            if (items.length > 3)
                                next2Angle = items[targetIdx + 2].angle * 180 / Math.PI;
                            else
                                next2Angle = items[targetIdx + 1].angle * 180 / Math.PI;

                            if (valuelineblurangle <= nextAngle)
                            {
                                if (valuelineblurangle <= prevAngle)
                                {
                                    items[targetIdx].isskip = false;
                                    if (valuelineblurangle <= next2Angle)
                                    {
                                        items[targetIdx].nextskip = false;
                                    }
                                }
                            }
                        }
                        else
                        {
                            if (targetIdx == items.length - 1)
                            {
                                ...
                                next2Angle = items[targetIdx].angle * 180 / Math.PI;

                            }
                            else
                            {
                                nextAngle = items[targetIdx + 1].angle * 180 / Math.PI;
                                prevAngle = items[targetIdx - 1].angle * 180 / Math.PI;
                                next2Angle = targetIdx < items.length - 2 ? items[targetIdx + 2].angle * 180 / Math.PI : items[items.length % (targetIdx + 2)].angle * 180 / Math.PI;
                    ...
                    if (!items[targetIdx].isskip) // 앞뒤에 gradation 가능
                    {
                        if (!items[targetIdx].nextskip)
                        {
                            if (targetIdx == 0)
                            {
                            ...
                            rEndAngle = rStartAngle + (endangle * (items[targetIdx].value / total)) - _valuelineblurangle;
                        }
                        else
                        {
                            rEndAngle = rStartAngle + (endangle * (items[targetIdx].value / total));
                        }
                    }
                    else
                    {
                    ...
                    items[targetIdx].rStartAngle = rStartAngle;
                    items[targetIdx].rEndAngle = rEndAngle;

                    rHighlightEndAngle = rHighlightStartAngle + (endangle * (items[targetIdx].value / total));
                }
                ...
                var startAngle = Math.round(rStartAngle * 180 / Math.PI),
                    ...
                    index = 0,
                    dx = 0, dy = 0,
                    centerX = targetThis._chart._centerLeft,
                    centerY = targetThis._chart._centerTop,
                    selectindent,
                    startangle = targetThis._startangle || 0;
                var idx;
                var astyle;

                if (targetThis.selectindent < 0 || nexacro._GraphicsLib.isEmpty(targetThis.selectindent))
                    selectindent = 0;
                else
                    selectindent = targetThis.selectindent / 100;

                items[targetIdx].startAngle = startAngle;
                items[targetIdx].endAngle = endAngle;
                items[targetIdx].angle = angle;
                ...
                if (isselectitem)
                {
                    ...
                    items[targetIdx].selectindentX = centerX;
                    items[targetIdx].selectindentY = centerY;
                ...
                items[targetIdx].pathData = pathData;


                index = targetIdx + 1;
                itemId = targetThis._configIndex + " SeriesPieItem_" + index;
                ...
                slice._seriesItem = items[targetIdx];
                slice._series = targetThis;
                slice.setPathData(pathData);
                items[targetIdx]._slice = slice;


                if (targetThis.highlightvisible)
                {
                    var highlightradius = targetThis.highlightradius,
                        hRadius = 0,
                        targetMaxRadius = Math.min(targetThis._chart._boardWidth, targetThis._chart._boardHeight) / 2,
                        highlightPathData;
                    ...
                    else
                    {
                        ...
                        {
                            hRadius = targetThis._highlightradius * targetMaxRadius;
                        }
                    }
                ...
                if (isselectitem)
                {
                    ...
                    {
                        selectfillstyle = items[targetIdx].color;
                    }
                    slice.set_strokepen(selectlinestyle);

                    if (targetThis._chart && targetThis._chart._usepattern)
                    {
                        idx = targetThis._chart.getPatternIndex(targetIdx);
                        astyle = targetThis._chart.getPattern(idx);
                ...
                else
                {
                    ...
                    if (targetThis._chart && targetThis._chart._usepattern)
                    {
                        ...
                        slice.set_fillstyle(items[targetIdx].color);
                ...
                slice._series = targetThis;
                rStartAngle += endangle * (items[targetIdx].value / total);
                rHighlightStartAngle += endangle * (items[targetIdx].value / total);
            }
        }
        function drawSliceBorder(targetThis, targetIdx)
        {
            if (items[targetIdx]._isShow)
            {
                var angle = items[targetIdx].angle;
                var valuelineblurangle = targetThis._valuelineblurangle || 0;
                var rtargetStartAngle;
                var rtargetEndAngle;
                var rStartAngle1;       
                var isskip = false;
                ...
                {
                    rtargetStartAngle = 0;
                    rtargetEndAngle = this._endangle || 6.28319;
                    if (effect && effect.isloadanimation)
                    {
                        var _angles = targetThis._getanimationdrawvalue(rtargetEndAngle);
        
                        rtargetStartAngle = _angles[0];
                        rtargetEndAngle = _angles[1];
                    }
                }
                else
                {
                    isskip = items[targetIdx].isskip;
                    if (!isskip)
                    {
        
                        if (i == 0 || i < items.length - 1)
                        {
                            rStartAngle1 = items[targetIdx].rEndAngle;
        
        
                            rtargetStartAngle = items[targetIdx].rEndAngle - 0.08;
                            rtargetEndAngle = items[targetIdx + 1].rStartAngle + 0.08;
                        }
                        else if (targetIdx == items.length - 1)
                        {
                            ...
                            rtargetStartAngle = items[items.length - 1].rEndAngle - 0.08;
                            rtargetEndAngle = rtargetStartAngle + valuelineblurangle + 0.15;
                        }
                ...
                var startAngle = Math.round(rtargetStartAngle * 180 / Math.PI),
                    endAngle = Math.round(rtargetEndAngle * 180 / Math.PI),
                    w = radius,
                    h = radius,
                    p0, p1, p2, p3,
                    la,
                ...
                p0 = (Math.sin(rtargetStartAngle) * w + centerX) + " " + (-Math.cos(rtargetStartAngle) * h + centerY);
                p1 = (Math.sin(rtargetEndAngle) * w + centerX) + " " + (-Math.cos(rtargetEndAngle) * h + centerY);
                ...
                if (!nexacro._GraphicsLib.isEmpty(innerradius) && innerradius > 0)
                {
                    ...
                    p2 = (Math.sin(rtargetStartAngle) * innerw + centerX) + " " + (-Math.cos(rtargetStartAngle) * innerh + centerY);
                    p3 = (Math.sin(rtargetEndAngle) * innerw + centerX) + " " + (-Math.cos(rtargetEndAngle) * innerh + centerY);
                ...
                index = targetIdx;
                itemId = targetThis._configIndex + " SeriesPieItemBorder_" + index;
                slice = seriesGroup.getObjectByID(itemId);
                if (slice)
                {
                    slice.setPathData(pathData);
                ...
                slice._seriesItem = items[targetIdx];
                slice._series = targetThis;
                items[targetIdx]._sliceborder = slice;
                ...
                if (i == endindex)
                {
                    firstcolor = items[targetIdx].color;
                    secondcolor = items[0].color;
                }
                else
                {
                    firstcolor = items[targetIdx].color;
                    secondcolor = items[targetIdx + 1].color;
                }
...
//-------------------------------------------------------
    ...
    _pChartPieSeriesControl._drawLabels = function ()
    {
        ...
        function drawLabel(item, startangle, index, targetThis)
        {
            var halfAngle = 0,
                maxRadius = Math.min(targetThis._chart._boardWidth, targetThis._chart._boardHeight) / 2,
                centerX = targetThis._chart._centerLeft,
                centerY = targetThis._chart._centerTop,
                itemText,
                itemtextguidesize = targetThis._itemtextguidesize,
                textX = 0,
                textY = 0,
                itempointsize,
                pointx,
                pointy,
                itemtextguidelinestyle = targetThis.itemtextguidelinestyle,
                itemtextguidelineopacity = targetThis.itemtextguidelineopacity,

                guideline,
                seriesGroup = targetThis._chart._seriesGroup,
                itemtextguidelocation = targetThis._itemtextguidelocation,
                itemId,
                radius,
                targetX = 0,
                targetY = 0,
                borderwidth = targetThis._borderwidth;

            if (nexacro._GraphicsLib.isEmpty(targetThis.radius))
            {
                radius = maxRadius * 0.8;
            }
            else
            {
                radius = maxRadius * targetThis._radius;
            }
            ...
            // itemtext 생성
            itemText = targetThis._createSeriesItemText(item);
            // trace("itemText", itemText);
            if (!nexacro._isNull(itemText))
            {
                var targetItems = targetThis._seriesitems;
                ...
                if (itemtextguidelocation == "out")
                {
                    ...
                    itemId = targetThis._configIndex + " SeriesPieItemLine_" + index;
                ...
                var length = targetThis._itemtextlist.length || 0;
                seriesGroup.addChild(targetThis._itemtextlist[length - 1], itemText);

                itemText._series = targetThis;
                itemText._seriesItem = item;


                targetThis._chart._setChangeInBoardAreaPos(itemText);
