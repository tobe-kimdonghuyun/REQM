...
if (!nexacro.DesignForm)
{
    ...
    /* 이후 K로 전환되면 nexacro._Browser == "Runtime" 부분은 기본적으로 다 삭제*/
    ...
    nexacro.getEnvironment = function ()
    {
        if (nexacro.isglobalmodule)
            ...
            return nexacro._peekCurrentEnvironment() ?? window.__accessport.env;
    }

    nexacro.__onNexacroStudioError = function(msg)
    {
        // 임시 디버그 처리
        // 이후 nexacro engine 쪽으로 fire 되어야 하니 customEvent를 사용해서, 주입된 design-runtime-script와 연계
        console.log("studio error",msg);
    }
    nexacro.__notifyToDesignWindow = function(handle, _design_notify_div_urlload, id, extra_info)
    {
        // 임시 처리
        // url을 통해 link 될 경우 fire 될것임
        // 이후 nexacro engine 쪽으로 fire 되어야 하니 customEvent를 사용해서, 주입된 design-runtime-script와 연계
    }
    ...
    {
        /* application event를 막는다 */
        var _pApplication = nexacro.Application.prototype;
        _pApplication._executeScript = nexacro._emptyFn;
        _pApplication.registerScript = nexacro._emptyFn;
    ...
    if (nexacro.FormBase)
    {
        ...
        _pFormBase.loadForm = function (formurl, async, reload, baseurl)
        {
            if (this._load_manager)
            {
                ...
                if (this._p_parent && (this._p_parent._is_frame && this._p_parent._p_form == this))
                {
                    // ChildFrame > Form   

                    env._getApplication()?._registerLoadforms(this);
                }
    ...
    if (nexacro._LoadManager)
    {
        ...
        if (__pLoadManager)
        {
            var _origin_on_load_main = __pLoadManager.on_load_main;

            /* 되도록이면 재정의를 하지않고 원형을 사용해야 한다.*/
            __pLoadManager.on_load_main = function (url, errstatus, module, fireerrorcode, returncode, requesturi, locationuri, extramsg)
            {
                try
                {
                    const obj = _origin_on_load_main.call(this, url, errstatus, module, fireerrorcode, returncode, requesturi, locationuri, extramsg);

                    var win, frame, designform, extra_info, form;
                    if (obj instanceof nexacro._InnerForm)
                    {
                        ...
                        if (frame)
                            ...
                            form = obj._getRootObject();
                        if (form instanceof nexacro.DesignForm)
                        {
                            designform = form;
                ...
                {
                    traceD("[ Error ] __pLoadManager.on_load_main", e)
                }
    ...
    if (nexacro._Browser == "Runtime")
    {
        ...
        {
            var valueobject = (new Function('return ' + value))();
    ...
    nexacro._setInitValueID = function (obj, value)
    {
        if (obj)
        {
            ...
            const nexacro_init = obj._getEnvironment()?._getInitValues();
    ...
    if (nexacro._Browser == "Runtime")
    {
        ...
        nexacro._getComputedStyleProperties = function (control_elem, propid, ret_handle, convert_prop)
        {
            ...
            */
            ...
            {
                //eval("tmp = {" + ret + "};");
                const tmp = new Function(`return { ${ret} };`)();
    ...
    if (nexacro._Browser !== "Runtime")
    {
        nexacro.__findElement = function (handle, classname)
        {
            const childs = handle.children;
    ...
    nexacro.DesignForm = function (id, left, top, width, height, right, bottom, minwidth, maxwidth, minheight, maxheight, parent, is_main = false)
    {
        ...
        // 임시
        this._outer_background_value = 'transparent';//this._outer_background_value = "#262626";
        this._inner_background_value = "#ffffff";

        nexacro.Form.call(this, id, left, top, width, height, right, bottom, minwidth, maxwidth, minheight, maxheight, parent, is_main);
    ...
    _pDesignForm.get_step_count = function (rootobj)
    {
        ...
        if (rootobj instanceof nexacro.Form)
        {
            ...
            var manager = nexacro._getLayoutManager(env);
            if (manager)
            {
    ...
    if (nexacro._Browser == "Runtime")
    {
        ...
        _pDesignForm.createComponentCSSPreview = function (classname, controlclassname, issubcontrol, parentid, left, top, width, height, compid, props, values, new_create, show)
        {
            try 
            {
                ...
                {
                    classnameobj = nexacro.Div;
                }
                else if (compclassname == "nexacro.DatePickerControl")
                {
                    classnameobj = nexacro.Calendar;
                }
                else if (compclassname == "nexacro.Step")
                {
                    classnameobj = nexacro.StepControl;
                }
                else if (compclassname == "nexacro.TimePickerControl")
                {
                    classnameobj = nexacro.DateRangePicker;
                }
                else
                {
                    classnameobj = compclassname.split('.')
                        .filter(key => key && key !== "nexacro")
                        .reduce((obj, key) => obj?.[key], nexacro);
    ...
    _pDesignForm.createComponentByRect = function (classname, parentid, left, top, width, height, compid, new_create)
    {
        try
        {
            ...
            if (classnameobj)
            {
                ...
                obj = new classnameobj(compid, left, top, width, height, null, null, null, null, null, null, parentform);
    ...
    _pDesignForm.createFrame = function (classname, parentid, frameid)
    {
        ...
        var classnameobj = classname.split('.')
            .filter(key => key && key !== "nexacro")
            .reduce((obj, key) => obj?.[key], nexacro);
        if (classnameobj)
        {
    ...
    nexacro.createGlobalObject = function (classname, objid, parentid)
    {
        // Dataset
        // Variable
        // Object
    ...
    nexacro.deleteGlobalObject = function (objid)
    {
        // datasets = array
        // all = collection
        // images = collection
        const app = nexacro.getApplication();
        const obj = nexacro._getApplicationObject(objid);
        if (!obj)
        {
            return;
        }
        ...
        {
            const index = app._datasets.findIndex((item) => item === obj);
            if (index >= 0)
            {    
                app._datasets.splice(index, 1);
            }
        }
        else if (obj instanceof nexacro.DataObject)
        {
            const index = app._dataobjects.findIndex((item) => item === obj);
            if (index >= 0)
            {
                app._dataobjects.splice(index, 1);
            }
        }
    };
    ...
    _pDesignForm.setProperty = function (compid, propid, propval, pseudo)
    {
                    ...
                    if (nexacro._Browser !== "Runtime")
                    {
                        if (propid === 'name')
                        {
                            ...
                            if (obj.form)
                            {
                                // Formbase
                                const form = obj.form;
                                this.setProperty(compid + '.form', propid, form.name, pseudo);
                                const children = form.all;
                                if (children && children.length)
                                {
                                    for (let i = 0; i < children.length; i++)
                                    {
                                        this.setProperty(compid + '.form.' + children[i].id, propid, children[i].name, pseudo);
                                    }
                                }
                            }
                            ...
                            {
                                // Tab
                                // Tabpage는 Formbase가 처리
                                const tabpages = obj.tabpages;
                                if (tabpages && tabpages.length)
                                {
                                    for (let i = 0; i < tabpages.length; i++)
                                    {
                                        this.setProperty(compid + '.' + tabpages[i].id, propid, tabpages[i].name, pseudo);
                                    }
                                }
                            }
                        }
        ...
        if (nexacro._Browser !== "Runtime")
        {
            if(compid == "this")
            {
                nexacro._setProperty(this, propid, propval);
            }
    ...
    nexacro._getApplicationObject = function (name)
    {
        if (!name || name === 'this')
        {
        ...
        try
        {
            const obj = nexacro.getApplication()[name]
            return obj;
        }
        catch (error)
        {
    ...
    nexacro.setGlobalProperty = function(id,attrId, attrValue)
    {
        const obj = nexacro._getApplicationObject(id);
        if (obj && nexacro._setProperty(obj, attrId, attrValue))
        {
            attrId = (attrId === 'id' ? 'name' : attrId);
            if (attrId === 'name')
            {
                const parent_obj = obj.parent;
                const old_id = obj.name;
                const new_id = attrValue;
                ...
                // all
                let idx = parent_obj.all.indexOf(old_id);
                if (idx < 0)
                {
                    return;
                }
    ...
    nexacro.getGlobalProperty = function (id, attrId)
    {
        const obj = nexacro._getApplicationObject(id);
        return nexacro._getProperty(obj, attrId);
    }
    ...
    _pDesignForm.swapPositionUnit = function (compid, pos, unit)
    {
        ...
        if (obj)
        {
            ...
            else
            {
                ...
                // LAP 코드 반영 
                // 문제가 있다면 this._swapPositionUnit(obj, pos, unit); 로 바꿔서 다시 호출
                ...
                const parent = obj._is_group ? obj._group_panel : obj._p_parent;
                if (!parent)
                {
                    return;
                }
                ...
                let parent_size;
                const root_form = obj._getRootForm();
                const owner_positionstep = this.get_owner_step_index(obj);
                const step_width = 0;
                ...
                const table_cell_rect = this.getTableCellRect(this._getScopeName(obj), this._getScopeName(parent));
                ...
                const arrange_info = obj._arrange_info;
                let adjust_pos, refer_font;
                let baseCompid = null;
                const _left = obj._adjust_left - owner_positionstep * step_width;
                ...
                if (pos === 'left')
                {
                    ...
                    // 25.03.12 baseCompid에 정상적으로 기준 comp id가 들어가지 않는 버그 수정
                    // arrange_info.compid -> info.compid
                    if (arrange_info && (info = arrange_info.left))
                    {
                        target = obj._findComponentForArrange(info.compid);
                        if (target)
                        {
                            baseCompid = info.compid;
                ...
                if (info)
                {
                    ...
                    if (distance.indexOf('rem') > -1 || distance.indexOf('em') > -1)
                    {
                        ...
                        tmp_size = obj._getFontRefSize(refer_font);
                    }

                    adjust_pos = nexacro.round(obj._convToPixel(distance, tmp_size));
                }

                // 현재 property 설정값을 무시하고 unit에 따라서 값 변경
                let new_prop_val;
                if (unit === 'px')
                {
                    if (baseCompid)
                ...
                else
                {
                    ...
                    if (unit === 'rem' || unit === 'em')
                    {
                        ...
                        refer_font = tmp_comp._getReferenceAbsoluteFont();
                        parent_size = obj._getFontRefSize(refer_font);
                    }

                    // px to unit
                    // 25.03.12 변환 값에 baseCompid를 붙여주지 않는 버그 수정
                    // = -> +=
                    new_prop_val += obj._convToRelativeUnit(adjust_pos, parent_size, unit);

                    // 2025/11/10 이인영 : 소수점 단위 보정.
                    if (unit === '%')
                    {
                        // 소수점 둘째짜리까지 보정. 
                        new_prop_val = Number(new_prop_val).toFixed(2);
                    }
                    else if (new_prop_val.indexOf('.') >= 0)
                    {
                        // rem, em
                        new_prop_val = Number(new_prop_val).toFixed(2);
                    }
    ...
    _pDesignForm.refreshPosition = function (compid)
    {
        ...
        if (obj)
        {
            var _restorePositionBase = function (obj, pos)
            {
                ...
                if (obj && pos) 
                {
                    const propName = pos + "base";
                    const methodName = "set_" + propName;


                    const temp = obj[propName];
    ...
    if (nexacro._Browser == "Runtime")
    {
        ...
        {
            const env = obj._getEnvironment();
            var registerclass = env?._getRegisterClassList(); //nexacro._registerclass;
            ...
            const getConstructor = (path) => 
            {
                ...
                // 첫 번째 요소가 "nexacro"라면 제거하고 nexacro 객체부터 탐색 시작
                const startObj = (parts[0] === 'nexacro') ? (parts.shift(), nexacro) : nexacro;

                return parts.reduce((acc, curr) => acc && acc[curr], startObj);
            ...
            for (var i = len - 1; i >= 0; i--)
            {
                ...
                if (obj instanceof getConstructor(item.classname))
                {
                    return item.id;
                }
            }
    ...
    _pDesignForm._getObject = function (name_)
    {
    ...
    _pDesignForm.setLayoutProperty = function (compid, layoutname, propid, propval)//, currentlayoutname)
    {
        ...
        const ret = layout[propid];
        if (nexacro._Browser !== "Runtime")
        {
            let obj = this._getObject(compid);
            if (compid !== 'this')
            {
                obj = obj.parent;
            }
            ...
            if (obj && this._checkFormBase(obj))
            {
                const element = window.document.getElementById(obj._unique_id);
                if(element)
                {
                    switch(propid)
                    {
                        ...
                        case "tabletemplate":
                        {
                            if (element.hasAttribute(`data-layout-${propid}`))
                            {
                                if (ret)
                                {
                                    element.setAttribute(`data-layout-${propid}`, ret);
                                }
                                else
                                {
                                    element.removeAttribute(`data-layout-${propid}`);
                                }
                            }
                            else
                            {
                        ...
                        case "stepcount":
                        {
                            if(propval && Number(propval) > 1)
                            {
                                element.setAttribute(`data-layout-${propid}`, propval);
                            }
                            else
                            {
                                if (element.hasAttribute(`data-layout-${propid}`))
                        ...
                        case "tablecellincompalign":
                        {
                            if (element.getAttribute('data-layout-type') === 'table')
                            {
                                const _element = window.document.getElementById(obj._unique_id + '._inner_form:container');
                                if (_element)
                                {
                                    if (propval)
                                    {
                                        _element.setAttribute('data-cellincompalign', propval);
                                    }
                                    else
                                    {
                                        if (_element.hasAttribute('data-cellincompalign'))
                                        {
                                            _element.removeAttribute('data-cellincompalign');
                                        }
                                    }
                                }
                            }
    ...
    _pDesignForm.applyFluid = function (compid)
    {
        let obj = this._getObject(compid);
        if (!obj)
        {
        ...
        const env = this._getEnvironment();
        const manager = nexacro._getLayoutManager(env);
        ...
        const is_fluid = manager.isFluidLayoutType(obj);
        if (!is_fluid)
        {
            return;
        }

        const fluidmanager = manager._fluidlayoutmanager;
        if (fluidmanager)
        {
            manager.applyFluid(obj);
        }
    };

    _pDesignForm.setElementClassName = function(id, className)
    {
        const obj = this._getObject(id);
        const element = window.document.getElementById(obj._unique_id);
        if (element)
        {
            element.setAttribute('data-classname', className);
        }
    }

    _pDesignForm.recalcLayout = function (compid)
    {
        const obj = this._getObject(compid);
        if (!obj)
        {
            return;
        }
    ...
    nexacro.setGlobalContents = function (compid, contents, jsonval)
    {
        const obj = nexacro._getApplicationObject(compid);
        ...
        {
            contents_val = JSON.parse(contents);
        }
    ...
    _pDesignForm.loadIncludeScript = function (mainurl)
    {
        if (nexacro._Browser == "Runtime")
        {
            nexacro.FormBase.prototype.loadIncludeScript.call(this.mainurl);
        }
        else
            return;
      
    };
    _pDesignForm.initEvent = function ()
    {
        if (nexacro._Browser == "Runtime" )
        {
            nexacro.FormBase.prototype.initEvent.call(this);
        }
        else
            this._init_status = 1;
        //this.on_initEvent();
    ...
    _pDesignForm._on_init = function ()
    {
        if (nexacro._Browser == "Runtime" )
        {
            nexacro.FormBase.prototype._on_init.call(this);
        }
        else
        {
            this._clear();
    ...
    if (nexacro._Browser == "Runtime")
    {
        ...
        _pDesignForm.getViewTemplateResult = function (filepath, src_script, fieldarray, contents, generationattr, preview)
        {
            ...
            try
            {
                ...
                if (preview)
                {
                    if (typeof globalThis.fn_GetViewGenerationPreview === "function")
                    {
                        ret = globalThis.fn_GetViewGenerationPreview(fieldarray, contents, generationattr);
                        globalThis.fn_GetViewGenerationPreview = undefined;
                    }
                }

                if (ret == null && typeof globalThis.fn_GetViewGenerationResult === "function")
                {
                    ret = globalThis.fn_GetViewGenerationResult(fieldarray, contents, generationattr);
                    globalThis.fn_GetViewGenerationResult = undefined;
                }
            }
            catch (e)
            {
        ...
        _pDesignForm.getViewTemplateGenerationAttrList = function (filepath, src_script)
        {
            ...
            try
            {
                ...
                if (typeof globalThis.fn_GetViewAttributeList === "function")
                {
                    ret = globalThis.fn_GetViewAttributeList();
                    globalThis.fn_GetViewAttributeList = undefined;
                }
            }
            catch (e)
            {
        ...
        _pDesignForm.getFieldUserAttrList = function (filepath, src_script)
        {
            ...
            try
            {
                ...
                if (typeof globalThis.fn_GetFieldUserAttributeList === "function")
                {
                    ret = globalThis.fn_GetFieldUserAttributeList();
                    globalThis.fn_GetFieldUserAttributeList = undefined;
                }
            }
            catch (e)
            {
    ...
    _pDesignForm.recalcDesignLayout = function (recalc_innerform, recalc_sublayout)
    {
        this._recalcDesignLayout(recalc_innerform, recalc_sublayout);
    }
    ...
    {
        // TODO : Design화면 언어코드     
        return nexacro.getEnvironment()._stringrc_lang;           
    };
    ...
    _pDesignForm._getChild = function (childname)
    {
        ...
        let childname_ = childname;
        if (childname_.substring(0, 5) === 'this.')
        {
            childname_ = childname_.substring(5);
        }

        var obj = this._inner_form;
        for (const part of childname_.split('.')) {
            if (!obj) return undefined;
            obj = obj[part];
        }
        return obj;
        ...
        _pDesignForm._swapPositionUnit = function (obj, pos, unit)
        {
            ...
            if (info)
            {
                ...
                if (distance.indexOf("rem") > -1 || distance.indexOf("em") > -1)
                {
                    if (distance.indexOf("rem") > -1)
                    {
                        tmp_comp = tmp_comp._getRootObject();
                    }
                    refer_font = tmp_comp._getReferenceAbsoluteFont();
                    tmp_size = obj._getFontRefSize(refer_font);
                }
    ...
    {
        var parent = comp._p_parent ?? this;
        ...
        if (!nexacro._isNull(parent._design_form))
        {
            ...
            const env = parent._getEnvironment();
            const manager = nexacro._getLayoutManager(env);
            if (manager)
            {
                var layout = manager.getCurrentLayout(parent);
    ...
    _pDesignForm.getDesignComponentContents = function(id)
    {
        let obj = this;
        if(id !== "this" && this.id !== id)
        {
            obj = this._getObject(id)
        }
        ...
        if(obj)
        {
            ...
            if(el && el.handle)
            {
                return nexacro._getHTMLOuter(el.handle,window.document);
            }
        }
        return "";
    }
    ...
    if (nexacro._Browser == "Runtime")
    {
        ...
        {
            var env = this._getEnvironment();
            var realurl = env._getFDLLocation(url);
        ...
        _pApplicationAccessPort.insertColumn = function (datasetid, idx, id, strtype, size, prop, text, datapath)
        {
            ...
            {
                type = datautils._typeint[strtype.toLowerCase()];
            }
            ...
            var newcolinfo = new nexacro.DSColumnInfo(id, strtype, type, size, prop, text, datapath, idx, undefined, datautils);
            ds._p_colcount++;
        ...
        _pApplicationAccessPort.createFrame = function (classname, parentid, frameid)
        {
            ...
            if (classnameobj)
            {
                ...
                if (classname == "MainFrame" || classname == "nexacro.MainFrame")
                {
                    // 이미 mainframe이 있는 경우?
                ...
                else if (classname == "ChildFrame" || classname == "nexacro.ChildFrame")
                {
                    // childframe는 formurl 이 들어간다.
                    obj = new classnameobj(frameid, null, null, null, null, null, null, "", parent);
                }
        ...
        _pApplicationAccessPort.deleteFrame = function (frameid)
        {
            ...
            var parent = frame ? frame._p_parent : null;
            if (frame && parent)
            {
        ...
        _pApplicationAccessPort.setContents = function (compid, contents, jsonval)
        {
            ...
            if (jsonval === true && !nexacro._isNull(contents) && contents.length > 0)
            {
                contents_val = JSON.parse(contents);//eval("contents_val = " + contents);
            }
        ...
        _pFormAccessPort.getVariant = function (varid)
        {
            ...
            const targetPath = varid.replace("this._inner_form", "this._inner_form.target");
            return targetPath.split('.').reduce((acc, curr) => 
            {
                if (acc === null || acc === undefined) return undefined;
                if (curr === "this") return this;
        ...
        _pFormAccessPort.setParameters = function (compid, parameters)
        {
            ...
            {
                var arrParam = [];
                arrParam = new Function("return " + parameters)();
    ...
    */
    ...
    /*globalThis.Form = nexacro.DesignForm;*/
    ...
    if (nexacro._Browser == "Runtime")
    {
        ...
        _pSubLayoutOverlayElement = null;
    }
}
