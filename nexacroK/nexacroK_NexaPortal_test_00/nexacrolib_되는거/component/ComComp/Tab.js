...
if (!nexacro.Tab)
{
    ...
    _pTab.on_attach_contents_handle = function (win, enablea11y)
    {
        var tabpages = this._p_tabpages;
        var tabpages_length = tabpages.length;
        var tabbuttonitems = this._tabbuttonitems;
        ...
        if (this._p_nextbutton)
        {
            this._p_nextbutton.attachHandle(win, enablea11y);
        }

        if (this._p_prevbutton)
        {
            this._p_prevbutton.attachHandle(win, enablea11y);
        }

        for (var i = 0; i < tabpages_length; i++)
        {
            tabpages[i].attachHandle(win, enablea11y);
        }

        for (i = 0; i < tabbuttonitems_length; i++)
        {
            tabbuttonitems[i].attachHandle(win, enablea11y);
        }
    ...
    _pTab.on_fire_user_onkeydown = function (key_code, alt_key, ctrl_key, shift_key, from_comp, from_refer_comp, meta_key)
    {
        ...
        if (!this.onkeydown || (this.onkeydown && !this.onkeydown.defaultprevented))
        {
            ...
            else if (this._getEnvironment()?._p_enableaccessibility && this._getEnvironment()?._p_accessibilitycontentsearchkey)
            {
    ...
    _pTab.on_touchend_basic_action = function (touch_manager, touchinfos, changedtouchinfos/*, refer_comp*/)
    {
        ...
        if (control)
        {
            ...
            if (!bsame && control instanceof nexacro._TabButtonItemControl)
            {
    ...
    _pTab._getPrevButtonSize = function ()
    {
        if (!this._prevbutton_size)
        {
            ...
            {
                var iconurl = nexacro._getURIValue(icon);
                iconurl = this._getEnvironment()?._getImageLocation(iconurl, this._getRefFormBaseUrl());
                var iconsize = nexacro._getImageSize_extended(iconurl, this._onload_prevbutton_iconimage, this);
                if (iconsize)
                {
    ...
    _pTab._getNextButtonSize = function ()
    {
        if (!this._nextbutton_size)
        {
            ...
            else
            {
                var iconurl = nexacro._getURIValue(icon);
                iconurl = this._getEnvironment()?._getImageLocation(iconurl, this._getRefFormBaseUrl());
                var iconsize = nexacro._getImageSize_extended(iconurl, this._onload_nextbutton_iconimage, this);
                if (iconsize)
                {
        ...
        if (beginTabindex == undefined || beginTabindex == null)
        {
            ...
            for (i = tabindex; i >= 0; i--)
            {
                ...
                if (tabposition == "top" || tabposition == "bottom")
                {
                    ...
                    is_visible = env?._p_enableaccessibility ? (tabbuttonitem._p_left > -1 && tabbuttonitem._p_visible) : tabbuttonitem._p_visible;
                }
                else
                {
                    is_within_maxsize = temp_height <= tab_max_height;
                    is_visible = env?._p_enableaccessibility ? (tabbuttonitem._p_top > -1 && tabbuttonitem._p_visible) : tabbuttonitem._p_visible;
                }
    ...
    _pTab._keydown_filter = function (keycode, alt_key, ctrl_key, shift_key, event_bubbles, fire_comp, refer_comp, meta_key)
    {
        ...
        switch (keycode)
        {
            ...
            case 38: //nexacro.Event.KEY_UP;                
                if (env?._p_enableaccessibility && _is_fire_keydown)
                {
                    //if button -> go tab
        ...
        for (i = 0; i < tabpage_nodes.length; i++)
        {
            ...
            if (!set_tabpage_props["url"])
            {
                ...
                while (contents_node)
                {
                    ...
                    class_name = this._getEnvironment()?._getRegisterClass(contents_node.tagName);
                    if (!class_name)
                    {
                        return false;	// parse error
                    }
...
if (!nexacro._TabButtonItemControl)
{
    ...
    _pTabButtonItemControl.on_attach_contents_handle = function (win, enablea11y)
    {
        if (this._tabbuttonitemtext)
            this._tabbuttonitemtext.attachHandle(win, enablea11y);
        if (this._tabbuttonitemicon)
            this._tabbuttonitemicon.attachHandle(win, enablea11y);
        if (this._extrabutton)
            this._extrabutton.attachHandle(win, enablea11y);
    ...
    _pTabButtonItemControl._getExtraButtonIconSize = function ()
    {
        ...
        if (icon)
        {
            ...
            var imagesize = nexacro._getImageSize_extended(iconurl, this._onload_extra_iconimage, this);
            if (imagesize)
            {
    ...
    _pTabButtonItemControl._getTabButtonIconSize = function ()
    {
        ...
        if (icon)
        {
            ...
            var imagesize = nexacro._getImageSize_extended(iconurl, this._onload_tabbutton_iconimage, this);
            if (imagesize)
            {
                return { width: imagesize.width, height: imagesize.height };
            }
