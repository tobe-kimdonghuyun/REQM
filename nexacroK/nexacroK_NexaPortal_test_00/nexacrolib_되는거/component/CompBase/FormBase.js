...
if (!nexacro.Form)
{
    ...
    _pTriggerManager.inittriggerinfo = function ()
    {
        ...
        for (var i = 0; i < triggers.length; i++)
        {
            ...
            // 기존 table 동작 호환용
            if (nexacro._trigger_type_table[trigger.triggertype])
            {
                triggertype = nexacro._trigger_type_table[trigger.triggertype];
            }
            
            if (triggertype)
            {
                triggerview = this._getTriggerView(trigger.triggerview);
                if (triggerview) 
                {
                    ...
                    {
                        triggerobj = (triggertype == "onload") ? triggerview : this._getTriggerObj(triggerview, trigger.triggerobj);

                        if (triggerobj) 
                        {
                            let notifytrigger = triggerview.on_notifytrigger;
                            if (notifytrigger && triggerobj._findEventHandler(triggertype, notifytrigger, triggerview))
                            {
                                triggerobj._addEventHandler(triggertype, notifytrigger, triggerview);
                            }
                        }
                    }
                }
    ...
    _pTriggerManager._getTriggerView = function (obj, child_view)
    {
        var form = this._form;
        if (!form)
            return;
        ...
        let view_name = triggerview.split(".");
        if (child_view)
        {
            targetview = child_view[view_name.splice(0, 1)];
        }
        else if (view_name.length > 0)
        {
            targetview = form[view_name.splice(0, 1)];
        }

        // multi name 매칭
        if (targetview && view_name.length > 0)
        {
            targetview = this._getTriggerView(view_name.join("."), targetview);
        }
    ...
    _pTriggerManager._getTriggerObj = function (obj, triggerobj, child)
    {
        var form = this._form;
        if (!form)
            return;
        ...
        let obj_name = triggerobj.split(".");
        if (triggerview._is_view && !child)
        {
            // targetview: view(root)
            targetobj = triggerview._p_form[obj_name.splice(0, 1)];
        }
        else
        {
            // targetview: form(root), view(child), Div(child), Tabpage(child)
            targetobj = triggerview[obj_name.splice(0, 1)];
        }

        // multi name 매칭
        if (targetobj && obj_name.length > 0)
        {
            targetobj = this._getTriggerObj(targetobj, obj_name.join("."), true);
        }
    ...
    // triggerview(form/view) 내 target의 name 가져오는 함수
    _pTriggerManager._getTargetName = function (targetobj, triggerview, triggertype)
    {
        if (!targetobj || !triggerview || triggertype == "onload")
            return "";

        if (targetobj._is_form)
            return targetobj == triggerview ? targetobj.name : "";

        let target_name = targetobj.name;
        if (triggerview._contains(targetobj))
        {
            while (targetobj.parent)
            {
                if (targetobj.parent == triggerview || targetobj.parent == triggerview._p_form)
                    break;

                targetobj = targetobj.parent;
                target_name = targetobj.name + "." + target_name;
            }
        }
        return target_name;
    };
    ...
    _pTriggerManager._findTrigger = function (triggertype, triggerobj, triggerview)
    {
        ...
        var triggerview_name = this._getTargetName(triggerview, this._form, triggertype); // triggerview가 ""가 아닐때 처리용
        var triggerobj_name = this._getTargetName(triggerobj, triggerview, triggertype); // triggerobj 찾아오는 함수
        var targetobj = triggers[triggerobj_name];
        var targettrigger = [];
        ...
        for (var i = 0; i < targetobj.length; i++)
        {
            ...
            {
                let target_triggertype = targetobj[i].triggertype;

                // 기존 table 동작 호환
                if (nexacro._trigger_type_table[targetobj[i].triggertype])
                {
                    target_triggertype = nexacro._trigger_type_table[targetobj[i].triggertype];
                }

                if (triggertype == target_triggertype &&
                    (targetobj[i].triggerview == "" || triggerview_name == targetobj[i].triggerview))
                {
    ...
    _pTriggerManager._notifyTrigger = function (triggertype, triggerobj, triggerview, evt)
    {
    ...
    {
        return nexacro._createExprFunc(expr_str, ds, ["triggerview", "triggerobj", "form"]);
    };
    ...
    _pBindManager.on_changevalue = function (obj, e)
    {
        ...
        var val = e.val;
        var bind_item;
        var abstract_comp = obj._getAbstractParent();
        if (abstract_comp)
        {
            bind_item = this._findBindItem(abstract_comp, prop_id);
        }
        else
        {
    ...
    _pBindManager._notifyAll = function (ds_id, col_id, col, all_flag, val)
    {
        ...
        if (arr_bind)
        {
            for (var sx = 0; sx < arr_bind.length; sx++)
            {
                ...
                if (!bind_item._dataset || bind_item._p_columnid == "")
                {
                    continue;
                }
    ...
    _pFormBase._use_native_scroll = true;
    _pFormBase._use_native_scroll_size = false;
    ...
    _pFormBase.on_created_contents = function (win)
    {
        ...
        if (this._is_created_contents)
        {
            ...
            var panellen = panels.length;
            var is_arrange = false;
            var panel;
            for (j = panellen - 1; j > -1; j--)
            {
                panel = panels[j];

                if (panel.fittocontents != "none")
                {
                    panel._update_position();
                }
                if (!is_arrange && panel._arrange_info)
                {
                    is_arrange = true;
                }
            }
            if (is_arrange)
            {
                for (j = 0; j < panellen; j++)
                {
                    panel = panels[j];

                    if (panel._arrange_info)
                    {
                        panel._update_position();
                    }
                }
        ...
        if (env._p_enableaccessibility && env._a11ytype == 2 && nexacro._Browser == "Chrome")
        {
    ...
    _pFormBase._clearUserFunctions = function ()
    {
        ...
        if (nexacro.DesignForm && this instanceof nexacro.DesignForm)objPrototype = nexacro.DesignForm.prototype;
        
        else if (this._type_name == "Div") objPrototype = nexacro.Div.prototype;
    ...
    _pFormBase._afterClearProperties = function (recovery_props)
    {
        ...
        {
            if (this["set_" + i] == nexacro._emptyFn)
                this["_p_" + i] = recovery_props[i];
            else
    ...
    _pFormBase._onHttpSystemError = function (obj, bfireevent, errorobj, errortype, url, returncode, requesturi, locationuri, extramsg)
    {
        ...
        else
        {
            ...
            nexacro._onHttpSystemError(obj, bfireevent, errorobj, errortype, url, returncode, requesturi, locationuri, extramsg);
        }
    };
    ...
    _pFormBase._onHttpTransactionError = function (obj, bfireevent, errorobj, errortype, url, returncode, requesturi, locationuri, extramsg, svcid)
    {
        ...
        if (bfireevent)
        {
            ...
            if (!ret)
            {
                var environment = this._getEnvironment();
                if (environment)
                {
    ...
    _pFormBase._addPreloadList = function (type, url, id, args)
    {
        ...
        {
            fullurl = env._getServiceLocation(url);
            this._load_manager.addPreloadItem(type, fullurl, id, args, service);
        }
        else
        {
            fullurl = env._getFDLLocation(url);
    ...
    _pFormBase._getApplication = function ()
    {
        return this._getEnvironment()?._getApplication();
    };
    ...
    _pFormBase.loadCss = function (url, base_url, async)
    {
        ...
        var cssurl = env._getServiceLocation(url, base_url, null, false);
        ...
        var service = env._getServiceObject(url);       
        var checkversion = env._p_checkversion;
    ...
    {
        for (var f = this; (f != null); f = (f._p_parent || f._p_opener))
        {
            if (name in f)
                return f[name];
        }
    ...
    /*
        _pFormBase._scrollTo = function (hpos, vpos, bapplyscrollbar, bautohidetimer, evttype, evtkind)
        {
            
            if (!this._p_stepselector) 
                nexacro.Component.prototype._scrollTo.call(this, hpos, vpos, bapplyscrollbar, bautohidetimer, evttype, evtkind);
            else
            {
                ...
                let stepselector = this._p_stepselector;
                var cur_stepindex = stepselector.stepindex;
                var next_stepindex = cur_stepindex;
    
                if ((vpos < current_vpos) || (hpos < current_hpos))
                {
                        this._createStepChangeAnimation(next_stepindex--, 600); //just reuse the function 
                       // stepselector.set_stepindex(next_stepindex);
                }
    ...
    */
    ...
    _pFormBase._on_urlduplicate = function ()
    {
        this._applyUrlQueryString();
        this._applyUrlScrollId();

        this.on_fire_urlduplicate();        
    };

    _pFormBase.on_fire_urlduplicate = function ()
    {
        if (this.onurlduplicate && this.onurlduplicate._has_handlers)
        {
            var evt = new nexacro.LoadEventInfo(this, "onurlduplicate", this._url);
            this.onurlduplicate._fireEvent(this, evt);
        }        
    };
    //===============================================================
    ...
    _pFormBase.loadForm = function (formurl, async, reload, baseurl)
    {
        ...
        var env = this._getEnvironment();
        var url = env._getFDLLocation(formurl, baseurl);
        var service = env._getServiceObject(formurl);
    ...
    _pFormBase._getTabOrderNext = function (current, direction, filter_type)
    {
        ...
        if (cur_idx < 0)
        {
            var ar_all = this._getComponentsByTaborder(this, this._getEnvironment()?._p_enableaccessibility ? (8 + 7) : 7);
    ...
    _pFormBase._getComponentsByTaborder = function (p, filter_type, include_not_created, bAccessibility)//, arEdit)
    {
        ...
        if (comps)
        {
            ...
            {
                var comp = comps[i];
                if (!comp || !comp._is_alive || (!include_not_created && !comp._is_created) || (comp._isVisible && !comp._isVisible() || !comp._p_visible) || 
                    ((comp._isEnable && !comp._isEnable() || !comp._p_enable) && !(filter_type & 2)) || comp._popup)
                    continue;
                if (!(filter_type & 1))
                {
                    if (bAccessibility)
                    {
                        if (!comp._p_accessibilityenable || (comp._isVisible && !comp._isVisible() || !comp._p_visible))
                            continue;
                    }
    ...
    _pFormBase._searchNextTabFocus = function (current, b_search_from_first, opt_force_cycle, filter_type)
    {
        ...
        var tabkeycirculation = 0;
        var env = this._getEnvironment();
        if (env)
            tabkeycirculation = env._tabkeycirculation;
        ...
        else
        {
            ...
            if (!next)
            {
                ...
                else if (tabkeycirculation == 2 || tabkeycirculation == 3) 
                {
                    //마지막일 경우
                    while (!next)
                    {
                        ret = parent._getNextFrame(this);
                        if (ret && ret.form)
                        {
                            next = ret.form._getTabOrderFirst(filter_type);
                            parent = ret.form.parent;
                        }
                        if (!ret)
                            break;
                    }
    ...
    _pFormBase._getComponentsByLayoutorder = function ()
    {
        const comps = this._p_components;
        let ar = [];
    ...
    _pFormBase._getLayoutOrderFirst = function ()
    {
    ...
    _pFormBase._getLayoutOrderLast = function ()
    {
        var ar = this._getComponentsByLayoutorder();
        if (ar && ar.length > 0)
            return ar[ar.length - 1];
    ...
    _pFormBase._getLayoutOrderNext = function (currentid, direction)
    {
    ...
    _pFormBase._processArrowKey = function (bdown, newfocus_comp)
    {
        ...
        //TODO
        var env = this._getEnvironment(), comp, _label;
        if (!env) 
            return;
    ...
    _pFormBase._setFittocontents = function (v)
    {
        ...
        var control_elem = this.getElement();
        if (control_elem)
            control_elem._setElementFittocontents(v);
    };
    /*
        _pFormBase._on_beforescroll = function (prehpos, prevpos, posthpos, postvpos, evttype, evtkind, is_native)
        {
            if (this._p_stepselector && is_native)
            {
                let control_elem = this.getElement();
                let step_count = control_elem._step_count;
                let step_preindex = control_elem._step_index;
                if (step_count > 0)
                {            
                   let client_width = control_elem.client_width;                              
                   let step_index = nexacro.round(posthpos / client_width - 0.5);
                   step_index = step_index < 0 ? 0 : step_index;
                   step_index = step_index >= step_count ? step_count - 1 : step_index;
                   if (step_preindex != step_index)
                        this.setStepIndex(step_index);                
                }      
            }
    ...
    */
    _pFormBase.on_scrollEnd = function (change_hpos, change_vpos, before_hpos, before_vpos, hpos, vpos)
    {
        nexacro.Component.prototype.on_scrollEnd.call(this, change_hpos, change_vpos, before_hpos, before_vpos, hpos, vpos);
        if (this._p_stepselector && change_hpos)
        {
            let control_elem = this.getElement();
            let step_count = control_elem._step_count;
            if (step_count > 0)
            {
                let client_width = control_elem.client_width;
                let step_index = nexacro.round(hpos / client_width - 0.5);
                step_index = step_index < 0 ? 0 : step_index;
                step_index = step_index >= step_count ? step_count - 1 : step_index;
                this.setStepIndex(step_index);
            }
    ...
    _pForm._adjustPosition = function ()
    {
        const app = this._getRootObject(); // chk
        if (this == app)
        {            
            const rect = nexacro._getContentRect(this.__containerhandle)
            this._left = this._p_left = rect.paddingLeft;
            this._top = this._p_top = rect.paddingTop;
            this._width = this._p_width = rect.contentsWidth;
            this._height = this._p_height = rect.contentsHeight;
        }
        
        nexacro.FormBase.prototype._adjustPosition.call(this);        
    };
    ...
    _pForm.on_created_contents = function (win)
    {
        ...
        if (this == app)
        {
            ...
            if (control_element)
            {                
                control_element.setElementRtl(this._getEnvironment()?._rtl);
            }
        }
    ...
    _pForm.destroyComponent = function (callremovechild)
    {
        ...
        if (this == app)
        {
            ...
            if (env._communication_waiting)
                env._communication_waiting = false;
    ...
    _pForm.createWindow = function ()
    {
        ...
        if (!parent && !nexacro.isglobalmodule)
    ...
    _pForm._initControlElementLayoutProps = function ()
    {
        ...
        if (this._isFluidContainer() && nexacro._is_native_fluidlayout && this.getStepCount() < 1)
        {
    ...
    _pForm._on_load = function (obj, url)
    {
        ...
        else if (this._p_parent || !nexacro.isglobalmodule)
        {
            this._on_loadcallback(obj, url);
        }
    ...
    _pForm._applyUrlQueryString = function ()
    {
        if (!this._p_parent)
            return;

        const urlquerystring = this._p_parent._getUrlQueryString();
        if (urlquerystring)
        {        
            const queryparams = urlquerystring.split('&');
            ...
            this._p_parent._deleteUrlQueryString();
        }  
        
    };

    _pForm._applyUrlScrollId = function ()
    {
        if (!this._p_parent)
            return;

        const urlscrollid = this._p_parent._getUrlScrollId();
        if (urlscrollid)
        {           
            const keys = urlscrollid.split(".");
            let parent = this;
            let scrollcomp = null;
            ...
            if(scrollcomp)
            {
                scrollcomp._resetScrollPos(scrollcomp, 
                    scrollcomp._adjust_left, 
                    scrollcomp._adjust_top, 
                    scrollcomp._adjust_left + scrollcomp._adjust_width, 
                    scrollcomp._adjust_top + scrollcomp._adjust_height);
            }

            this._p_parent._deleteUrlScrollId();
        }
    }
    ...
    _pForm._on_loadcallback = function ()
    {
        ...
        if (this._is_main || (parent && parent._is_frame && parent._p_form == this && parent._window_type == 0))
        {
            application._pushContentsForm(this);
        }
        ...
        if (parent)
        {
            ...
            else
            {
                ...
                if (_window && _window.root && _window.root._activate == true)
                {
                    ...
                    const env = this._getEnvironment();
                    const a11ywholereadtype = (env._p_enableaccessibility && env._a11ywholereadtype > 1);
    ...
    //===============================================================
    _pForm._update_position = function ()
    {
    ...
    _pForm.on_update_position = function (resize_flag, move_flag, update)
    {
        ...
        if (this == app)
        {            
            if (this._is_window && ((this._isAllowDefaultPinchZoom() && !nexacro._isDesktop()) ||
    ...
    {
        if (nexacro.Component.prototype.on_apply_prop_stringresource.call(this) == false)
            return false;

        if (nexacro.isDesignMode)
            return false; 

        if (this._p_parent && !(this instanceof nexacro._InnerForm))
        {
    ...
    _pForm._reapplyRemValues = function ()
    {
        var comps = this._p_components;
        if (!comps) return;
        for (var i = 0, n = comps.length; i < n; i++)
    ...
    _pForm._on_rootcssclasschange = function (precssclass, postcssclass)
    {
        if (!this._is_alive)
            return;
        ...
        {
            comp = comps[i];
            if (comp._hasContainer() && comp._on_rootcssclasschange)
                comp._on_rootcssclasschange(precssclass, postcssclass);
        }
    };
    ...
    _pForm.on_fire_sys_onrbuttonup = function (button, alt_key, ctrl_key, shift_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, meta_key) // 2013-09-04 pss - eventflow 
    {
        ...
        nexacro._quickview_mode = true;  // check
            ...
            if (this._isComponentKeydownAction() && (keycode == nexacro.Event.KEY_LEFT || keycode == nexacro.Event.KEY_RIGHT))
            {
                ...
                if (hscrollbar && hscrollbar._p_visible && (ctrl_key || env._p_accessibilitycontentsearchkey))
                {
                    if (!dlgc.want_arrows) // arrowkey를 직접 처리하는 컴포넌트는 제외
                    {
                        ...
                        var scroll_pos = hscrollbar.pos;
                        hscrollbar.set_pos(scroll_pos + line);
                        return scroll_pos == hscrollbar.pos ? false : true;
                    }
                }
            }
            else if (this._isComponentKeydownAction() && (keycode == nexacro.Event.KEY_UP || keycode == nexacro.Event.KEY_DOWN))
            {
                ...
                if (vscrollbar && vscrollbar._p_visible && (ctrl_key || env._p_accessibilitycontentsearchkey))
                {
                    if (!dlgc.want_arrows) // arrowkey를 직접 처리하는 컴포넌트는 제외
                    {
                        ...
                        var scroll_pos = vscrollbar.pos;
                        vscrollbar.set_pos(scroll_pos + line);
                        return scroll_pos == vscrollbar.pos ? false : true;
                    }
                }
            }
        }
    ...
    _pForm.on_fire_onrootcssclasschange = function (precssclass, postcssclass)
    {
    ...
    _pForm._resetScroll = function (bForceUpdatePosition)
    {
        if (this._is_created_contents)
        {
            var comp;
            ...
            for (var i = 0, n = comps.length; i < n; i++)
            {
                ...
                if (bForceUpdatePosition || comp._arrange_info || (comp.fittocontents != "none"))
                    comp._update_position();
            }
            var panellen = panels.length;
            var is_arrange = false;
            for (j = panellen - 1; j > -1; j--)
            {
                ...
                if (bForceUpdatePosition || panel.fittocontents != "none")
                {
                    panel._update_position();
                }
                if (!is_arrange && panel._arrange_info)
            ...
            if (is_arrange)
            {
                for (j = 0; j < panellen; j++)
                {
                    ...
                    if (bForceUpdatePosition || panel._arrange_info)
                    {
                        panel._update_position();
                    }
                }
    ...
    _pForm.close = function (arg)
    {
        ...
        childframe._close_argument = arg;
        if (childframe._window)
            childframe._window.returnValue = arg;
    ...
    _pForm._getNextComponent = function (comp, no_composite_flag, only_enable_flag)
    {
        ...
        if (cur_idx < 0)
        {
            ...
            if (cur_all_idx < 0)
            {
                var p_form = comp._p_parent;

                if (p_form && p_form !== this)
                {
                    if (p_form._getNextComponent)
                    {
                        var next = p_form._getNextComponent(comp, no_composite_flag, only_enable_flag);
                        if (next) return next;
                    }

                    var parent = (p_form instanceof nexacro._InnerForm) ? p_form._p_parent : p_form;
                    var parent_form = parent._getForm();
                    if (!(parent_form instanceof nexacro.ChildFrame))
                        return parent_form._getNextComponent(parent, no_composite_flag, only_enable_flag);
                }
            }
    ...
    _pForm._getPrevComponent = function (comp, no_composite_flag, only_enable_flag)
    {
        ...
        if (cur_idx < 0)
        {
            ...
            if (cur_all_idx < 0)
            {
                ...
                if (p_form && p_form !== this)
                {
                    if (p_form._getPrevComponent)
                    {
                        var prev = p_form._getPrevComponent(comp, no_composite_flag, only_enable_flag);
                        if (prev) return prev;
                    }
    ...
    _pForm.insertChild = function (idx, id, obj)
    {
        ...
        if (obj._is_component)
        {
            if (this._p_components.length == 0)
            {
    ...
    _pForm.transaction = function (id, url, inDatasetsParam, outDatasetsParam, argsParam, callbackFn, isAsync, datatype, isCompress)
    {
        ...
        var realurl = env._getServiceLocation(url, this._base_url, null, false);
    ...
    _pForm.setStepCount = function (cnt)
    {
        ...
        if (this._p_stepselector && manager)
        {
            var layout_name = manager.getCurrentLayoutName();
            manager.setLayoutProperty(this, layout_name, "stepcount", this._p_stepselector.stepcount);
        }
    };
    ...
    _pForm.getNavTarget = function (navtarget)
	{
		if (!navtarget) return;
        if (typeof navtarget != "string") return;
		
        const app = this._getApplication();
		if (!app) return;

		return app._getNavidObject(navtarget);		
	};
    ...
    {
        var control_elem = this._control_element;
        if (this._is_scrollable && control_elem && this._is_created_contents && this._is_alive)
        {
            ...
            if (!fromComp)
            {
                ...
                if (this._p_stepselector && this._p_stepselector.stepcount > 0)
                {
                    ...
                    {
                        comp = comps[i];
                        if (comp && comp.getOffsetWidth() <= 0 || comp.getOffsetHeight() <= 0) continue;                        
            ...
            */
            ...
            if (vscroll)// && (vscroll._p_visible || (vscroll instanceof nexacro.ScrollIndicatorControl)))
            {
                ...
                if (top - vpos < client_height && bottom - vpos > client_height)
                {
                    ...
                    else if (vpos >= top && vpos < bottom) //현재 컴포넌트가 보인다면
                        ;//Do nothing
                    else
                        vscroll.set_pos(top);
                }
                ...
                {
                    var _vpos = vscroll.pos;
                    if (focus_direction == 1 && bottom - top > client_height)
                        _vpos = bottom - client_height;
                    else if (_vpos >= top && _vpos < bottom) //현재 컴포넌트가 보인다면
                        ;//Do nothing
                    else
                        _vpos = top;
    ...
    _pForm._setSize = function (width, height)
    {
        if (this._adjust_width != width || this._adjust_height != height)
        {
            ...
            */
            ...
            const env = this._getEnvironment();
            var layout_manger = env._getLayoutManager();
            if (layout_manger)
            {
    ...
    _pForm._getSortedDecendants = function (p, include_not_tabstop, bAccessibility)//, arEdit)
    {
        ...
        if (comps)
        {
            ...
            for (var i = 0; i < comp_len; i++)
            {
                ...
                if (!comp || !comp._is_created || !comp._p_visible || ((comp._isEnable && !comp._isEnable() || !comp._p_enable) && (!env?._p_enableaccessibility || env?._a11ytype != 5)) || comp._popup)
                {
                    continue;
                }
                /*
    ...
    _pForm.on_notifytrigger = function (obj, info)
    {
        if (info)
        {
            ...
            if (manager)
            {
                manager._notifyTrigger(triggertype, triggerobj, triggerview, info);
            }
        }
    };
    ...
    _pForm._selectLayout = function ()
    {
        ...
        const env = this._getEnvironment();
        var layout_manager = env._getLayoutManager();
        if (layout_manager)
        {
    ...
    _pForm._setInnerFlexibleFlag = function (is_flexible, is_calc)
    {
        ...
        if (is_flexible != this._is_flexible_container)
        {
            ...
            if (this._is_created)
            {
                if (nexacro._is_native_fluidlayout && this.getStepCount() < 1)
                {
                    this._setLayoutContainerProps();
                }
            }
    ...
    _pForm._applyFlexiblePosition = function (is_flexible)
    {
        ...
        for (var i = 0, len = components.length; i < len; i++)
        {
            ...
            if (control_elem)
            {
                control_elem.setElementLayoutFluidPos(is_flexible && control_elem.visible);
                if (is_flexible)
                    comp._setLayoutContentsProps();
                else
                {
                    control_elem._clearLayoutBasis();
                    control_elem.setElementPosition(comp._adjust_left,comp._adjust_top);
                    //if (comp._p_right || comp._p_bottom)
                    //    comp.move(comp._p_left, comp._p_top, comp._p_width, comp._p_height, comp._p_right, comp._p_bottom);  
                }
            }
        }
    };
        ...
        if (this == app)
        {
            if (this._window)
            {
                ...
                if (wait_flag == true)
                {
                    if (this._ref_comm == 0)
                    {                       
                        env._communication_waiting = true;
                        ...
                        var waitcursor_imgurl = this._getEnvironment()?._getLoadingImageUrl();
                        if (waitcursor_imgurl)
    ...
    if (!nexacro._InnerForm)
    {
        ...
        _pInnerForm._getScrollType = function ()
        {
            var parent = this._p_parent;
            if (parent)
                return parent._p_formscrolltype ? parent._p_formscrolltype : this._p_scrolltype;
            return this._p_scrolltype;
        };

        _pInnerForm._getScrollPropagation = function ()
        {
            let scrollpropagation = this._p_scrollpropagation;
            let parent = this._p_parent;
            if (parent)
                scrollpropagation = parent._p_formscrollpropagation ? parent._p_formscrollpropagation : scrollpropagation;
            return scrollpropagation ? scrollpropagation : (this._getEnvironment()?._p_scrollpropagation || "both");
        };
        ...
        _pInnerForm.loadForm = function (formurl, async, reload, baseurl)
        {
            ...
            if (this._load_manager)
            {
                ...
                {
                    // ChildFrame > Form  
                    var application = this._getApplication(); // chk
                    if (application)
                        application._registerLoadforms(this);
                }
    ...
    if (!nexacro._CompositeComponent)
    {
        ...
        _pCompositeComponent._load_module = function ()
        {
            ...
            if (refform)
            {
                ...
                {
                    var env = this._getEnvironment();
                    var url = env._getFDLLocation("CompositeInner.xfdl", baseurl);
        ...
        _pCompositeComponent.on_attach_contents_handle = function (win/*, enablea11y*/)
        {
            var form = this._p_form;
        ...
        _pCompositeComponent.set_scrollpropagation = function (scrollpropagation)
        {
            if (this._p_scrollpropagation != scrollpropagation)
                this._p_scrollpropagation = nexacro.Component.prototype.set_scrollpropagation.call(this._p_form, scrollpropagation);
        };
        ...
        _pCompositeComponent._on_rootcssclasschange = function (precssclass, postcssclass)
        {
            if (this._p_form)
            {
                this._p_form._on_rootcssclasschange(precssclass, postcssclass);
            }
        };
