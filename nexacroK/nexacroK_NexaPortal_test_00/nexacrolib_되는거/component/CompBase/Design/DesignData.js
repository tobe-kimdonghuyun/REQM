...
if (nexacro.DatasetBase)
{
    ...
    _pDatasetBase._updateColType = function (idx, type)
    {
        ...
        {
            updated = colList._updateType(idx, type, this._datautils);
            if (updated)
            {
                this._clearAllExprs();
                if (this._eventstat)
        ...
        {
            updated = constList._updateType(idx - colList.length, type, this._datautils);
            if (updated)
            {
                this._clearAllExprs();
                if (this._eventstat)
    ...
    _pDSColumnInfoList._updateType = function (idx, type, datautils)
    {
        var colinfo = this[idx];
        var intype = nexacro._toString(type).toLowerCase();
        var ntype = datautils._typeint[intype];
    ...
    _pVariableList._updateType = function (idx, type, datautils)
    {
