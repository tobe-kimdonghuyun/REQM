...
if (!nexacro.TextField)
{
    ...
    {
        if (nexacro.FieldBase.prototype.on_apply_prop_stringresource.call(this) == false)  
            return false;
    ...
    */
    ...
    _pTextFieldBox.on_attach_contents_handle = function (win, enablea11y)
    {
        this._input_element.attachHandle(win);

        if (this._leadbutton)
        {
            this._leadbutton.attachHandle(win, enablea11y);
            this._leadbutton._setAccessibilityStatHidden(true);
        }

        if (this._prefixctrl)
        {
            this._prefixctrl.attachHandle(win, enablea11y);
            this._prefixctrl._setAccessibilityStatHidden(true);
        }

        if (this._postfixctrl)
        {
            this._postfixctrl.attachHandle(win, enablea11y);
            this._postfixctrl._setAccessibilityStatHidden(true);
        }

        if (this._trailbutton)
        {
            this._trailbutton.attachHandle(win, enablea11y);
    ...
    {
        return nexacro._checkShowContextMenu(this._p_parent, this._p_parent._getEnvironment() ? this._p_parent._getEnvironment()?._p_usecontextmenu : "all");
    };
