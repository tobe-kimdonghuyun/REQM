...
if (!nexacro.Radio)
{
    ...
    _pRadio.on_change_bindSource = function (propid, ds, row, col/*, index*/)
    {
        if (propid == "value")
        {
            ...
            if (ds && item_len)
            {
                ...
                else
                {
                    if (dir == "horizontal")
                    {
                        ...
                        else if ((radio_columncount == - 1 && radio_rowcount == 0) ||
                                 (radio_columncount == 0 && radio_rowcount == -1) ||
                    ...
                    else
                    {
                        ...
                        else if (radio_columncount > 0 && ds_rowcount >= radio_rowcount)
                        {
    ...
    _pRadio._on_radio_onkeydown = function (obj, e)
    {
        ...
        if (nexacro._enableaccessibility)
        {
            if (keycode == E.KEY_UP || keycode == E.KEY_DOWN) // 다음,이전 컴포넌트로 이동
                return false;
        }
        ...
        else if (keycode == E.KEY_LEFT || keycode == E.KEY_RIGHT || keycode == E.KEY_UP || keycode == E.KEY_DOWN) // radioitem 이동
        {
            var ds = this._innerdataset;
            if (!ds || this.readonly)
            {
    ...
    _pRadio._recalcLayout = function ()
    {
        ...
        if (control_elem)
        {
            ...
            if (ds && item_len)
            {
                ...
                if (fittocontents == "none" && radio_columncount == -1 && radio_rowcount == -1)
                {
                    if (dir == "horizontal")
                    {
                        apply_rowcnt = 1;
