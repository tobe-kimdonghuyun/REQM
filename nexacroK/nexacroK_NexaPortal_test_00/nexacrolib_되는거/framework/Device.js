...
if (!nexacro.DeviceI)
{
	...
	*/
	...
	_pDeviceI.encodeString = function (source)
	{
		...
		value = value.replace(/\t/g, "&#9;");
		value = value.replace(/\\/g, "&#92;");
		value = value.replace(/\u001D/g, "&#29;"); // Group Separator
		value = value.replace(/\u001E/g, "&#30;"); // SSV - RS
		value = value.replace(/\u001F/g, "&#31;"); // SSV - US
		value = value.replace(/\u0003/g, "&#3;");
		return value;
	};
	...
	_pDeviceI.decodeString = function (source)
	{
		...
		value = value.replace(/&#9;/g, "\t");
		value = value.replace(/&#92;/g, "\\");
		value = value.replace(/&#29;/g, String.fromCharCode(29)); // Group Separator
		value = value.replace(/&#30;/g, String.fromCharCode(30)); // SSV - RS
		value = value.replace(/&#31;/g, String.fromCharCode(31)); // SSV - US
		value = value.replace(/&#3;/g,  String.fromCharCode(3));
		value = value.replace(/&amp;/g, "&");
		return value;
	};
	...
	_pDeviceI.DatasetToJSONString = function (dataset)
	{
		...
		var datautils = dataset._getDataUtils();
		//var colNames = [];
		var started = false;
			...
			for (var j = 0; j < colSize; j++)
			{
				...
				else 
				{
					switch (colInfo.ntype) 
					{
						case 2: /* int */
						case 3: /* float, double */
							valueString = datautils.toTextFromDecimal(value);
							break;
						case 4: /* decimal, bigdecimal */
							valueString = '"' + datautils.toTextFromDecimal(value) + '"';
							break;
						case 5: /* date */
							valueString = '"' + datautils.toTextFromDate(value) + '"';
							break;
						case 6: /* time */
							valueString = '"' + datautils.toTextFromTime(value) + '"';
							break;
						case 7: /* datetime */
							if (value.dateObj == undefined)
								valueString = '"' + datautils.toTextFromDateTime(value) + '"';
							else
								valueString = '"' + datautils.toTextFromDateTime(value.dateObj) + '"';
							break;
						case 1: /* string */
	...
	_pDeviceI.JSONObjectToDataset = function (jsonObject, dataset)
	{
		...
		{
			dataset.addColumn(colInfos[i].name, datautils.toTypeName(colInfos[i].type));
		}
...
if (nexacro.System)
{
	...
	{
		var _id = nexacro._OS == "iOS" ? "0000" : nexacro.Device.makeID();
		var json = {
			id: _id,
			div: "PhoneInfo",
			method: "getAll"
...
if (nexacro.ProtocolAdp)
{
	...
	nexacro.ProtocolAdp.setProtocolVar = function (name, key, val, env)
	{
		if (!nexacro.ProtocolAdp._isRuntime() || !env)
			return;

		var createadaptor = false;
		var protocoladp = env._getProtocol(name);
		if (!protocoladp)
		{
			var isprotocol = env._isProtocol(name);
			if (isprotocol)
			{
		...
		if (protocoladp)
		{
			if (createadaptor && protocoladp.initialize)
			{
				...
				env._addProtocol(name, protocoladp);
			}
		}
