(function()
{
    return function(nexacro, system, trace, environment, application)
    {
        ...
        {
            this.set_name("TA_Dataset_M_createDataset_01_v2");
            this.set_titletext("TA_Dataset_M_createDataset_01_v2");
            ...
            obj.getSetter("position").set("absolute");
            obj.set_taborder("1");
            obj.set_text("[Dataset_M_createDataset]");
            this.addChild(obj.name, obj);
        ...
        // User Script
        this.addIncludeScript("A_Dataset_M_createDataset_01.xfdl","lib::libCommon.xjs");
        this.addIncludeScript("A_Dataset_M_createDataset_01.xfdl","libevent::event.util.xjs");
        this.registerScript("A_Dataset_M_createDataset_01.xfdl", function(nexacro, system, trace, environment, application) {
        /***********************************************************************
        * 설명      : Dataset createDataset Method
        ***********************************************************************
        *     수정일     작성자   내용
        ***********************************************************************
        ...
        ***********************************************************************/
        var tc1  = {
        	description: "Dataset createDataset Method Test",
        ...
        tc1.task['createDataset'] = function(strDatasetID, arrColList, strWhereExpr, nStartIdx, nEndIdx) {
        	var result = [];
        	var rtncreateDataset;
        	var rtnsaveCSV;

        	if(strWhereExpr == "생략" && nStartIdx == "생략" && nEndIdx == "생략")
        		rtncreateDataset = this.dataset.createDataset(strDatasetID, arrColList);
        	else if(nStartIdx == "생략" && nEndIdx == "생략")
        		rtncreateDataset = this.dataset.createDataset(strDatasetID, arrColList, strWhereExpr);
        	else if(nEndIdx == "생략")
        		rtncreateDataset = this.dataset.createDataset(strDatasetID, arrColList, strWhereExpr, nStartIdx);
        	else
        		rtncreateDataset = this.dataset.createDataset(strDatasetID, arrColList, strWhereExpr, nStartIdx, nEndIdx);

        	getSaveCSV = rtncreateDataset.saveCSV();

        	this.txt_result.set_value(getSaveCSV);
        	rtnsaveCSV = this.txt_result.value;

        	result.push(rtncreateDataset);
        ...
        tc1.validator ['tc1_vd1'] = {
        	description: "createDataset(strDatasetID, arrColList, strWhereExpr, nStartIdx, nEndIdx) 수행",
        	autoTaskFlow : ["createDataset"],
        	vcinfo : {
        	...
        	run: function () {
        		...
        		switch(taskName) {
        		case "createDataset":
        			var rtn = tc1.task['createDataset'].call(this, strDatasetID, arrColList, strWhereExpr, nStartIdx, nEndIdx);
        ...
        ***********************************************************************/
        this.TA_Dataset_M_createDataset_01_v2_onload = function(obj,e)
        {
        ...
        ***********************************************************************/
        ...
        {
            this.addEventHandler("onload",this.TA_Dataset_M_createDataset_01_v2_onload,this);
            this.btn00.addEventHandler("onclick",this.btn00_onclick,this);
            this.btn_main.addEventHandler("onclick",this.btn_main_onclick,this);
            this.btn_Combo_menu.addEventHandler("onclick",this.btn_button_menu_onclick,this);
        };

        this.loadIncludeScript("A_Dataset_M_createDataset_01.xfdl");
