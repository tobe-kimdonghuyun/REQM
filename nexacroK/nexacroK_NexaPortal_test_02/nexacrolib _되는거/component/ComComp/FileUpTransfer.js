...
if (!nexacro.FileUpTransfer)
{
	...
	_pFileUpTransfer._promise_resolve = null;
	_pFileUpTransfer._promise_reject = null;
	...
	_pFileUpTransfer.upload = function (strUrl, strReturnType)
	{
		// init
		this._aborted = false;
		...
		// url validate
		strUrl = strUrl ? env._getServiceLocation(strUrl, null, null, false) : (this._p_url ? env._getServiceLocation(this._p_url, null, null, false) : "");
		//if (!strUrl)
		//	return;

		this._response_headers = undefined;
		var isPromise = (strReturnType === "promise") ? true : false;
		return nexacro._uploadTransfer(this._p_filelist, this._p_postdatalist, env._getHttpVariableAll(), strUrl, -1, this, isPromise);
	};

	_pFileUpTransfer.getAllResponseHeaders = function ()
	{
		return this._response_headers;
	};

	_pFileUpTransfer.getResponseHeader = function (headername)
	{
	...
	_pFileUpTransfer.on_load_android_webview = function (objData)
	{
		...
		else
		{
			...
			if (data)
			{
				...
				if (nexacro._Deserializer[data_type])
				{
					data = nexacro._Deserializer[data_type](data, this);
				}
	...
	{
		var resolve = this._promise_resolve;
	...
	_pFileUpTransfer.on_fire_onerror = function (errortype, errormsg, errorobj, statuscode, requesturi, locationuri, index)
	{
		...
		if (reject) reject({ErrorCode: statuscode, ErrorMsg: errormsg, url: requesturi});
	};
