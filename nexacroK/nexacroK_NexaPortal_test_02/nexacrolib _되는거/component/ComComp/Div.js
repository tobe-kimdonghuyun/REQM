...
if (!nexacro.Div)
{
    ...
    _pDiv._p_formscrollpropagation = undefined;

    /* Navigate Component Init*/
    _pDiv._p_navid = "";
    _pDiv._urlquerystring = undefined;
    _pDiv._urlscrollid = undefined;
    ...
    _pDiv._is_navtarget_component = true;
    ...
    _pDiv.on_attach_contents_handle = function (win, enablea11y)
    {
        var form = this._p_form;
        if (!this._p_url)
        {
    ...
    _pDiv._getUrlQueryString = function ()
    {        
        return this._urlquerystring;        
    }

    _pDiv._deleteUrlQueryString = function ()
    {
        delete this._urlquerystring;
    }

    _pDiv._getUrlScrollId = function ()
    {        
        return this._urlscrollid;        
    }

    _pDiv._deleteUrlScrollId = function ()
    {
    ...
    _pDiv._on_apply_formscroll = function ()
    {
        ...
        if (form)
        {
            ...
            if (this._p_formscrollpropagation !== undefined)
                form.on_apply_scrollpropagation();
        }
    }
    ...
    _pDiv.set_formscrollpropagation = function (v)
    {
        if (v)
        {  
        ...
        if (this._p_formscrollpropagation != v)
        {
            this._p_formscrollpropagation = v;
            if (this._p_form)
                this._p_form.on_apply_scrollpropagation();
        }       
    };
...
/*
    _pDiv.set_navid = function (val)
    {
        this._p_navid = val;
        this._registerNavid(this._p_navid, this); 
    };
  */  
    _pDiv.on_apply_font = function (font)
    {
        nexacro.Component.prototype.on_apply_font.call(this, font);
        this._reapplyRemValues();
    };

    _pDiv._reapplyRemValues = function ()
    {
        if (this._p_form)
            this._p_form._reapplyRemValues();
    };

    _pDiv._properties = [{ name: "async" }, { name: "semantictag" }, { name: "url" }, { name: "urlchangeeffect" }, { name: "text" }, { name: "textAlign" },
    { name: "verticalAlign" }, { name: "formscrollbarbarminsize" }, { name: "formscrollbarbaroutsize" }, { name: "formscrollbardecbuttonsize" }, 
    { name: "formscrollbarincbuttonsize" }, { name: "formscrollindicatorsize" }, { name: "formscrolltype" }, { name: "form", readonly: true }, 
    { name: "formscrollbarsize" }, { name: "formscrollbartrackbarsize" }, { name: "formscrollbartype" }, { name: "formscrollpropagation" }, { name: "navid" }   
    ];
    ...
    _pDiv.getParentContext = function ()
    {
        ...
        {
            while (parent && !parent._load_manager) parent = parent.parent;
        }
    ...
    _pDiv._on_rootcssclasschange = function (precssclass, postcssclass)
    {
        if (this._p_form)
        {
    ...
    _pDiv._on_getFluidTableContainerSize = function (contents_max_rect, contents_scroll_rect)
    {
        ...
        {
            case "width":
                //c_w = contents_max_rect ? contents_max_rect[0] : (contents_scroll_rect ? contents_scroll_rect[0] : 0);
                c_w = contents_scroll_rect ? contents_scroll_rect[0] : contents_max_rect[0];
                c_w = border ? c_w + border._getBorderWidth() : c_w;
                c_h = this._adjust_height;
                break;

            case "height":
                c_w = this._adjust_width;
                //c_h = contents_max_rect ? contents_max_rect[1] : (contents_scroll_rect ? contents_scroll_rect[1] : 0);
                c_h = contents_scroll_rect ? contents_scroll_rect[1] : contents_max_rect[1];
                c_h = border ? c_h + border._getBorderHeight() : c_h;
                break;
        ...
        // 스크롤바 보정: 1번째 인자(fit_contents_max_rect = 셀 영역) 기준
        if (contents_max_rect)
        {
            var orig_c_w = c_w;
            var orig_c_h = c_h;
            ...
            {
                if (orig_c_h < contents_max_rect[1])
                {
                    var vscroll = this._getVScrollBarSize ? this._getVScrollBarSize() : 0;
                    c_w += vscroll;
                }
            }
            ...
            {
                if (orig_c_w < contents_max_rect[0])
                {
                    var hscroll = this._getHScrollBarSize ? this._getHScrollBarSize() : 0;
                    c_h += hscroll;
                }
            }
        }
        ...
        // min/max 보정
        if (this._minwidth != null && this._minwidth >= 0) c_w = Math.max(c_w, this._minwidth);
        if (this._maxwidth != null && this._maxwidth >= 0) c_w = Math.min(c_w, this._maxwidth);
        if (this._minheight != null && this._minheight >= 0) c_h = Math.max(c_h, this._minheight);
        if (this._maxheight != null && this._maxheight >= 0) c_h = Math.min(c_h, this._maxheight);
    ...
    _pDiv._setContents = function (str)
    {
        ...
        while (node)
        {
            if (node.nodeType == 1)
            {
                var className = this._getEnvironment()?._getRegisterClass(node.tagName);
                if (!className)
                    return false;
    ...
    _pDiv._updateMinSize = function (current_min, value)
    {
        if (current_min == null) return current_min;
        return value > current_min ? value : current_min;
    };

    // width 축 보정 처리
    _pDiv._adjustWidthAxis = function (width, minwidth, parent_fittocontents)
    {
        var adjust_fit_width = this._getAdjustFitPositionWidth(width, parent_fittocontents);
        if (adjust_fit_width !== width)
        {
            this._fit_width = adjust_fit_width;
            return {
                width: adjust_fit_width,
                minwidth: adjust_fit_width,
                changed: true
            };
        }
        return { width: width, minwidth: minwidth, changed: false };
    };

    // height 축 보정 처리
    _pDiv._adjustHeightAxis = function (height, minheight, parent_fittocontents)
    {
        var adjust_fit_height = this._getAdjustFitPositionHeight(height, parent_fittocontents);
        if (adjust_fit_height !== height)
        {
            this._fit_height = adjust_fit_height;
            return {
                height: adjust_fit_height,
                minheight: adjust_fit_height,
                changed: true
            };
        }
        return { height: height, minheight: minheight, changed: false };
    };
    ...
    _pDiv._getSizeInfo = function ()
    {
        ...
        if (fit_type === "width" || fit_type === "both")
        {
            ...
            if(fit_size[0] <= 0 && width && typeof width === "string" && width.indexOf("%") > -1)
            {
                var containerComp = this._getParentComponent();
                if (containerComp)
                {
                    var refW = containerComp._basis_width || containerComp._adjust_width;
                    if (refW > 0)
                    {
                        width = this._convToPixel(width, refW);
                    }
                }
            }
            else
            ...
            minwidth = this._updateMinSize(minwidth, width);
           // this._basis_width = width;
        }

        if (fit_type === "height" || fit_type === "both")
        {
            if(fit_size[1] <= 0 && height && typeof height === "string" && height.indexOf("%") > -1) // fittocontents 결과가 0이고, 원래 % height가 있으면 fallback
            {
                var containerComp = this._getParentComponent();
                if (containerComp)
                {
                    var refH = containerComp._basis_height || containerComp._adjust_height;
                    if (refH > 0)
                    {
                        height = this._convToPixel(height, refH);
                    }
                }
            }
            else
            ...
            minheight = this._updateMinSize(minheight, height);
         //   this._basis_height = height;
        }

        // 3. height 보정 (모든 케이스에서 수행)
        var height_result = this._adjustHeightAxis(height, minheight, parent_fittocontents);
        height = height_result.height;
        if (height_result.changed)
        {
            minheight = height_result.minheight;
        }

        // 4. width 보정 (모든 케이스에서 수행)
        var width_result = this._adjustWidthAxis(width, minwidth, parent_fittocontents);
        width = width_result.width;
        if (width_result.changed)
        {
            minwidth = width_result.minwidth;
        }
        //trace("_getSizeInfo: ", this._type_name, " fit_type=", fit_type, " width=", width, " height=", height);
    ...
    //href = prefix::url.xfdl?A=B&C=D#id 형식으로 전달
    _pDiv.on_click_navigate = function (href, option)
    {
        let bsameurl = false;
        if (href)
        {
            let url = href.split('?')[0];  // url 설정
            if (this._p_url == url)            
                bsameurl  = true;
        }        
