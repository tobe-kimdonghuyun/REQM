...
if (!nexacro.BasicChart)
{
    ...
    _pBasicChart._setGroupColumnDatapoints = function (series)
    {
        ...
        function findPosition(targetSeries)
        {
            var pos = 0;

            var stackindex = pThis._getVisibleStackGroupindexs(targetSeries);
            if (stackindex)
            {
                pos = stackindex[0];
            }
        ...
        function calculCenterBarShift()
        {
            ...
            var midSeries = barVisibleSeriesset[Math.floor(barVisibleCnt / 2)];
            if (midSeries)
            {
                serieswidth = midSeries._groupbarwidth;
                if (serieswidth)
                {
        ...
        function isBarAtLeftOfCenter(seriesPosition)
        {
            return seriesPosition <= Math.ceil(barVisibleCnt / 2);
        }

        function sumWidth(seriesSet, start, end)
        {
            var totalWidth = 0,
                barwidth = 0;
            ...
            {
                var seriesbarwidth = seriesSet[i]._groupbarwidth,
                    groupbarwidth = 0;
                ...
                barwidth = pThis._getcheckTimeAxisBarWidth(barwidth);
                totalWidth += barwidth + barBorderHalfWidthInAxisUnit * 2;
            }

            return totalWidth;
        }

        function shiftPoints(targetDatapoints, targetSeries, dx)
        {
            var ps = targetDatapoints.pointsize;
            var points = targetDatapoints.points;
            var j = 0;
            var linetype = targetSeries.linetype;

            if ((linetype == "step" && targetSeries.linevisible) || isstepline)
                ps += 3;
            ...
            for (var i = rotateaxis ? 1 : 0; i < points.length; i += ps)
            {
                ...
                if ((linetype == "step" && targetSeries.linevisible) || isstepline)
                {
                    if (targetDatapoints.points.length > i + 3)
                        points[i + 3] = points[i];
                }
                targetSeries._data[j][3] = points[i];
                j++;
            }
            return points;
        }

        function getAxeMinMaxValues(targetDatapoints)
        {
            var resultMinMax = [];
            ...
            points = targetDatapoints.points;
            pointsize = targetDatapoints.pointsize;
            format = targetDatapoints.format;
            ...
            if (rotateaxis)
            {
                ...
                {
                    resultMinMax[0] = Number(ymin);
                }

                if (nexacro._isNumber(ymax))
                {
                    resultMinMax[1] = Number(ymax);
                }
            }
            else
            {
                ...
                {
                    resultMinMax[0] = Number(xmin);
                }

                if (nexacro._isNumber(xmax))
                {
                    resultMinMax[1] = Number(xmax);
                }
            }
            return resultMinMax;
        }

        function calculBorderAndBarWidth(targetSeries)
        {
            var barBorderHalfWidth = targetSeries._barborderwidth ? (targetSeries._barborderwidth / 2) : 0;
            barBorderHalfWidthInAxisUnit = barBorderHalfWidth * pixelInXWidthEquivalent;
        ...
        if (board)
        {
            ...
            if (rotateaxis)
            {
                ...
                boardWidth = board.width - boardBorderWidth;
                boardHeight = board.height - boardBorderHeight - tickstartgap - tickendgap - tickendspace;
            }
            else
            {
                axis = series._xaxis;
                ...
                boardWidth = board.width - boardBorderWidth - tickstartgap - tickendgap - tickendspace;
                boardHeight = board.height - boardBorderHeight;
            }
        }
        ...
        {
            offset = -1 * (sumWidth(barVisibleSeriesset, position - 1, Math.floor(barVisibleCnt / 2) - 1)) - centerBarShift;
        }
        else
        {
            offset = sumWidth(barVisibleSeriesset, Math.ceil(barVisibleCnt / 2), position - 2) + centerBarShift - barBorderHalfWidthInAxisUnit;
        }

        shiftedPoints = shiftPoints(datapoints, series, offset);
...
if (!nexacro.ChartBasicSeriesControl)
{
    ...
    _pChartBasicSeriesControl._drawSeriesCurveLines = function ()
    {
        ...
        function checkpoints(px1, py1, px2, py2, xaxisObj, yaxisObj)
        {
            var _pts = [];
            _pts[0] = px1;
            _pts[1] = py1;
        ...
        function addCurveLine(type, targetPoints, cpoints, bskipmove)
        {
            ...
            if (line.length == 0)
            {
                ...
                line.push([targetPoints[0], targetPoints[1], cpoints.concat(targetPoints.slice(2)), type]);
            }
            else if (type == "quadraticCurveTo" && targetPoints.length == 2)
            {
                cpoints = cpoints.slice(0, 2).concat(targetPoints);

                line.push([targetPoints[0], targetPoints[1], cpoints, type]);
            }
            else
            {

                line.push([targetPoints[2], targetPoints[3], cpoints.concat(targetPoints.slice(2)), type]);
            }
        }

        function getControlPoints(px0, py0, px1, py1, px2, py2, targetTension)  
        {
            var pow = Math.pow,
                sqrt = Math.sqrt,
    ...
    _pChartBasicSeriesControl._drawBarItemText = function (left, right, bottom, top, width, height, item)
    {
        ...
        function positionstart(targetItemText, l, r, b, t, w, h, targetItemtextGap, rotateAxisObj)
        {
            var textTop = 0;
            var textLeft = 0;

            if (!targetItemtextGap) targetItemtextGap = 0;

            if (rotateAxisObj)
            {
                targetItemText.set_verticalAlign("middle");
                targetItemText.set_textAlign("left");
                textTop = t + (h / 2);
                textLeft = l + targetItemtextGap;
            }
            else
            {
                targetItemText.set_verticalAlign("bottom");
                targetItemText.set_textAlign("center");
                textLeft = l + (w / 2);
                textTop = b - targetItemtextGap;
            }

            targetItemText.set_x(textLeft);
            targetItemText.set_y(textTop);

        }
        function positionmiddle(targetItemText, l, r, b, t, w, h, targetItemtextGap, rotateAxisObj)
        {
            targetItemText.set_verticalAlign("middle");
            targetItemText.set_textAlign("center");
            var textTop = 0;
            ...
            {
                textTop = t + (h / 2);
                textLeft = l + (w / 2) + targetItemtextGap;
            }
            else
            {
                textLeft = l + (w / 2);
                textTop = t + (h / 2) - targetItemtextGap;
            }
        ...
        function positionend(targetItemText, l, r, b, t, w, h, targetItemtextGap, rotateAxisObj)
        {
            var textTop = 0;
            var textLeft = 0;
            ...
            {
                targetItemText.set_verticalAlign("middle");
                targetItemText.set_textAlign("right");
                textTop = t + (h / 2);
                textLeft = r - targetItemtextGap;
            }
            else
            {
                targetItemText.set_verticalAlign("top");
                targetItemText.set_textAlign("center");
                textLeft = l + (w / 2);
                textTop = t + targetItemtextGap;
        ...
        function positionoutside(targetItemText, l, r, b, t, w, h, targetItemtextGap, rotateAxisObj)
        {
            var textTop = 0;
            var textLeft = 0;
    ...
    _pChartBasicSeriesControl._drawPointItemText = function (cx, cy, item)
    {
        ...
        function positionlefttop(targetItemText, x, y, itemtextGap)
        {
            // xy
            targetItemText.set_verticalAlign("bottom");
            targetItemText.set_textAlign("right");
            ...
            targetItemText.set_x(textLeft - itemtextGap);
            targetItemText.set_y(textTop - itemtextGap);

        }
        function positioncentertop(targetItemText, x, y, itemtextGap)
        {
            // y
            targetItemText.set_verticalAlign("bottom");
            targetItemText.set_textAlign("center");
        ...
        function positionrighttop(targetItemText, x, y, itemtextGap)
        {
            //xy
            targetItemText.set_verticalAlign("bottom");
            targetItemText.set_textAlign("left");
        ...
        function positionleftmiddle(targetItemText, x, y, itemtextGap)
        {
            //x
            targetItemText.set_verticalAlign("middle");
            targetItemText.set_textAlign("right");
        ...
        function positioncentermiddle(targetItemText, x, y, itemtextGap)
        {
            //y
            targetItemText.set_verticalAlign("middle");
            targetItemText.set_textAlign("center");
        ...
        function positionrightmiddle(targetItemText, x, y, itemtextGap)
        {
            //x
            targetItemText.set_verticalAlign("middle");
            targetItemText.set_textAlign("left");
        ...
        function positionleftbottom(targetItemText, x, y, itemtextGap)
        {
            //xy
            targetItemText.set_verticalAlign("top");
            targetItemText.set_textAlign("right");
        ...
        function positioncenterbottom(targetItemText, x, y, itemtextGap)
        {
            //y
            targetItemText.set_verticalAlign("top");
            targetItemText.set_textAlign("center");
        ...
        function positionrightbottom(targetItemText, x, y, itemtextGap)
        {
            //xy
            targetItemText.set_verticalAlign("top");
            targetItemText.set_textAlign("left");
    ...
    _pChartBasicSeriesControl._drawLineItemText = function (datapoints, axisx, axisy, item)
    {
        ...
        function positionlefttop(itemText, cx, cy, targetItemtextGap, width)
        {
            // xy
            ...
            itemText.set_x(textLeft - targetItemtextGap);
            itemText.set_y(textTop - targetItemtextGap);

        }
        function positioncentertop(itemText, cx, cy, targetItemtextGap, width)
        {
            // y
            itemText.set_verticalAlign("bottom");
        ...
        function positionrighttop(itemText, cx, cy, targetItemtextGap, width)
        {
            //xy
            itemText.set_verticalAlign("bottom");
        ...
        function positionleftmiddle(itemText, cx, cy, targetItemtextGap, width)
        {
            //x
        ...
        function positioncentermiddle(itemText, cx, cy, targetItemtextGap, width)
        {
            //y
            itemText.set_verticalAlign("middle");
            itemText.set_textAlign("center");
        ...
        function positionrightmiddle(itemText, cx, cy, targetItemtextGap, width)
        {
            //x
            itemText.set_verticalAlign("middle");
            itemText.set_textAlign("left");
        ...
        function positionleftbottom(itemText, cx, cy, targetItemtextGap, width)
        {
            //xy
        ...
        function positioncenterbottom(itemText, cx, cy, targetItemtextGap, width)
        {
            //y
            itemText.set_verticalAlign("top");
            itemText.set_textAlign("center");
        ...
        function positionrightbottom(itemText, cx, cy, targetItemtextGap, width)
        {
            //xy
            itemText.set_verticalAlign("top");
            itemText.set_textAlign("left");
