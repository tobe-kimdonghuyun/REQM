...
if (!nexacro.TitleBarControl)
{
    ...
    _pTitleBarControl._on_endtrack = function (x, y, dragdata)
    {
        ...
        if (_window && (_window._wheelZoom != undefined))
        {
            ...
            if (ownerframe.getElement().zoom != 100)    // ownerframe.zoom이 100인 상태에서 _window._wheelZoom이 걸린 상태는 modal. modal인 경우 이곳에서 wheel zoom 처리 안해도 됨 
            {
                wheelZoom = 1.0;
            }
        }
    ...
    _pTitleBarControl._on_movetrack = function (x, y, dragdata, windowX, windowY)
    {
        ...
        if (ownerframe)
        {
            ...
            {
                var callbackfunc = function () { ownerframe._on_titlebar_movetrack(x / wheelZoom, y / wheelZoom, dragdata, windowX, windowY); };
                if (!this._movetrack_animationframe)
                {
                    var pThis = this;
