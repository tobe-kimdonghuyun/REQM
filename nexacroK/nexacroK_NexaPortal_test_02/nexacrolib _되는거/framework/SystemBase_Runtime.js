...
if (nexacro._Browser == "Runtime")
{
    ...
    if (!nexacro._Init_systembase_runtime)
    {
        ...
        */
        ...
        // css
        nexacro._getRootCSSClass = nexacro._emptyFn;
        nexacro._broadcastXcssGroupUpdate = nexacro._emptyFn;
        nexacro._broadcastXcssGroupFilterUpdate = nexacro._emptyFn;
        nexacro._refreshAllRootCSSVariables = nexacro._emptyFn;
        // for error message
        ...
        //extended : src -> nexacro._getURIValue 처리, fullurl -> nexacro._getImageLocation 처리
        //service 처리에 대한 문제가 있음 
        nexacro._getImageSize_extended = function (src, callbackFn, pThis, fullurl, org_src, basync, imagewidth, imageheight)
        {           
        ...
        nexacro._download = function (url, is_popup_frame, handle, initname, targetpath, filefilter, filefilterindex, action, env)
        {
            ...
            if (env)
            {
                env._addCookieFromCookieVariables();
            }

            let app = env?._getApplication();
            app?._beginCommProgress();
        ...
        // Overlay Utils
        //==============================================================================
        nexacro._setTrackOverlay = nexacro._emptyFn;
        ...
        // Transfer Utils
        //==============================================================================
        nexacro._uploadTransfer = function (filelist, postdatalist, httpvariables, url, index, evttarget)
        {
            ...
            // cookie 설정
            const env = evttarget._getEnvironment();
            if (env)
            {
                env._addCookieFromCookieVariables();
            }
        ...
        nexacro._downloadTransfer = function (postdatalist, httpvariables, url, saveurl, evttarget, method)
        {
            var callback_onload = function ()
            {
                    ...
                    catch (e)
                    {
                        ...
                        if (bprinterror == true)
                        {
                            ...
                            else
                            {
                                ...
                                var environment = pthis._getEnvironment();
                                if (environment)
                                {
        ...
        nexacro.__startCommunication = function (loadItem, path, cachelevel, async, referer, senddata, ndatatype, compress, ver, failpass, service)
        {
            ...
            else if (loadItem.type == "data")
            {
                ...
                if (availableArrayBuffer)
                {
                    ...
                    {
                        protocoladp = env._getProtocol(loadItem.protocol);
        ...
        _pProgressDataSSV._setColInfo_from_chunk = function (colLine, target_ds, bOrgLayout)
        {
            ...
            if (bOrgLayout)
            {
                ...
                {
                    const env = target_ds._getEnvironment()
        ...
        /*
        nexacro.__calcZoomFactor = function (curscreen)
        {
            let zoom_factor = 0;
            ...
            const allow_default_pinchzoom = curscreen._allow_default_pinchzoom;
        ...
        */
        ...
        {
            let projecturl = nexacro._getProjectBaseURL(url);
...
*/
        ...
        nexacro._initScreen = function (screeninfo)
        {
            ...
            nexacro._applySelectedScreen(screeninfo, env);
        ...
        if (!nexacro.XMLHttpRequest)
        {
            ...
            _pXMLHttpRequest.open = function (method, url, async, user, password)
            {
                ...
                if (url)
                {
                    try
                    {   
                        var _url = this._url = url;
                        let env;
                        if (this._p_parent)
                        {
                            env = this._p_parent.getEnvironment();
                            if (env)
                                _url = this._url = env._getServiceLocation(url, env.getProjectPath());
                        }
                        else
