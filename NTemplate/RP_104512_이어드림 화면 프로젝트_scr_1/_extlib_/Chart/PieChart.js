...
if (!nexacro.PieChart)
{
    ...
    _pPieChart._createSeriesset = function (o, id)
    {
        ...
        if (seriesset)
        {
            ...
            if (seriesLength > 0)
            {
                // trace("pie chart의 경우 pieseries하나만 가질 수 있습니다.");
                return;
            }
        }
...
if (!nexacro.ChartPieSeriesControl)
{
    ...
    _pChartPieSeriesControl._drawPie = function ()
    {
        ...
        ////debug
        //var circle1 = new nexacro.GraphicsEllipse(centerX, centerY, 6, 6);
        //circle1.set_id(i +"center");
        //circle1.set_strokepen("2px solid red");
        ...
        function drawSlice(pThis, i)
        {
            if (items[i]._isShow)
            {
                ...
                // if (!Eco.isEmpty(selectedPieItem))
                // {
                //      selectitem = selectedPieItem[i];
                // }

                // if (chart.ishalfpie)
                // {
                //      rEndAngle = rStartAngle + (Math.PI  * (seriesItems[i].keydata/ total));
                // }
                // else
                // {
                //	    rEndAngle = rStartAngle + (Math.PI * 2 * (items[i].value / total));
                // }
                if (items.length == 1)
                {
                    rStartAngle = 0;
                ...
                else
                {
                    ...
                    var next2Angle = 0;
                    var prevAngle = 0;
                    //trace("valuelineblurangle", valuelineblurangle, "valuelineblurhalfangle ", valuelineblurhalfangle);
                    if (valuelineblurangle <= curAngle)
                    {
                        ...
                        else
                        {
                            if (i == items.length - 1)
                            {
                                ...
                                if (items.length > 3)
                                {
                                    next2Angle = items[1].angle * 180 / Math.PI;
                                }
                            }
                            else
                            {
                    ...
                    if (!items[i].isskip) // 앞뒤에 gradation 가능
                    {
                        if (!items[i].nextskip)
                        {
                            ...
                            rEndAngle = rStartAngle + (Math.PI * 2 * (items[i].value / total)) - _valuelineblurangle;
                        }
                        else
                        {
                            rEndAngle = rStartAngle + (Math.PI * 2 * (items[i].value / total));
                    ...
                    rHighlightEndAngle = rHighlightStartAngle + (Math.PI * 2 * (items[i].value / total));
                }

                // test
                //var isskip = items[i].isskip;
                //var nextskip = items[i].nextskip;
                //trace("drawSlice ", i, "curAngle", curAngle, "isskip", isskip, "nextskip", nextskip);
                //trace("drawSlice ", i, "curAngle", curAngle, "rStartAngle ", rStartAngle, "rEndAngle ", rEndAngle);
                //trace("drawSlice ", i, "curAngle", curAngle, "isskip", isskip, "nextAngle ", nextAngle, "prevAngle ", prevAngle);
                ...
                //trace("drawSlice index", i, " startAngle", startAngle, " endAngle", endAngle, " angle", angle);
                ...
                {
                    currentAngle += items[pThis._itemCnt - 1].angle;
                }
                ...
                if (isselectitem)
                {
                    ...
                    ////debug
                    //var circle1 = new nexacro.GraphicsEllipse(centerX, centerY, 15, 15);
                    //circle1.set_id(i +"selectdd");
                    //circle1.set_strokepen("2px solid red");
                    //circle1.set_fillstyle("green");
                    //seriesGroup.addChild(circle1);	
                }
                ...
                if (!nexacro._GraphicsLib.isEmpty(innerradius) && innerradius > 0)
                {
                    ...
                    //// debug
                    //var circle1 = new nexacro.GraphicsEllipse((Math.sin(rStartAngle) * innerw + centerX),(-Math.cos(rStartAngle) * innerh + centerY), 12, 12);
                    //circle1.set_id("test0" + i);
                    //circle1.set_strokepen("2px solid black");
                    //circle1.set_fillstyle("#00FFFF");
                    //seriesGroup.addChild(circle1);
                ...
                //// debug
                //var circle1 = new nexacro.GraphicsEllipse((Math.sin(rStartAngle) * w + centerX), (-Math.cos(rStartAngle) * h + centerY), 10, 10);
                //circle1.set_id("test00" + i);
                //// circle1.set_strokepen("2px solid red");
                //circle1.set_fillstyle("red");
                //seriesGroup.addChild(circle1);

                //var circle2 = new nexacro.GraphicsEllipse((Math.sin(rEndAngle) * w + centerX), (-Math.cos(rEndAngle) * h + centerY), 10, 10);
                //circle2.set_id("test11" + i);
                //circle2.set_strokepen("2px solid black");
                //circle2.set_fillstyle("blue");
                //seriesGroup.addChild(circle2);
                ...
                rStartAngle += Math.PI * 2 * (items[i].value / total);
                rHighlightStartAngle += Math.PI * 2 * (items[i].value / total);
            }
        } // end drawSlice
        ...
        function drawSliceBorder(pThis, i)
        {
            if (items[i]._isShow)
            {
                ...
                else
                {
                    ...
                    if (!isskip)
                    {
                        // gradation 설정 안한 경우도 빗살무늬의 흰줄이 일부 slice 에서 확인 -> draw할 때 라디안 실수값을 넘겨서 문제?
                        // -> gradation slice와 value slice 시작, 끝각의 라디안값 수치는 정상 (flot chart도 path 그릴 때 동일하게 소수점 radian 값을 넘겨주는거 정상확인)
                        // gradation 표현을 위해서 gradation slice의 시작, 끝각의 라디안값의 보정치를 임의로 넣어 흰줄이 표시되지 않도록 처리 (opacity를 주면 보정치에 대한 겹칩부분발생)
                    ...
                    //trace("drawSliceBorder index", i, "rStartAngle", rStartAngle, "rEndAngle", rEndAngle, "curAngle", curAngle);
                }

                //trace("drawSliceBorder", i, "rStartAngle", rStartAngle, "rEndAngle", rEndAngle);
                var startAngle = Math.round(rStartAngle * 180 / Math.PI),
					endAngle = Math.round(rEndAngle * 180 / Math.PI),
					w = radius,
					h = radius,
					...
					centerX = pThis._chart._centerLeft,
					centerY = pThis._chart._centerTop,
					selectindent = (pThis.selectindent || 0) / 100;

                items[i].startAngle = startAngle;
                items[i].endAngle = endAngle;
                items[i].angle = angle;

                //trace("drawSliceBorder index", i, " startAngle", startAngle, " endAngle", endAngle, " angle", angle);
                ...
                //    var circle1 = new nexacro.GraphicsEllipse((Math.sin(rEndAngle) * w + centerX), (-Math.cos(rEndAngle) * h + centerY), 10, 10);
                //    circle1.set_id("test11" + i);
                //    //circle1.set_strokepen("2px solid black");
                //    circle1.set_fillstyle("pink");
                //    seriesGroup.addChild(circle1);
                //}
                ...
                if (!nexacro._GraphicsLib.isEmpty(innerradius) && innerradius > 0)
                {
                    ...
                    //{
                    //    // debug
                    //    var circle1 = new nexacro.GraphicsEllipse((Math.sin(rStartAngle) * innerw + centerX), (-Math.cos(rStartAngle) * innerh + centerY), 10, 10);
                    //    circle1.set_id("test0" + i);
                    //    // circle1.set_strokepen("2px solid red");
                    //    circle1.set_fillstyle("blue");
                    //    seriesGroup.addChild(circle1);

                    //    var circle1 = new nexacro.GraphicsEllipse((Math.sin(rEndAngle) * innerw + centerX), (-Math.cos(rEndAngle) * innerh + centerY), 10, 10);
                    //    circle1.set_id("test1" + i);
                    //    //circle1.set_strokepen("2px solid black");
                    //    circle1.set_fillstyle("black");
                    //    seriesGroup.addChild(circle1);
                    //}
                ...
                //trace("firstcolor", firstcolor, "secondcolor", secondcolor);

                var startAngle1 = Math.round(rStartAngle1 * 180 / Math.PI);

                var startangle = pThis.startangle || 0;
                if (startAngle1 >= 360)
                    startAngle = startAngle1 % 360;

                //trace("index ", i, "startAngle1", startAngle1);
                ...
                //slice.set_fillstyle(firstcolor);
                //slice.set_opacity(0.5);
                //slice.set_strokepen("1px solid black");
                ...
                if (!isskip)
                {
                    //trace("index >> ", i, "isskip", isskip, "startAngle", startAngle, "gradation", gradation);
                    seriesGroup.addChild(slice);
                }
            }
        }
    ...
    _pChartPieSeriesControl._drawLabels = function ()
    {
        ...
        // // debug
        // var circle = new nexacro.GraphicsEllipse(this._chart._centerLeft, this._chart._centerTop, 10, 10);
        // circle.set_strokepen("2px solid black");
        // circle.set_fillstyle("blue");
        // this._chart._seriesGroup.addChild(circle);	
        ...
        function drawLabel(item, startangle, index, pThis)
        {
            ...
            {
                itemtextguidesize = radius - itemtextguidesize;
                if (itemtextguidesize <= 0)
                {
                    textX = centerX + Math.round(Math.sin(halfAngle) * (radius));
                    textY = centerY - Math.round(Math.cos(halfAngle) * (radius));
                }
                else
                {
            ...
            if (!nexacro._isNull(itemText))
            {
                ...
                {
                    var titlealign, titlehalign, titlevalign,
						centerAngle = halfAngle * 180 / Math.PI,
                ...
                //if (guideline)
                //{
                //   var circle1 = new nexacro.GraphicsRect(pointx, pointy, 8, 8);
                //   circle1.set_strokepen("1px solid red");
                //   circle1.set_fillstyle("yellow");
                //   circle1.set_id("GraphicsRect666");
                //   seriesGroup.addChild(circle1);
                //}

                ////test	
                //var circle2 = new nexacro.GraphicsEllipse(textX, textY, 8, 8);
                //circle2.set_strokepen("1px solid red");
                //circle2.set_fillstyle("red");
                //circle2.set_id("GraphicsEllipse_Test22");
                //seriesGroup.addChild(circle2);

                ////test	
                //if (guideline)
                //{
                //   var circle3 = new nexacro.GraphicsEllipse(rotateX, rotateY, 8, 8);
                //   circle3.set_strokepen("1px solid black");
                //   circle3.set_fillstyle("green");
                //   circle3.set_id("GraphicsEllipse_Test33");
                //   seriesGroup.addChild(circle3);	
                //}
            }
        }
    };
