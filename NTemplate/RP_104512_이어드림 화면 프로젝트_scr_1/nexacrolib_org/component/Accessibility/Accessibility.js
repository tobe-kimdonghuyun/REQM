            ...
            if (comp._has_inputElement || comp._input_element)
            {
                if (!comp.password)
                {
                    ...
                    else
                    {
                        ...
                        {
                            //if (comp._is_compound)
                            //    strAdditionalLabel = (comp.text) ? comp.text : comp.displaynulltext ? comp.displaynulltext : "";
                            //else
                            //{
                                //strAdditionalLabel = (comp._primitivevalue) ? comp._makeCalendarText(comp._primitivevalue) : "";
                                strAdditionalLabel += " " + comp._on_getAccessibilityAdditionalLabel();
...
nexacro._AccessibilityUtil.supportMobileApplicationAccessibility = function (container_handle, bForce)
{
    if (nexacro._accessibilitytype == 4)
    {
        ...
        if (container_element && !bForce)
        {
            ...
            {
				if (comp.accessibilityrole != "spin" && comp.accessibilityrole != "grid" && comp.accessibilityrole != "treegrid" && comp.accessibilityrole != "radio" && comp.accessibilityrole != "listbox" && comp.accessibilityrole != "textbox" && comp.accessibilityrole != "edit" && comp.accessibilityrole != "webbrowser" && comp.accessibilityrole != "webview" && comp.accessibilityrole != "form" && comp.accessibilityrole != "fileupload")
                {
...
if (nexacro.Component)
{
    ...
    _pComponent._setAccessibilityControls = function (id_list, subcomp)
    {
        var control_elem = this._control_element;
        if (control_elem)
            control_elem.setElementAccessibilityControls(subcomp ? subcomp : this, id_list);
    };

    _pComponent._setAccessibilityOwns = function (id_list, subcomp)
    {
        var control_elem = this._control_element;
        if (control_elem)
            control_elem.setElementAccessibilityOwns(subcomp ? subcomp : this, id_list);
    };
    ...
    _pComponent._setAccessibilityStatFlag = function (status, userstatus)
    {
        ...
        if (userstatus == "selected" && status != "focused")
        {
            if (nexacro._accessibilitytype == 4 || nexacro._accessibilitytype == 5)
            {
                ...
                {  
                    this._apply_setfocus(true);   //ie 에서는 항상 방향키로 변경시 focus를 줘야한다.
                }
            }
        }
    ...
    {
        //trace("accessibility focus in : ", this.name, refer_comp, related_comp);                
        this._on_focus(true, "");
    ...
    {
        //trace("accessibility focus out : ", this.name, refer_comp, related_comp);
        //trace(this._status, refer_comp.name, refer_comp._type_name);
    ...
    _pComponent._accessibility_keyaction = function (elem, refer_comp, keycode, alt_key, ctrl_key, shift_key, meta_key)
    {
