...
/**
* @class 법정공휴일 구하기 <br>
* @param {Object} nResultDt - {minDate: "YYYYMMDD", maxDate: "YYYYMMDD"} 날짜 범위
* @return {Array} 휴일정보
* @example this.gfnGetHolidaysInRange({minDate: "20240101", maxDate: "20240131"});
*/
pForm.gfnGetHolidaysInRange = function(nResultDt) {
    var aHoliday = [];
    var bLeapYear = false;
    var nYear = parseInt(nResultDt.minDate.substring(0, 4));
    ...
    // aAllHolidays 배열에서 해당 날짜가 이미 존재하는지 확인
    function addHoliday(date, label) {
        var holiday = date + label;
        if (!aAllHolidays.some(item => item.substring(0, 8) === date)) {
            aAllHolidays.push(holiday); // 존재하지 않으면 배열에 공휴일 추가
        }
    }

	aAllHolidays[0] = this.gfnLunarToSolar(nYear + "0101", bLeapYear) + "설날";		// 설 당일
	aAllHolidays[1] = this.gfnAddDate(aAllHolidays[0].substring(0, 8), -1) + "설날";	// 설 전일
	aAllHolidays[2] = this.gfnAddDate(aAllHolidays[0].substring(0, 8), 1) + "설날";		// 설 다음날
	// 석가탄신일
	aAllHolidays[3] = this.gfnLunarToSolar(nYear + "0408", bLeapYear) + "석가탄신일";
	// 추석
	aAllHolidays[4] = this.gfnLunarToSolar(nYear + "0815", bLeapYear) + "추석";		// 추석 당일
	aAllHolidays[5] = this.gfnAddDate(aAllHolidays[4].substring(0, 8), -1) + "추석";	// 추석 전일
	aAllHolidays[6] = this.gfnAddDate(aAllHolidays[4].substring(0, 8), 1) + "추석";		// 추석 다음날

	/////// 양력 체크
	aAllHolidays[7] = nYear+"0101" + "신정";
	aAllHolidays[8] = nYear+"0301" + "삼일절";
	aAllHolidays[9] = nYear+"0505" + "어린이날";	
	aAllHolidays[10] = nYear+"0606" + "현충일";		
	aAllHolidays[11] = nYear+"0815" + "광복절";	
	aAllHolidays[12] = nYear +"1003" + "개천절";
	aAllHolidays[13] = nYear +"1009" + "한글날";
	aAllHolidays[14] = nYear+"1225" + "성탄절";	

    // 다음 해 공휴일 포함 (캘린더 종료일이 다음 해일 수 있음)
    var nNextYear = nYear + 1;
    var bNextLeapYear = (nNextYear % 4 === 0 && nNextYear % 100 !== 0 || nNextYear % 400 === 0);

    // 다음 해 설날
    var nextYearLunarNewYear = this.gfnLunarToSolar(nNextYear + "0101", bNextLeapYear);
    addHoliday(nextYearLunarNewYear, "설날");
    addHoliday(this.gfnAddDate(nextYearLunarNewYear, -1), "설날");
    addHoliday(this.gfnAddDate(nextYearLunarNewYear, 1), "설날");

    // 다음 해 신정
    addHoliday(nNextYear + "0101", "신정");

    // 날짜 범위 필터링
    for (var i = 0; i < aAllHolidays.length; i++) {
        var holidayDate = aAllHolidays[i].substring(0, 8); // 공휴일 날짜 추출
        if (holidayDate >= nResultDt.minDate && holidayDate <= nResultDt.maxDate) {
            aHoliday.push(aAllHolidays[i]);
        }
    }
...
/**
* @class 공휴일 데이터 생성
* @return {Number} 시작날짜 ~ 종료날짜
* @example this.gfnCreateHolidayData(this.fvDate.substr(0, 4));
*/
pForm.gfnCreateHolidayData = function(nResultDt) 
{
	// 조회조건 데이터셋
	if (this.gfnIsNull(this.dsHoliDayCond))
	{
		var objCondDs = new Dataset();
		this.addChild("dsHoliDayCond", objCondDs);
		
		objCondDs.addColumn("startDate", "string");
		objCondDs.addColumn("endDate", "string");
		objCondDs.addColumn("USED_YN", "string");
		objCondDs.addRow();
	};
	
	// 공휴일 목록 데이터셋
	if (this.gfnIsNull(this.dsHoliDayDate))
	{
		var objListDs = new Dataset();
		this.addChild("dsHoliDayDate", objListDs);
		
		objListDs.addColumn("HOLYDAY_DT", "string");
		objListDs.addColumn("SCD_NM", "string");
	}
	
	// 조회리스트 클리어
	this.dsHoliDayDate.clearData();
	
	this.dsHoliDayCond.setColumn(0, "startDate", nResultDt.minDate); // 캘린더 시작날짜 세팅
	this.dsHoliDayCond.setColumn(0, "endDate", nResultDt.maxDate); // 캘린더 종료날짜 세팅
	this.dsHoliDayCond.setColumn(0, "USED_YN", 1); // 1 : 사용
	
	
	var strSvcId    = "searchHoliDayList";
	var strSvcUrl   = "searchHoliDayList.do";
	var inData      = "dsCond=dsHoliDayCond";
	var outData     = "dsHoliDayDate=dsList";
	var strArg      = "";	
