...
* var strArg    = "";
* this.gfnTransaction("save", strSvcUrl, inData, outData, strArg, "fnCallback", true, "N", true);
*/
pForm.gfnTransaction = function(strSvcId, strSvcUrl, inData, outData, strArg, callBackFnc, isAsync, sSilent)
{
	...
	// 그리드 rowposition 설정
	// 전체 컴포넌트 중 ID가 grd로 시작하는 컴포넌트 목록 배열 생성
	var arrComp = this.components._idArray.filter(item => item.startsWith("grd"));
	
	// User Property를 체크하여 현재 row 를 form 변수로 저장
	if (this.gfnIsNotNull(arrComp)) this._gfnGridSetUserPorperties(arrComp);
	...
	// session 체크 관련 cookie 추가
	//var strUserId = nexacro.getApplication().gdsUser.getColumn(0, "USER_ID");
	//nexacro.setCookieVariable("cUserId", strUserId);
...
pForm.gfnCallback = function (svcID, errorCode, errorMsg)
{
	...
	// 그리드 rowposition 설정
	if (this.gfnIsNotNull(this._fvRowposition))
	{
		var arrItem = this._fvRowposition.split(":");
		if (this.gfnIsNotNull(arrItem))
		{
			this.all[arrItem[0]].set_rowposition(arrItem[1]);
			
			if (arrItem[2] == "N") this._fvRowposition = "";
		}
	}
	...
	var evQuickView = nexacro.getEnvironmentVariable("evQuickView"); //2024.10.07 메인프레임일때만 서비스에러메세지 띄우기

	// 에러 공통 처리
	if(errorCode != 0)
	{
		// 초기 로그인 화면 등 arguments 객체가 없을경우 예외처리
		if(this.getOwnerFrame().arguments) {
			...
			objApp.gdsUserLog.setColumn(nRow, "LOG_TYPE"	, "T");
			objApp.gdsUserLog.setColumn(nRow, "ALOG_DTTM"	, objDate);
			objApp.gdsUserLog.setColumn(nRow, "MENU_ID"		, this.gfnNvl( this.getOwnerFrame().arguments[this.FRAME_MENUCOLUMNS.menuId], "Frame"));
			objApp.gdsUserLog.setColumn(nRow, "MENU_NM"		, this.gfnNvl(this.getOwnerFrame().arguments[this.FRAME_MENUCOLUMNS.menuNm], "Frame"));
			objApp.gdsUserLog.setColumn(nRow, "PRGM_ID"		, this.gfnNvl(this.getOwnerFrame().arguments[this.FRAME_MENUCOLUMNS.prgmId], "Frame"));
			objApp.gdsUserLog.setColumn(nRow, "PRGM_PATH"	, this.gfnNvl(this.getOwnerFrame().arguments[this.FRAME_MENUCOLUMNS.menuUrl], "Frame"));
			objApp.gdsUserLog.setColumn(nRow, "ERR_TYPE"	, "ServerError");
			objApp.gdsUserLog.setColumn(nRow, "ERR_MSG" 	, "DevPro D \n" +"서비스 아이디:"+objSvcID.svcId+ "\n서비스URL : "+objSvcID.svcUrl+"\n"+ errorMsg);
		}
		
		switch(errorCode)
		{
			case -1 :
				if (objSvcID.silent != "Y" && evQuickView =="N") //2024.10.07 메인프레임일때만 서비스에러메세지 띄우기
				{				
					// 서버 오류입니다.\n관리자에게 문의하세요.
...
// 				}
			...
			case -100 : 
				...
				{	
					...
					if(nStart > 0) 
					{
						errorMsg = errorMsg.substr(nStart, 9);
					}
					
					// 데이터베이스 오류입니다. \n에러코드 : {0}.
					if(objApp.gdsMessage.findRow("MSGE_CD", "msg.database.error") > 0) this.gfnAlert("msg.database.error", [errorMsg]);
					else this.gfnAlert("msg.err.tran", [errorMsg]);
				}
				return;
				break;
			case -999 :
	...
	// 그리드 필터 초기화
	var nLength = this.all.length;
	for (var i = 0; i < nLength; i++) {
		var objComp = this.all[i];
		if (objComp instanceof Grid) this.gfnClearUserFunction(objComp);
	}
