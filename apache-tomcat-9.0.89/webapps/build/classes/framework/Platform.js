...
//                    return ret;
    ...
    */
    ...
    __pLayoutManager.addLayoutList = function (objContainer, objLayout)
    {
        ...
        if (layout_list)
        {
            var item = layout_list.get_item(objLayout.name);
            if (item && objLayout.width < 11) //timing problem //DesignForm
                return;
    ...
    __pLayoutManager.selectLayout = function (objContainer)
    {
        ...
        if (container_info)
        {
            ...
            if (cur_layout != new_layout)
            {
                ...
                var is_defaultload = old_layoutname == "" && new_layout.name == "default";
                old_layoutname = old_layoutname == "" ? "default" : old_layoutname;
                var ret;
                if (!is_defaultload)
                    ret = objContainer.on_fire_canlayoutchange(objContainer, "canlayoutchange", old_layoutname, new_layout.name, old_layoutwidth, new_layout.width, old_layoutheight, new_layout.height);
    ...
    __pMultiLayoutManager.getValidLayoutObject = function (objContainerInfo, container_width, container_height)
    {
        ...
        for (var i = 0; i < layout_len; i++)
        {
            ...
            var screenid_list = layout.screenid.split(",");
            if (layout.screenid != "" && screenid_list.indexOf(curscreen_id) < 0)
            {
                continue;
            }
        ...
        switch (arrange_type)
        {
            case 0: // CSS Native
                for (var i = 0; i < contents.length; i++)
                {
                    ...
                    if (target_elem)
                    {
                        target_elem.setElementSize(content._adjust_pos[2], content._adjust_pos[3]);
   ...
   // 활성화된 Layer에 속한 컴포넌트인지 판단한다.
    ...
    __pWindow._on_default_sys_lbuttonup = function (elem, strButton, altKey, ctrlKey, shiftKey, windowX, windowY, screenX, screenY, offsetX, offsetY, metaKey)
    {
        ...
        this._cur_evt_type = null; //clear evttype at lbuttonup.
                ...
                if (this.frame && this.frame._is_click_openstatus === false)
                {
                    ...
                    else if (this.frame._state_openstatus == 3)
                    {
                        if (wparam === 0) // max->normal;
                        {
                            ...
                            if (this.frame._window_type == -1 || this.frame._window_type == 2 || this.frame._window_type == 5 )
...
//            resize_func = null;
       ...
       // Google Chrome에서 activate 이벤트가 2번씩 나오는 문제로 validation한다.
        if (this._is_active_window !== true)
        {
            ...
            if (frame)
            {
                ...
                if (this._last_focused_elem == null)
                {
                    var comp;
                    ...
                    if (app)						
                    {
                        ...
                        if (active_form)
                        {
                            comp = active_form._getTabOrderFirst();
                            if (comp)
                            {
                                comp._setFocus(true); // check : sensereader 환경에서 taborderfirst로 이동하지 않음
                                this._last_focused_elem = comp._control_element;
                            }
                        }
                    }
                }
    ...
    __pWindow._on_default_sys_deactivate = function ()
    {
        ...
        nexacro._isEmbeddedWindow(this);
    };
    ...
    __pWindow._on_default_sys_touchstart = function (elem, touchid, clientX, clientY, screenX, screenY, curtime, orgtime, is_last_changedtouch, first_touchid)
    {
        ...
        this._is_fire_sys_touchstart = true;
    ...
    __pWindow._on_default_sys_touchend = function (elem, touchid, clientX, clientY, screenX, screenY, curtime, orgtime, is_last_changedtouch)
	{
        ...
        // accessibility init flag
        this._is_fire_sys_touchstart = false;
        this._is_fire_sys_focusin = false;
...
*/
    ...
    */
    ...
    _pTouchGestureManager.checkPreventDefault = function (elem, type)
    {
        ...
        */
        ...
        if (linkedcontrol && linkedcontrol._refform)
        {
            ...
            else if (linkedcontrol._refform.form && linkedcontrol._refform.form.getFocus)
            {
                focused_comp = linkedcontrol._refform.form.getFocus(); 
            }
        }
            ...
            switch (type)
            {
                ...
                case 2: // touchend
                    ...
                    if (focused_comp_type == "edit")
                    {
                        ...
                        {
                            return nexacro._is_keypad_show ? false : true;                         
                        }
                        else
                            return true;                        
                    }
    ...
    _pEnvironment.accessibilitycontentsearchkey = true; // component searching by nexacro
    ...
    _pEnvironment.multivalueseparator = "";
    _pEnvironment.usedateautocalibration = true;
    ...
    nexacro._usedateautocalibration = true;  
    ...
    _pEnvironment.set_usedateautocalibration = function (v)
    {
        v = nexacro._toBoolean(v);
        if (nexacro._usedateautocalibration != v || this.usedateautocalibration != v)
        {
            this.usedateautocalibration = v;
            nexacro._usedateautocalibration = v;
        }
    };
    ...
    {
        this.accessibilitycontentsearchkey = v;
    ...
    nexacro.loadStyle = function (url)
    {
        ...
        var idx = nexacro._cssurls.indexOf(url)
        if (idx != -1)
            nexacro._cssurls.splice(idx, 1)
        nexacro._cssurls.push(url);
