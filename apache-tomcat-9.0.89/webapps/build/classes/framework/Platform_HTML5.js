...
if (nexacro._Browser != "Runtime" && !nexacro._init_platform_HTML5)
{
	...
	nexacro._useStreamDataReader = false; // IE11 ArrayBuffer flag
	...
	if (nexacro._Browser == "IE" || nexacro._Browser == "Edge" && nexacro._BrowserType == "Edge")
	{
		nexacro._createSysEvent_ForwardFuncs = function (_cur_win)
		{
			...
			_cur_win._touch_list = new nexacro.Collection();	// IE는 IE10 이상 버젼부터 Touch를 지원함.
		...
		nexacro._createSysEvent_ForwardFuncs = function (_cur_win)
		{
			...
			_cur_win._syshandler_onfocusin_forward = function (evt) // using focus in
			{
				if (nexacro._enableaccessibility && nexacro._accessibilitytype < 4)
				{
					...
					// 모바일웹은 touch 이벤트 발생 없이 발생한 focusin을 처리 
					// 데스크탑웹은 keydown 이벤트 발생 없이 발생한 focusin 처리
					var win = nexacro._findWindow(_cur_win.nexacro_HTMLSysEvent._win_win);					
					if (win && (nexacro._accessibilitytype > 3 && win._is_fire_sys_touchstart === false)
						|| (nexacro._accessibilitytype < 4 && win._is_fire_sys_keydown === false))
					{
			...
			_cur_win._syshandler_onorientationchange_forward = function (evt)
			{
				...
				if (reset_viewport)
				{
					...
					{
						var win = nexacro._findWindow(this);
						_tester.swap_screen_timer = setInterval(function ()
						{
							...
							var s_h = nexacro._getScreenHeight();
							...
							if (is_changed && nexacro._BrowserExtra == "SamsungBrowser" && nexacro._OSVersion > 9)
							{
								win._on_sys_resize(s_w, s_h);
							}
						}, 100);
					}
					else
	...
	nexacro._createPopupFrame = function (_cur_win, urlparams)
	{
		...
		var init_info = nexacro._getLocalStorage("initinfo", 2);
		if (init_info)
		{
			try
			{
				nexacro._init_info = JSON.parse(init_info);   
			}
			catch(e)
			{
				nexacro._settracemsg(e);
	...
	if (nexacro._Browser == "IE" || (nexacro._Browser == "Edge" && nexacro._BrowserType == "Edge"))
	{
	    ...
	    nexacro._syshandler_onmousedown = function (_sysEvent, node, evt)
		{
			...
			if (evt.button == nexacro_HTMLSysEvent.MOUSE_LBUTTON)
			{
				...
				// Windows XP, Window7 IE에서 form에 lbuttondown시 조합이 완료됨.
				...
				{
					if (!(elem.isInputElement() && elem.enable) && !(/*last_focused_elem instanceof nexacro._WebBrowserPluginElement*/nexacro._isWebTypeElement(last_focused_elem)))
						nexacro._stopSysEvent(evt);
				}
			...
			else // middlebutton은 4로 인식함
		    {
		        ...
		        // Window7 IE에서 form에 mbuttondown시 조합이 완료됨.
		...
		nexacro._syshandler_onmousewheel = function (_sysEvent, node, evt)
		{
			...
			ret = win._on_sys_mousewheel(elem, nexacro.__getWheelDeltaX(evt), nexacro.__getWheelDeltaY(evt), nexacro._getSysEventBtnString({ button: 4, which: 2 }), evt.altKey, evt.ctrlKey, evt.shiftKey, evt.clientX, evt.clientY, evt.screenX, evt.screenY, evt.offsetX, evt.offsetY, evt.metaKey);
...
*/
	...
	nexacro._syshandler_ondblclick = function(_sysEvent, node, evt)
	{
		...
		{
		    _sysEvent._cur_win.__clearGC();
		    var ret = win._on_sys_dblclick(elem, nexacro._getSysEventBtnString({ button: 1, which: 1 }), evt.altKey, evt.ctrlKey, evt.shiftKey, evt.clientX, evt.clientY, evt.screenX, evt.screenY, evt.offsetX, evt.offsetY, evt.metaKey);
        ...
        {           
			...
			if (w != win.clientWidth || h != win.clientHeight)
			{
				...
				if (is_active || is_web_elem)
				{
					...
					{
						is_keypad_switch = (is_input_focused || is_web_elem) && (w == win.clientWidth && (Math.abs(h - win.clientHeight) < addressbar_height ? true : false));
						if (nexacro._BrowserExtra == "NaverBrowser" && w == win.clientWidth && h != win.clientHeight && h >= win._previous_client_height) is_keypad_switch = true;
					...
					if (is_keypad_switch)
					{
						...
						if (nexacro._OS == "Android")
						{					
							var view_height = window.visualViewport ? parseInt(window.visualViewport.height) : h;
							nexacro._is_keypad_show = (is_input_focused) && (w == win.clientWidth && win.clientHeight - view_height > 200);

							if (!nexacro._is_keypad_show)
							{
								var heightDelta = Math.abs(win.clientHeight - view_height);
								var systemUIChanged = heightDelta > 20 && heightDelta < 100;
								if (systemUIChanged)
								{
									var force = true;
									_sysEvent._cur_win.__clearGC();
									ret = win._on_sys_resize(w, view_height, force);
								}
							}
						}
			...
			nexacro._checkInformation(nexacro._init_info, win);
			win._previous_client_height = h;
        }
    ...
    nexacro._createWindowHandle = function (parent_win, target_win, name, left, top, width, height, resizable, layered, taskbar, is_main)
    {
        ...
        if (nexacro._allow_default_pinchzoom)
        {
			...
			if (nexacro._OS == "iOS")
				nexacro.__setDOMStyle_overscrollBehavior(nexacro._getWindowDocumentHandle(_win_handle).documentElement.style, "none");
		}    
	...
	nexacro._setLocalStorageforOpen = function (id, parentwin, parentframe, frameopener, arrarg)
	{
		...
		var init_info = nexacro._init_info;
		if (init_info)
		{
			try
			{
				nexacro._setLocalStorage("initinfo", JSON.stringify(init_info), 2);
			}
			catch(e)
			{
				nexacro._settracemsg(e);
    ...
    if ((nexacro._Browser == "IE" && nexacro._BrowserVersion <= 8) ||
        (nexacro._OS == "iOS" && parseFloat(nexacro._OSVersion) >= 8))
    {
	...
	nexacro._isEmbeddedWindow = function (win)
	{
		...
		if (_win_handle)
		{
            if (_win_handle.parent && _win_handle.document)
			{
		...
		if (nexacro._BrowserExtra == "SamsungBrowser" || (nexacro._OS == "Android" && nexacro._Browser == "Chrome"))
	    {
			...
			if (_win)
			{
				...
				if (orientation_info && (orientation_info.type.indexOf("portrait") > -1))
				{
					...
					{
						adjust_screen_height = Math.round(_doc_elem.clientHeight);
					}					
				}
