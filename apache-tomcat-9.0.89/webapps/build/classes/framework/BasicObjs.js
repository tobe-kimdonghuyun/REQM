...
if (!nexacro.Decimal)
{
    ...
    nexacro.Decimal._opt_digits = 31;
    ...
    _pDecimal._toString = function (precision)
    {
        if (this.isZero())
        {
            return "0";
        ...
        var opt_precision = precision | 0;
        if (opt_precision < 1 || opt_precision > 100)
            opt_precision = nexacro.Decimal._opt_precision;
		
        var offset = (dec.hi | 0) + 1;
        var digitlen = opt_precision + offset;
		digitlen = Math.min(nexacro.Decimal._opt_digits, digitlen);

        var _cvt = this._getCVT(digitlen);
    ...
    _pDecimal._getCVT = function (precision)
    {
        var sign = 0;
        ...
        var exp = Math.floor(Math.log(dAbs) * Math.LOG10E) | 0; /*  ) | 0;*/

        /*
        if (exp < 0) 
        {
            off = (dAbs >= nexacro.Decimal._fraction_10_n[Math.abs(exp + 1)] ? 2 : 1);
        }
        else 
        {
            off = (dAbs >= nexacro.Decimal._fraction_10[exp + 1] ? 2 : 1);
        }
        */
