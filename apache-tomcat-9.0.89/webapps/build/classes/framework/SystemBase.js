...
if (!nexacro._Init_systembase)
{
    ...
    else
    {
        ...
        else if (navigator.userAgent.match(/Edge/))
        {
            ...
            var ret = /Edge\/([\.\d]+)/.test(navigator.userAgent);
            nexacro._BrowserVersion = parseInt(RegExp.$1) | 0;
            ret = null;
		}
    ...
    nexacro.DevicePermissionType =
    {
        ...
        NOTIFICATIONS: "notifications", // Android: NOTIFICATIONS(POST_NOTIFICATIONS)
    ...
    nexacro._inputelement_user_override_func = ["on_apply_ime_environment_process", "on_apply_force_imeSet", "on_updateEvtOrder_process", "setCompositionComplete_process", "setCompositionCancel_process",
        ...
        "on_sys_beforeinput_forward_before_process", "on_sys_beforeinput_forward_process",
        "on_sys_propertychange_before_process", "on_sys_propertychange_process",
        "on_sys_keyinput_before_process", "on_sys_keyinput_process", "on_sys_compositionstart_process", "on_sys_compositionupdate_process", "on_sys_compositionend_process",
        ...
        nexacro._executeGlobalEvalStr = function (evalstr, url)
        {
            try
            {
                ...
                */
                ...
                if (url)
                {
                    var reg_pre = /^\(function\(\)\s*\{\s*return\s*(?=function)/;
                    var reg_post = /\}\s*\)\s*\(\s*\)\s*\;\s*$/;

                    var pre = evalstr.match(reg_pre);
                    if (pre)
                    {
                        var post = evalstr.match(reg_post);
                        if (post)
                        {
...
//			return window.btoa(input);
    ...
    else
    {
        (function ()
        {
            ...
            // RP 98783 avest 바이러스프로그램이 바이러스로 검출하는 내용 수정(deploy시 문제 됨. 수정하면 안됨)
            var val2 = 2, val3 = 3, val4 = 4, val6 = 6, val15 = 15, val63 = 63;
    ...
    (function ()
    {
        ...
        //RP 98783 avest 바이러스프로그램이 바이러스로 검출하는 내용 수정(deploy시 문제 됨. 수정하면 안됨)
        var val1 = 1, val2 = 2, val3 = 3, val4 = 4, val5 = 5, val6 = 6, val7 = 7;
    ...
    nexacro._CommunicationStatusTable =
   {
    ...
    __pCommunicationItem.on_decrypt_arraybuffer = function (data)
    {
        ...
        if (this._protocol < 0)
        {
            ...
            if (protocoladp)
            {
                var tempdata = ab2str(data.buffer);
                var strdata = protocoladp.decrypt(this.path, tempdata);
                if(strdata != tempdata)
                    data.buffer = str2ab(strdata);
            }
        }
        return data;
    };
...
//            bcache = false;
    ...
    nexacro._loadDataObject = function (path, target, handler, service, form, svcid, method, outdataobj, parameters, async, datatype, compress)
    {
        ...
        service.cachelevel = servicecachelevel; // restore
    ...
    nexacro.__startRESTAPICommunication = function (loadItem, path, cachelevel, async, referer, senddata, ndatatype, compress, ver, failpass, service)
    {
        function getRESTAPI()
        {
            ...
            if (use_fakexhr)
            {
                ...
                // [24/04/02][RP 99613] - 여기서 destory 해줘야. FakeXMLHttpRequest에서 release됨.
    ...
    nexacro._accessibilitycontentsearchkey = true;
...
//        var checkversion = env.checkversion;
    ...
    nexacro._checkShowContextMenu = function (comp)
    {
        ...
        else
        {
            ...
            {
                if (bEdit || bSketch) return comp.usecontextmenu;
            }
        }
