...
if (!nexacro.Form)
{
    ...
    _pFormBase._on_init = function ()
    {
        ...
        //for autosize
        if (this.parent && this.parent.form == this && !this.parent._is_autosized)
        {
        ...
        if (comps)
        {
            ...
            for (var i = 0; i < comp_len; i++)
            {
                ...
                if (!(filter_type & 1))
                {
                    if (bAccessibility)
                    {
                        if (!comp.accessibilityenable || !comp.visible)
                           continue;
                    }
                ...
                else if ((tabkeycirculation == 2 || tabkeycirculation == 3) && this.parent && this.parent._is_frameset)
                {
                    ...
                    for (var i = 0; i < n; i++)
                    {
                        if (_frames[i] instanceof nexacro.ChildFrame)
                        {
                            ...
                            else if (_idx != -1)
                            {
                                ...
                                {
                                    if (!(this.parent instanceof nexacro.FrameSet) ||
                                        (this.parent instanceof nexacro.FrameSet &&
                                            ((_tabkeycirculation == 3 && this.parent._arrange != 0) ||  // auto, cascade일때는 skip
                                    ...
                                    {
                                        if (_frames[i].form && _frames[i].formurl)
                                        {
                    ...
                    if (!_childframe)
                    {
                        ...
                        if (_frameset_idx != -1)
                        {
                            ...
                            if (_childframe)
                            {
                                next = _childframe.form ? _childframe.form._getTabOrderFirst(filter_type) : null;
                                if (!next)
                                    next = _childframe.form;
                            }
                            else
                            {
                                ...
                                if (_next_frame_)
                                {
                                    ...
                                    {
                                        next = _next_frame_.form ? _next_frame_.form._getTabOrderFirst(filter_type) : null;
                                        if (!next)
                                            next = _next_frame_.form;
                                    }
                                    else
                        ...
                        else
                        {
                            ...
                            {
                                // this.parent.parent는 MainFrame
                                // 가장 끝에 있는 ChildFrame구해와야 한다.   
                                if (tabkeycirculation == 2) 
                                {
                                    // cycle                                    
                                    if ((_tabkeycirculation == 1 && this.parent._state_openstatus == 2)  // minimize
                                        || (_tabkeycirculation == 2 && this.parent._activate) // active_only
                                        || (_tabkeycirculation == 3 && this.parent._arrange == 0)) // auto, cascade일때
                                    {
                                        next = this.form ? this.form._getTabOrderFirst(filter_type) : null;
                                        //if (!next)
                                        //    next = _next_frame_.form;
                                    }
                                    else
            ...
            if (!ret)
            {
                ...
                if (!next)
                {
                    ...
                    else
                    {
                        // if (this._checkContainerTabFocus() == true)
                        //    ret = [this];
                        if ((tabkeycirculation == 2 || tabkeycirculation == 3) && this.parent && this.parent._is_frameset)
                        {
                            // this.parent = FrameSet
                            var _tabkeycirculation = this.parent._tabkeycirculation;
                            var _frames = this.parent.frames;
                            ...
                            if (!_childframe)
                            {
                                ...
                                if (_frameset_idx != -1)
                                {
                                    ...
                                    if (_childframe)
                                    {
                                        next = _childframe.form ? _childframe.form._getTabOrderLast(filter_type) : null;
                                        if (!next)
                                            next = _childframe.form;
                                    }
                                }
                                    ...
                                    if (_prev_frame_)
                                    {
                                        if (_prev_frame_ instanceof nexacro.ChildFrame)
                                        {
                                            if (tabkeycirculation == 3)
                                            {
                                                ...
                                                {
                                                    next = _prev_frame_.form ? _prev_frame_.form._getTabOrderFirst(filter_type) : null;
                                                    if (!next)
                                                        next = _prev_frame_.form;

                                                }
                                                else
                                                    next = _prev_frame_.form ? _prev_frame_.form._getTabOrderLast(filter_type) : null;
                                            }
                                            else
                                            {
                                                next = _prev_frame_.form ? _prev_frame_.form._getTabOrderLast(filter_type) : null;
    ...
    _pFormBase._on_prepare_stepcontents = function (stepcount, stepindex)
    {
        ...
        if (control_elem)
        {
            if (this._is_created)
            {
                ...
                var step_ctrl_cnt = step_ctrl ? step_ctrl.stepcount : 0;
    ...
    _pFormBase._on_refresh_stepcontents = function (stepcount, stepindex)
    {
            ...
            if (this._is_created)
            {
                ...
                if (step_ctrl_cnt > 0 || stepcount > 0)
                {
                    ...
                    for (var i = 0; i < comp_len; i++)
                    {
                        ...
                        {
                            if (!comps[i]._is_created || comps[i].positionstep >= step_ctrl_cnt)
                            {
                                comps[i].on_created();
                            }
                            var comp_elem = comps[i].getElement();
        ...
        {            
            if (this.stepselector.stepcount != stepcount)
            {
                this.stepselector.set_stepcount(stepcount);
                this.stepselector.set_stepindex(stepindex);
            }
        }
    };
    ...
    _pForm.onCreatedAfter = function (resultCreated)
    {
        if (resultCreated)
        {                        
            if (this.stepselector && !this.stepselector.getElement())
            {
                this.stepselector.createComponent(true);
                this.stepselector.on_created();
                this.on_apply_stepshowtype(this.stepshowtype);
            }
    ...
    _pForm._changeControlElement = function (parent_elem)
    {
        if (parent_elem && parent_elem.handle)
        {
            ...
            this._control_element.parent = this;
            return true;
        }
        return false;
    };
    ...
    _pForm.on_fire_user_onkeydown = function (keycode, alt_key, ctrl_key, shift_key, fire_comp, refer_comp, meta_key)
    {
        ...
        if (!this.onkeydown || (this.onkeydown && !this.onkeydown.defaultprevented))
        {
            ...
            if (this._isComponentKeydownAction() && (keycode == nexacro.Event.KEY_LEFT || keycode == nexacro.Event.KEY_RIGHT))
            {
                ...
                if (hscrollbar && hscrollbar.visible && (ctrl_key == true || !nexacro._accessibilitycontentsearchkey))
                {
            ...
            else if (this._isComponentKeydownAction() && (keycode == nexacro.Event.KEY_UP || keycode == nexacro.Event.KEY_DOWN))
            {
                ...
                if (vscrollbar && vscrollbar.visible && (ctrl_key == true || !nexacro._accessibilitycontentsearchkey))
                {
                    if (!dlgc.want_arrows) // arrowkey를 직접 처리하는 컴포넌트는 제외
                    {
    ...
    _pForm.setStepCount = function (cnt)
    {
        ...
        if (this.stepselector && layout)
        {
            ...
            layout.setLayoutProperty(this, layout_name, "stepcount", this.stepselector.stepcount);
        }
    };
            ...
            if (this._is_scrollable && this._control_element)
            {
                ...
                for (var i = 0, n = comps.length; i < n; i++)
                {
                    ...
                    if (comp && comp.visible)
                    {
                        ...
                        if (comp.minwidth || comp._right < 0 || !comp.right && (comp.width > 0) || (nexacro._isString(comp.width) && comp.width.indexOf("%") < 0))
                            _c_w = Math.max(_c_w, offsetright);
                        if (comp.minheight || comp._bottom < 0 || !comp.bottm && (comp.height > 0) || (nexacro._isString(comp.height) && comp.height.indexOf("%") < 0))
    ...
    _pForm._setSize = function (width, height)
    {
        if (this._adjust_width != width || this._adjust_height != height)
        {
            if (this instanceof nexacro._InnerForm)
            {
                this.parent._calced_container = false;
            }
    ...
    _pForm._selectLayout = function ()
    {
        ...
        if (layout_manager)
        {
            layout_manager.selectLayout(this);
    ...
    if (!nexacro._InnerForm)
    {
        ...
        _pInnerForm._getScrollBarSize = function ()
        {
            ...
            if (this.parent && this.parent.formscrollbarsize != null)
            {
                scrollbarsize = this.parent.formscrollbarsize;
            }
        ...
        _pInnerForm._getScrollIndicatorSize = function ()
        {
            ...
            if (this.parent && this.parent.formscrollindicatorsize != null)
            {
                scrollindicatorsize = this.parent.formscrollindicatorsize;
            } 
        ...
        _pInnerForm._createVScrollBar = function (scroll_size)
        {
            ...
            var _parent = this.parent;
            var scrollbarbaroutsize = nexacro._isNull(_parent.formscrollbarbaroutsize) ? this.scrollbarbaroutsize : _parent.formscrollbarbaroutsize;
            if (!nexacro._isNull(scrollbarbaroutsize))
                vscrollbar.set_baroutsize(scrollbarbaroutsize);
            var scrollbarincbuttonsize = nexacro._isNull(_parent.formscrollbarincbuttonsize) ? this.scrollbarincbuttonsize : _parent.formscrollbarincbuttonsize;
            if (!nexacro._isNull(scrollbarincbuttonsize))
                vscrollbar.set_incbuttonsize(scrollbarincbuttonsize);
            var scrollbardecbuttonsize = nexacro._isNull(_parent.formscrollbardecbuttonsize) ? this.scrollbardecbuttonsize : _parent.formscrollbardecbuttonsize;
            if (!nexacro._isNull(scrollbardecbuttonsize))
                vscrollbar.set_decbuttonsize(scrollbardecbuttonsize);
            var scrollbarbarminsize = nexacro._isNull(_parent.formscrollbarbarminsize) ? this.scrollbarbarminsize : _parent.formscrollbarbarminsize;
            if (!nexacro._isNull(scrollbarbarminsize))
                vscrollbar.set_barminsize(scrollbarbarminsize);
            var scrollbartrackbarsize = nexacro._isNull(_parent.formscrollbartrackbarsize) ? this.scrollbartrackbarsize : _parent.formscrollbartrackbarsize;
        ...
        {            
            ...
            var v = this.scrollbartype ? this.scrollbartype : "";
        ...
        _pInnerForm.on_apply_scrolltype = function ()
        {
            ...
            var parent = this.parent;
            if (parent) 
                this._scrolltype = parent.formscrolltype ? parent.formscrolltype : v;
            var control_elem = this.getElement();
            if (control_elem)
            {
                var hscrollbarsize = this._getHScrollBarSize();
        ...
        {
            var scrollbarbarminsize = nexacro._isNull(this.parent.formscrollbarbarminsize) ? this.scrollbarbarminsize : this.parent.formscrollbarbarminsize;
        ...
        {
            var scrollbarbaroutsize = nexacro._isNull(this.parent.formscrollbarbaroutsize) ? this.scrollbarbaroutsize : this.parent.formscrollbarbaroutsize;
            var scrollbar = this.vscrollbar;
            if (scrollbar)
        ...
        {
            var scrollbardecbuttonsize = nexacro._isNull(this.parent.formscrollbardecbuttonsize) ? this.scrollbardecbuttonsize : this.parent.formscrollbardecbuttonsize;
            var scrollbar = this.vscrollbar;
            if (scrollbar)
        ...
        {
            var scrollbarincbuttonsize = nexacro._isNull(this.parent.formscrollbarincbuttonsize) ? this.scrollbarincbuttonsize : this.parent.formscrollbarincbuttonsize;
            var scrollbar = this.vscrollbar;
            if (scrollbar)
        ...
        {
            var scrollbartrackbarsize = nexacro._isNull(this.parent.formscrollbartrackbarsize) ? this.scrollbartrackbarsize : this.parent.formscrollbartrackbarsize;
            var scrollbar = this.vscrollbar;
            if (scrollbar)
