...
if (!nexacro.Frame)
{
    ...
    _pMainFrame.accessibilityrole = "frame";
    ...
    _pChildFrame.showModal = function (str_id, _parent_frame, arr_arg, opener, callback, is_async)
    {
        ...
        if (this.left == null)
        {
            ...
            {
                if (_window) this.left = _window.clientWidth / wheelZoomScale - this.width - this.right;
            }
        }
        if (this.top == null)
        {
        ...
        if (this.width == null)
        {
            ...
            {
                if (_window) this.width = _window.clientWidth / wheelZoomScale - this.left - this.right;
            }
        }
        if (this.height == null)
        {
            ...
            {
                if (_window) this.height = _window.clientHeight / wheelZoomScale - this.top - this.bottom;
            }
        }
        ...
        var recalculated_pos = this._recalcModalPosition((this.left == null) ? null : this.left, (this.top == null) ? null : this.top, this.width, this.height);
    ...
    _pChildFrame._showModalWindow = function (str_id, _parent_frame, arr_arg, opener, _lockmode)
    {
        ...
        */
        ...
        if (parent_frame && parent_frame._setModalLock) // parent_frame이 ChidFrame인 경우만 modallock 처리
        {
            this._setModalLock(this._overlaycolor);
        }
        var rv = this._window.createModal(parent_window, this.id, width, height, left, top, this.resizable, this.layered, lockmode);

        nexacro._SysOnceCallbackTimer.callonce(parent_window, function ()
            {
    ...
    _pChildFrame._createdForm = function ()
    {
        ...
        {   
            if (this._isNotProcessedWheelZoom == true)
            {
                ...
                this._isNotProcessedWheelZoom = false;
            }
        }
    ...
    _pFrameSetBase._getNextFrame = function (id, cycle)
    {
        ...
        for (var i = 0; i < n; i++)
        {
            ...
            {
                if (frames[i]._isFocusAcceptable() && frames[i].formurl)
                {
    ...
    _pFrameSetBase._getPrevFrame_ = function (id, cycle)
    {
        ...
        for (var i = n - 1; i >= 0; i--)
        {
            ...
            {
                if (frames[i].formurl && childframe_idx == -1 && frames[i]._isFocusAcceptable())
                {
                    childframe_idx = i;
                    frame = this._getFocusChildFrame(frames[i]);
                    break;
    ...
    _pFrameSet.tabkeycirculation = "all";    // all, (all,no_minimize), active_only, auto
