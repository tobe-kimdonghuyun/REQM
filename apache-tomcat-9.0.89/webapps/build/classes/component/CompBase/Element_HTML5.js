...
if (nexacro._Browser != "Runtime" && !nexacro.Element)
{
    ...
    */
    ...
    _pElement._refreshCommonStyleProps = function (handle_style)
    {
        ...
        if (!(control && control._is_frame && (control._is_main && !nexacro._allow_default_pinchzoom)))   //if it is not mainframe
        {
    ...
    _pElement._getCommonStyleStr = function ()
    {
        ...
        if (manager && control)
        {
            var layouttype = manager.getLayoutProperty(control._getForm(), "type");
            if (!control._isFluidContainerCalculation() && (layouttype == "horizontal" || layouttype == "vertical" || layouttype == "table"))
            {
                if (!control._is_subcontrol && this.visible)
    ...
    { // IE7,8,9
        ...
        __pEdgeImageElement.create = function (_doc)
        {
            ...
            if (owner_elem.handle && !this.handle)
            {
                ...
                this._on_createAccessibilityCommand(handle);
        ...
        __pEdgeImageElement._create_inner_command = function (edge_type, width, height, img_url, img_width, img_height, edgex, edgey)
        {
            ...
            var str = "";
            str += "<img " + nexacro.__getDOMNode_Alt("") + " class='nexacenter' ";
    ...
    */
    ...
    // kindlion :현재 IE9브라우저에서 한글 입력후 백스페이스 누르면 compositionend후 oninput이 발생 안함.
    // 그래서 아래 로직 필요    
    ...
    if ((nexacro._Browser == "IE" && nexacro._BrowserVersion == 9) ||
        (nexacro._Browser == "Edge" && nexacro._BrowserType == "Edge") || nexacro._OS == "iOS" ||
    ...
    else
    {
        _pInputElement._createElementHandle = function (owner_elem, _doc)
        {
            ...
            if (nexacro._OS == "iOS")
            {
                nexacro.__setDOMNode_Autocorrect(handle, "off");
    ...
    _pInputElement.setElementEnable = function (enable, color)
    {
        ...
        {
            this.enable = enable;
            this._disabled_color = enable ? null : color;
    ...
    _pInputElement.setElementFocus = function (trigger_type, self_flag)
    {
        ...
        if (input_handle)
        {
            ...
            {
                if (trigger_type == "focus" || trigger_type == "lbutton" || trigger_type == "lbuttondown" || trigger_type == "rbuttondown" ||
            ...
            else
            {
                ...
                else if (this.autoselect && nexacro._OS == "iOS")
                {
            ...
            if (this._is_focused)
            {
                if (this.displaynulltext)
                {
                    if ((nexacro._Browser == "IE" || nexacro._BrowserVersion == 11) && this._trigger_type)
                    {
                        ...
                        if (!v) return input_handle.defaultValue;
                    }
                }
    ...
    _pInputElement.on_sys_keydown_before_process = function (evt)
    {
        ...
        if ((nexacro._isTouchInteraction && nexacro._SupportTouch) || nexacro._isMobile())
        {
            ...
            if (stepselector)
            {
                ...
                if (__form instanceof nexacro._InnerForm && (__form.parent instanceof nexacro.Div || __form.parent instanceof nexacro.PopupDiv ||
                                                             __form.parent instanceof nexacro.Tabpage))
                {
                    var p_positionstep = __form.parent.positionstep;
                    if (__form.parent instanceof nexacro.Tabpage)
                        p_positionstep = __form.parent.parent.positionstep;

                    if (stepselector.stepindex != p_positionstep)
    ...
    _pInputElement._on_sys_keydown = function (evt)
    {
        ...
        if (input_handle)
        {
            ...
            if (!pThis.readonly)
            {
                ...
                if (!pThis.on_sys_keydown_specialkey_process.call(pThis, evt)) return;
        ...
        else
        {
            ...
            if (pos != -1 && composing_status != nexacro._CompositionState.END)
            {
                ...
                else
                {
                    ...
                    // IE11 한글 (prev/curr_status)
                    // 첫입력: 1/2, 두번째 입력: 1/2, 세번째 아스키입력: 3/0

                    // IE9 한글,일본어 (selection후 입력도 동일)
                    // 첫입력: 0/1, 두번째 입력: 2/2

                    // IE9 한글 입력후 숫자
                    // 첫입력: 0/1, 두번째 입력: 2/2 3/0
                    ...
                    // IE8 한글,일본어
                    // 첫입력 1/2, 두번째 입력: 2/2
                    // IE8 selection 일때(왜? 이런식의 이벤트가? 첫 이벤트는 selection값 지우고 두번째에 값을 넣는작업을 하는거 같다. IE8브라우저에서)
                    // 첫입력 1/2 2/2, 두번째 입력: 2/2

                    // IE8 한글 입력후 숫자
                    // 첫입력 1/2 두번째 입력: 2/3 0/0

                    // Edge 일어 입력
                    // 첫입력 1/2 두번째 입력: 2/2
                    // 값확정 2/3 (isComposing은 false)
                    ...
                    if (prev_status == nexacro._CompositionState.NONE && nexacro._CompositionState.START ||     // IE9/Safari(PC)
                        prev_status == nexacro._CompositionState.END && nexacro._CompositionState.START ||      // IE9/Safari(PC)
                        prev_status == nexacro._CompositionState.START && nexacro._CompositionState.COMPOSING ||
                        (pThis._check_imetype_composing_status && prev_status == nexacro._CompositionState.COMPOSING && composing_status == nexacro._CompositionState.COMPOSING)) // IE8                        
                    {
                        if (value)
                        {
                            pThis._updateLocale(evt);
    ...
    _pInputElement.on_sys_propertychange_process = function (evt)
    {
        ...
        if (pThis)
        {
            ...
            if (!pThis._processing_oninput)
            {
                ...
                // delay_status = END로 setCompositionComplete()를 통해 다시 propertychange가 들어올때 
                // caret 위치 보정
                if (pThis._delay_pos)
                {
    ...
    if (_pInputElement._use_event_beforeinput)
    {
        ...
        {            
            ...
            if ((nexacro._OS == "Android" || nexacro._AndroidDesktopMode == true) && this.imemode == "none" && this._inputtype != "text" && evt.inputType =="insertCompositionText")
            {
                input_handle.blur();
                input_handle.focus();
            }
            ...
            if (pThis._beforeinput_result)
            {
                ...
                {                    
                    switch (state)
                    {
                        ...
                        case nexacro.InputElement.prototype._BeforeinputState.CANCEL:
                            ...
                            //start [RP 84327] <- [RP 97051] 수정전 코드로 원복 (원복하여도 이전 증상 발생 안함)
				...
				if (bcancle)// && !ismax && !pThis.autoskip)// maxlength를 판단하는 위치와 방법 변경
				{
					return;
				}
            }
        ...
        {            
            ...
            {                
                var pThis = input_handle._linked_element;                   
                if (!pThis.on_sys_keyinput_before_process.call(pThis, evt)) return;                                
    ...
    _pInputElement._on_sys_mousedown = function (evt)
    {
        ...
        {            
            ...
            if (pThis)
            {
                ...
                if (nexacro._Browser == "Edge" && nexacro._BrowserType == "Edge" ||
                    (nexacro._Browser == "Gecko" && nexacro._BrowserVersion >= 85))
                {
    ...
    _pTextAreaElement.setElementFocus = function (trigger_type, self_flag)
    {
        ...
        if (input_handle)
        {
            ...
            if (!this._is_sys_focused)
            {
                ...
                {
                    /*
                    if (nexacro._Browser == "Chrome" || nexacro._Browser == "Gecko" || (nexacro._Browser == "Edge" && nexacro._BrowserType == "WebKit"))
                    {
    ...
    _pTextAreaElement.getElementScrollWidth = function ()
    {
        ...
        if (input_handle)
        {
            ...
            /*
                parent %좌표로 되어있고, textarea의 좌표가 %또는 right좌표로 되어있을 때
                zoom동작 또는 form의 width,height를 조절하면
                소수점 계산에 계산을 하다보면 scrollwidth, scorllHeight가 이유없이 1px~3px 오차가 발생
                element의 사이즈는 실수로 입력하지만, element에서 내주는 값은 정수로 내주고 있어
                오차가 생길수밖에 없는 구조로 판단됨
                -> position 실수 입력과 zoom에 관련된 것으로 추측됨
                -> 로직변경으로 풀수 있으면 풀어도 됨 
            */
            if (scrollmax <= 3)
                scrollmax = 0;
    ...
    _pControlElement.createCommandEnd = function ()
    {
        if (!this._is_popup)
        {
            ...
            var linkedcontrol = this.linkedcontrol;
            if (linkedcontrol && linkedcontrol.semantictag)
            {
                str += "</" + linkedcontrol.semantictag + ">";
            }
            else
            {
                str += "</div>";
    ...
    _pControlElement.setElementFlexGrow = function (flexgrow)
    {
    ...
    _pControlElement.setElementFlexShrink = function (flexshrink)
    {
    ...
    _pControlElement._setElementLayoutBasis = function (bupdate)
    {
        var control = this.linkedcontrol;
        if (!control)
            return;
        ...
        {
            var layouttype = manager.getLayoutProperty(control._getForm(), "type");
            if (layouttype == "default" || layouttype == undefined)
            {                
                if (this._layoutbasis)
                {
            ...
            {
                var arrange_type = fluidlayoutmanager.getLayoutArrangeType();
                if (arrange_type == 1) return;
            }            
        }
        else
        {
        ...
        // position base
        var info = control._arrange_info;
        if (info)
        {
            var obj, comp;
            if (obj = info.width)
            {
                comp = control._findComponentForArrange(obj.compid);
                if (comp)
                {
                    width = control._convToPixel(obj.distance, comp._width);
                }
            }
            ...
            if (obj = info.height)
            {
                ...
                if (comp)
                {
                    height = control._convToPixel(obj.distance, comp._height);
                }
            }
        }

        var minwidth = control.minwidth ? control.minwidth + "px" : undefined;
        var maxwidth = control.maxwidth ? control.maxwidth + "px" : undefined;

        var minheight = control.minheight ? control.minheight + "px" : undefined;
        var maxheight = control.maxheight ? control.maxheight + "px" : undefined;
        ...
        if (is_percent_width)
        {
            this.width = width.substring(0, width.length-1) == 0 ? 0 : width;
        }
        else
        {
            this.width = width;
        ...
        {
            this.height = height.substring(0, height.length - 1) == 0 ? 0 : height;
        }
        else
        {
            this.height = height;
        ...
        // Handle Prop set
        if (control.fittocontents == "width")
        {
            fit_size = control._on_getFitSize();

            minwidth = width = fit_size[0] + "px";
            height = is_percent_height ? height : height + "px";
        }
        else if (control.fittocontents == "height")
        {
            fit_size = control._on_getFitSize();

            width = is_percent_width ? width : width + "px";
            minheight = height = fit_size[1] + "px";
        }
        else if (control.fittocontents == "both")
        {
            fit_size = control._on_getFitSize();
    ...
    */
    ...
    _pScrollableControlElement.initElementInfo = function ()
    {
        ...
        if (control._is_scrollable)
        {
            ...
            this.setElementScrollbarSize(control._getHScrollBarSize(), control._getVScrollBarSize(), control._getHScrollBarType(), control._getVScrollBarType(), control._scrolltype);
        }
    ...
    */
    ...
    __pContainerElement.createCommandStart = function ()
    {
        ...
        if (owner_elem)
        {
			...
			if (scroll_type & 0x10)
				style_str += nexacro.__getHTMLStyle_ScrollLimitX(true);
			if (scroll_type & 0x01)
				style_str += nexacro.__getHTMLStyle_ScrollLimitY(true);
            str += " style='" + style_str + "'";
            str = this._on_createAccessibilityCommand(str);
            str += ">";
...
//	        if (this.left || this.top)
    ...
    * position 이동  : 터치시 화면반응속도 중간, 스크롤속도 중간, 화면출렁거림 거의없음 (노드량이 많으면 느려질 수 있음)
    ...
    nexacro.GridRowControlElement = function (parent_elem, left_gap)
    {
        ...
        // 안드로이드 접근성에서는 scroll 동작이 필요함
        this._client_body_element._no_focus_scroll = (nexacro._enableaccessibility && nexacro._accessibilitytype == 5) ? false : true;
