    ...
    if (!nexacro.Element)
    {
        ...
        */
        ...
        _pInputElement.setElementBlur = function ()
        {
            ...
            if (handle)
            {
                // element handle이 focus를 가져야 하는 동작
                if (this._is_focused)
                {
                    if (this._checkmax_editing_only && this.maxlength > 0)
                    {
                    ...
                    if (range)
                    {
                        nexacro.__setInputElementHandleSetSelect(handle, range[0], range[1], false);
                    }
                    else
                    {
                ...
                // focused 상태와 관계없이 invalid와 displaynulltext를 처리
        ...
        _pInputElement.getElementCaretPos = function ()
        {
			...
			if (handle)
			{
				...
				if (!this._is_focused)
				{
					...
					{
						pos = this._last_selection_range;
					}
				}
				else
				{
					pos = nexacro.__getInputElementHandleCaretPos(handle);
				}
        ...
        _pInputElement._on_sys_keydown_forward = function (keycode, altkey, ctrlkey, shiftkey, metakey)
        {
            ...
            if (ret === false || this._event_stop || !this._is_focused)
            {
                this._event_stop = false;
                return false;
            }
        ...
        _pInputElement.on_sys_keydown_specialkey_process = function (keycode/*, altkey, ctrlkey, shiftkey*/)
		{
			if (keycode == nexacro.Event.KEY_INSERT)
            {
                ...
                // [24/06/26][RP 100201, 100202][MaskEdit InsertMode]
                if (comp && comp instanceof nexacro.MaskEdit)
                {
                    this._is_insert_mode = false;
                    this._event_stop = true;
                    return false;
                }
        ...
        _pInputElement._on_sys_keypress_forward = function (keycode, charcode, altkey, ctrlkey, shiftkey, metakey)
        {
            ...
            if (comp)
            {
                ...
                // [RP 100515][메세지(showModalSync) 팝업 종료시..]
            ...
            else   //for is_popup
            {
                if (!this.handle)
                {
                    ...
                    if (linkedcontrol._findOwnerElementHandle)
                    {
                        ...
                        this.owner_elem = owner_elem.frame._control_element;
        ...
        // TODO2 동적 set검토 필요
        _pControlElement.setElementFlexGrow = nexacro._emptyFn;
