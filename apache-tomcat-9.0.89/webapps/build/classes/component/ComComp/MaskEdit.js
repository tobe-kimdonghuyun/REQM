...
if (!nexacro.MaskEdit)
{
    ...
    _pMaskEdit._apply_setfocus = function (evt_name)
	{
        ...
        if (input_elem)
        {
            ...
            if (!this._onlydisplay)
            {
                ...
                if (nexacro._enableaccessibility)
                {
                    if (!this.autoselect)
                    {
                        if (this._caret_pos == -1)
                            ...
                            input_elem.setElementSetSelect(this._caret_pos, this._caret_pos);
                    }
                }
                if (this.autoselect)
                {
                ...
                /* 추후 반영예정
                if (!this.autoselect && this.postfixtext)
                {
                    if (this._caret_pos == -1)
                        input_elem.setElementSetSelect(0, 0);
                    else
    ...
    _pMaskEdit._on_input_autoskip = function ()
    {
        ...
        if (maskobj)
        {
            ...
            if (this.autoskip && maskobj.isFilled() && nexacro._nvl(this.value, true) != true)
            {
    ...
    _pMaskEdit.on_killfocus_basic_action = function (new_focus, new_refer_focus)
	{
        ...
        if (input_elem)
        {
            ...
            if (!this._onlydisplay)
            {
                ...
                if (idx < 0)                //[RP 80313] killfocus처리중에 다시 자기한테 포커스가 온경우 blur처리 스킵   
