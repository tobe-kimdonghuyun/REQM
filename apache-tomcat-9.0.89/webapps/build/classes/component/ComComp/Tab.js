...
if (!nexacro.Tab)
{
    ...
    _pTab.prevbutton = null;        // nexacro.Button
    _pTab.nextbutton = null;        // nexacro.Button   
    ...
    _pTab.preload = false;
    _pTab.tabbuttongap = undefined;
    _pTab.tabbuttonstartgap = undefined;
    _pTab.usetabbutton = true;  // todo
    _pTab._bshow_tabpage_accordion = false;
    _pTab.tabstyle = "normal";
    _pTab._buse_tabbuttonicon = true;
    _pTab.showtabbuttonicon = false;
    _pTab.tabbuttoniconsize = null;
    ...
    _pTab._on_accessibility_create_spinbutton = nexacro._emptyFn;
	...
	{
		// tab에 focus시 여기를 탈일이 없음 & 가상커서의 초점이 tab 이동시 scroll 튀는 현상
		if (nexacro._enableaccessibility && !this._isComponentKeydownAction() && target_comp === this)
			return;
    ...
    _pTab.set_showtabbuttonicon = function (v) {
        v = nexacro._toBoolean(v);
        if (this.showtabbuttonicon != v) {
            this.showtabbuttonicon = v;
    ...
    {
        if (this.tabbuttongap != tabbuttongap)
        {
            this.tabbuttongap = tabbuttongap;
    ...
    {
        if (this.tabbuttonstartgap != tabbuttonstartgap)
        {
            this.tabbuttonstartgap = tabbuttonstartgap;
    ...
    _pTab.set_tabbuttoniconsize = function (v) {
        ...
        if (this.tabbuttoniconsize == v)
            return;

        this.tabbuttoniconsize = v;
    ...
    {
        var pre_value = nexacro._toString(this.tabstyle);
        var post_value = nexacro._toString(v);
    ...
    _pTab.removeTabpage = function (nIndex)
    {
        ...
        if (newtabpage)
        {
            ...
            if (tabbuttonitems) {
                this._changeUserStatusAll(tabbuttonitems[this.tabindex], true);
            }
    ...
    _pTab._on_focus = function (self_flag, evt_name, lose_focus, refer_lose_focus, new_focus, refer_new_focus)
    {
        ...
        if (!this._focusobj)
        {
            ...
            {
                if (!nexacro._enableaccessibility  && !self_flag)
    ...
    */
    ...
    _pTab.on_touchend_basic_action = function (touch_manager, touchinfos, changedtouchinfos/*, refer_comp*/)
    {
        ...
        if (control)
        {
			...
			if (control instanceof nexacro._TabExtraButtonControl)
			{
				...
				var curindex = this.tabindex; //onextrabuttonclick 이후의 index로 change 여부를 check한다.
				if (curindex == idx) 
				{
					return ret;
				}
				...
				{
					control = control.parent; //control의 parent가 remove등으로 삭제 되었다면 아래 change를 타지 않는다.
				}
			}
    ...
    _pTab._createSpinButton = function (bCreateOnly)
    {
        if (!this.prevbutton)
        {
            ...
            this.prevbutton.set_accessibilityenable(false);
    ...
    _pTab._changeTabIndex = function (index, is_apply_focus,is_mouse_action)
    {
        ...
        if (this.tabstyle == "normal")
        {
            if (!this._is_created || this.tabindex == index ||
                (nindex != nindex) || index < 0 || (this.tabpages && index >= this.tabpages.length))
            {
                return;
            }
        }
...
//          if (this.tabstyle != "accordion")   // multi tappage 사용시
    ...
    _pTab._rearrangeContents = function (beginTabindex)
    {
       ...
       // var brotatetext = this.rotatetext & false;  //TODO brotatetext = false;
        ...
        var bshowtabbuttonicon = this.showtabbuttonicon;
        ...
        if (beginTabindex == undefined || beginTabindex == null)
        {
            ...
            for (i = tabindex; i >= 0; i--)
            {
                ...
                if (tabposition == "top" || tabposition == "bottom")
                {
                    ...
                    is_visible = nexacro._enableaccessibility ? (tabbuttonitem.left > -1 && tabbuttonitem.visible) : tabbuttonitem.visible;
                }
                else
                {
                    is_within_maxsize = temp_height <= tab_max_height;
                    is_visible = nexacro._enableaccessibility ? (tabbuttonitem.top > -1 && tabbuttonitem.visible) : tabbuttonitem.visible;
                }
        ...
        for (i = 0; i < tabbuttonitems_len; i++)
        {
            ...
            if (tabposition == "top" || tabposition == "bottom")
            {
                ...
                else
                {
                    ...
                    {
                        tabbuttonitem.set_visible(this.usetabbutton);
...
*/                
        ...
        if (!bshowspin || !this.usetabbutton)
        {
    ...
    _pTab._rearrangeContentsForAccrdion = function (beginTabindex, getvalue)
    {
        ...
        if (this._bshow_tabpage_accordion)
        {
            if (this.fittocontents == "both" || this.fittocontents == "height")
            {
                for (i = firstTabindex; i < tabpages_len; i++)
                {
                    if (i == tabindex)
                    {
                        ...
                        {
                            size = tabpage._on_getFitSize();
                            tabpage_position.height = size[1];

                            if (this.fittocontents == "both")
                            ...
                            {
                                var hscrollbar = tabpage.form ? tabpage.form.hscrollbar : null;
                ...
                for (i = firstTabindex; i < tabpages_len; i++)
                {
                    if (i == tabindex)
                    {
                        ...
                        if (tabpage = tabpages[i])
                        {
                            ...
                            if (this.fittocontents == "width")
                            {
                                var vscrollbar = tabpage.form ? tabpage.form.vscrollbar : null;
        ...
        {
            if (this.fittocontents == "both" || this.fittocontents == "width")
        ...
        else
        {
            ...
            if (this.fittocontents == "width" || this.fittocontents == "both")
            {
    ...
    _pTab._getTabButtonIconSize = function () {
        ...
        if (this.tabbuttoniconsize) {
            tabbuttoniconsize = this.tabbuttoniconsize.split(" ");
        ...
        return tabbuttonitem._getTabButtonIconElementSize(this.showtabbuttonicon);
    };
    ...
    _pTab._keydown_filter = function (elem, keycode, alt_key, ctrl_key, shift_key, event_bubbles, fire_comp, refer_comp, meta_key)
    {
        ...
        switch (keycode)
        {
            case 9: // nexacro.Event.KEY_TAB:
                ...
                else
                {
                    ...
                    else if (this._focusobj instanceof nexacro.Tabpage)
                    {
                        ...
                        if (!this.focusacceptable && !_is_fire_keydown)
                            return false;
                        ...
                        if (shift_key == false)
                        {
                            ...
                            {
                                comp = this.parent._searchNextTabFocus(this, undefined, undefined, 0);
                                if (comp && comp[0])
                                {
                                    comp[0]._setFocus(true, 0, true);
                                }
            ...
            case 38: //nexacro.Event.KEY_UP;
                {
                    if (nexacro._enableaccessibility && _is_fire_keydown)
                    {
                        //if button -> go tab
            ...
            case 39: // nexacro.Event.KEY_RIGHT:
                if (this.focusacceptable && ctrl_key == false && _is_fire_keydown)
                {
                    focusobj = this._focusobj;
...
if (!nexacro._TabButtonItemControl)
{
    ...
    _pTabButtonItemControl.on_apply_text = function (v)
    {
        ...
        if (control_elem)
        {
            ...
            if (!tabbuttonitemtext)
            {
                ...
                tabbuttonitemtext.accessibilityenable = false;
            ...
            else
            {
                tabbuttonitemtext.set_text(v);
                if (this._extrabutton)
                    this._extrabutton._setAccessibilityStatHidden(true);
    ...
    _pTabButtonItemControl._onload_tabbutton_iconimage = function (imgurl, width, height) {
        ...
        this.parent._on_get_tabbutton_iconsize(width, height);
    };
