...
if (!nexacro.Radio)
{
    ...
    _pRadio.text = "";
    _pRadio.acceptvaluetype = "allowinvalid";   //allowinvalid | ignoreinvalid
	...
	_pRadio._getDlgCode = function (keycode, altKey, ctrlKey, shiftKey)
	{
        ...
        {
            _want_arrows = (!nexacro._enableaccessibility || !nexacro._accessibilitycontentsearchkey);
        }
	...
	_pRadio._on_focus = function (self_flag, evt_name, lose_focus, refer_lose_focus, new_focus, refer_new_focus)
	{
		...
		if (self_flag == false)
		{
			...
			if (focusdir == 0 || focusdir == 1)   //tab, shifttab
            {
                ...
                var index = nexacro._enableaccessibility && this.index == -1 ? 0 : this.index;
			...
			else if (nexacro._enableaccessibility && (focusdir == 2 || focusdir == 3)) // down,up key (accessibility)
            {
    ...
    _pRadio._doSelect = function (index)
    {
        ...
        if (item)
        {
            ...
            //item._changeUserStatus("mouseover", this._isItemAccessibilityEnable());
    ...
    _pRadio._doDeselect = function (index)
    {
        ...
        if (item)
        {
            ...
            //item._changeUserStatus("mouseover", flase);
            item._block_read_aria_stat = false;
