...
if (!nexacro.DataObject)
{
    ...
    */
    ...
    _pDataObject.url = "";
    _pDataObject.enableevent = true; //default
    _pDataObject.preload = false; //default
    _pDataObject.data = null; //default
    ...
    _pDataObject._getRootObjectByPath = function (datapath)
    {
     ...
     _pDataObject._setValueInKey = function (target, key, value)
     {
        ...
        if (arr_datapath.length > 0)
        {
            ...
            {
                var temp_target = [];
                for (i = 1, len = arr_datapath.length; i < len; i++)
                {
                    if (i == len - 1)
                    {
            ...
            {
                for (i = 0, len = arr_datapath.length; i < len; i++)
                {
                    if (i == len - 1)
                        target[arr_datapath[i]] = value;
                    else
