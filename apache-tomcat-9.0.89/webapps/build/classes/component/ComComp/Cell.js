...
if (!nexacro.CellInfo)
{
    ...
    {
        var prop, bind, str, type, styl, eval_str = ""; // deft
    ...
    {
        var val = this._getAttrValue(this.radioitemsize, rowidx);
    ...
    {
        var multicombodataset = this._getAttrValue(this.multicombodataset, rowidx);
        var multicombocodecol = this._getAttrValue(this.multicombocodecol, rowidx);
        var multicombodatacol = this._getAttrValue(this.multicombodatacol, rowidx);
        ...
        if (multicombodataset && multicombodataset.length && multicombocodecol && multicombocodecol.length && multicombodatacol && multicombodatacol.length)
        {
            ...
            if (ds)
            {
                ...
                for (var i = 0; i < codes.length; i++)
                {
                    ...
                    var data = ds.lookupNF(multicombocodecol, code, multicombodatacol);
                    if (data)
                    {
        ...
        {
            text = this._getAttrValue(this.multicombodisplaynulltext, rowidx);
        }
        ...
        for (i = 0; i < n; i++)
        {
            ...
            else if (this.hasOwnProperty(prop) == false)    // userdata
            {
                switch (prop)
                {
                    ...
                    //----------------
                    default:
                        this[prop] = new nexacro.BindableValue(undefined);
                        this[prop]._propname = prop;
...
if (!nexacro._CellControl)
{
    ...
    _pCellControl._needToggle = function ()
    {
        ...
        if (nexacro._toBoolean(view.readonly))
            return;
    ...
    _pCellControl._updateDisplayer = function ()
    {
        ...
        if (this._isChangeDisplayer(displayType))
        {
            ...
            if (view && view._lbuttondown_proc)
            {
                ...
                if (window._cur_ldown_elem && window._cur_ldown_elem.parent == null && this.parent != null) // ex) displaytype expr 사용시
    ...
    _pCellControl._destroyEditor = function ()
    {
        ...
        if (this._editor)
		{
			...
			else
			{
                ...
                {
                    // mainframe으로 초점 빠지는 것 방지 (접근성)
                    this._control_element.setElementFocus(nexacro._enableaccessibility); 
                }
			}
    ...
    _pCellCombo._setProperty = function (onlycontrolprop)
    {
        ...
        if (!onlycontrolprop)
        {
            v = cellinfo._getAttrValue(cellinfo.text, datarow, this._displaymode);
    ...
    _pCellMultiCombo._setProperty = function (onlycontrolprop)
    {
        ...
        v = cellinfo._getAttrValue(cellinfo.multicombodisplaynulltype, datarow);
        if (v == "nulltext")
        {
            v = cellinfo._getAttrValue(cellinfo.multicombodisplaynulltext, datarow);
            this.set_displaynulltext(v);
        }
        else
        {
    ...
    _pCellMultiCombo.on_apply_text = function (text)
    {
        if (this._displaymode == true)
        {
            if (!nexacro._isNull(this.value))
            {
                ...
                var v = cellinfo._getAttrValue(cellinfo.multicombodisplaynulltype, datarow);
                if (v == "nulltext")
                {
                    if (this.index < 0)   // value가 있어도 선택된 아이템이 존재하지 않는다면..
                        this._setEditValue(undefined);
    ...
    {
        return this.multicomboedit.value;
    };
    ...
    _pCellRadioItem._setProperty = function (onlycontrolprop)
    {
        ...
        this.set_value(v);
        this._cellobj._setAccessibilityStatChecked(!!this.value);
    };
    ...
    {
        this._changeUserStatus("selected", this.codevalue == this.value);
    };
    ...
    _pCellRadioItem._itemSelect = function ()
    {
        ...
        if (view && editType == "radioitem")
        {
            if (cellinfo.text._bindtype == 1)
            {
                view._dsEventOccured = true;
                var dataset = cellobj._getBindDataset();
                if (dataset && dataset.setColumn(datarow, cellinfo.text._bindexpr, this.codevalue))
                {
                    var v = cellinfo._getTextValueForDisp(datarow);
                    this.set_value(v);
                }
