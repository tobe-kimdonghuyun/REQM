...
if (!nexacro.ListBox)
{
    ...
    _pListBox.acceptvaluetype = "allowinvalid";   //allowinvalid | ignoreinvalid
    ...
    _pListBox._on_focus = function (self_flag, evt_name, lose_focus, refer_lose_focus, new_focus, refer_new_focus)
    {
        ...
        if (this._last_focused && (focusdir < 0 || self_flag == false))
        {
            this._do_defocus(this._last_focused, false);
        }

        var item_idx = this._accessibility_index;
        if (focusdir >= 0)
        {   
            if (self_flag == false)
            {
                ...
                if (nexacro._enableaccessibility)
                {
					...
					if (item_len)
                    {
                        ...
                        {
                            case 0: //tab key
                            case 1: //shift + tab key
                                item_idx = -3; // tab 진입 체크
                                break;
                            case 2: //down key
                        ...
                        var item = this._getItem(item_idx);
                        if (item)
                        {
                            this._refreshScroll(item_idx, focusdir);
							item._on_focus(true);							
						}
                    }

                    this._accessibility_index = item_idx;
                    this._shift_select_base_index = item_idx;
                }
            }
		...
		{
            if (this._select_multi.lastselected === undefined && item_idx > -1)
			{
    ...
    _pListBox._adjustScrollRows_callback = function (no_ani, pos)
    {
        ...
        if (this.index > -1 && !nexacro._enableaccessibility)
        {
    ...
    _pListBox.on_keydown_basic_action = function (keycode, alt_key, ctrl_key, shift_key, refer_comp)
    {
        ...
        {
            this._on_keydown_basic_accessibility_action(keycode, alt_key, ctrl_key, shift_key, refer_comp);
        }
    };
