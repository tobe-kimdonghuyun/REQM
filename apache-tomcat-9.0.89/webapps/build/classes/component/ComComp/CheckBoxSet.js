...
if (!nexacro.CheckBoxSet)
{
    ...
    _pCheckBoxSet.on_apply_value = function (values)
    {
        ...
        var ds = this._innerdataset;
        var codecolumn = this.codecolumn || this.datacolumn;
        var datacolumn = this.datacolumn || codecolumn;
    ...
    _pCheckBoxSet.on_apply_readonlycolumn = function (readonlycolumn)
    {
        ...
        if (control_elem && ds)
        {
            ...
            for (var i = 0; i < item_len; i++)
            {
                val = this.readonly ? this.readonly : ds.getColumn(i, readonlycolumn);
        ...
        if (ds && column)
        {
            ...
            if (selectedCount > 0)
            {
                ...
                for (var i = 0; i < selectedCount; i++)
                {
                    if (i == 0)
                    {
                        text += ds.getColumn(selectedItems[i], this.datacolumn || this.codecolumn);
                    }
                    else
                    {
                        text += this.textseparator + ds.getColumn(selectedItems[i], this.datacolumn || this.codecolumn);
                    }
                }
                this._setText(text);
            }
    ...
    _pCheckBoxSet._on_dataset_onload = function (obj, e)
    {
        if (e.reason == 0)
        {
            ...
            if (this.index != -1)
            {
                if (this._insertIndex(this.index))
                {
                    this.on_apply_index(this.index);
                }
            }
                ...
                else
                {
                    ...
                    else
                    {
                        if (accIdx == -1)
                        {
                            if (this._isAccessibilityEnable())
                            {
                                last_focused = this._last_focused;
                                if (last_focused)
    ...
    _pCheckBoxSet._select_withkeyupevent = function (shiftkey)
    {
        ...
        if (shiftkey)
        {
            ...
            else
            {
                ...
                if (this.index.toString() != rows.toString())
                {
                    this._select_clear(true);
        ...
        if (ds)
        {
            ...
            if (rows > 0)
            {
                ...
                for (var i = 0; i < rows; i++)
                {
                    ...
                    value = ds.getColumn(i, codecolumn);
                    value = this._convertValueType(value, true);
                    readonly = this.readonly ? this.readonly : ds.getColumn(i, readonlyCol);
        ...
        if (!isNotFireEvent)
        {
            ...
            if (dataset && column)
            {
                ...
                for (i = 0; i < v.length; i++)
                {
                    ...
                    datavalue = dataset.getColumn(idx, this.datacolumn || this.codecolumn);
                    codevalue = dataset.getColumn(idx, this.codecolumn || this.datacolumn);
