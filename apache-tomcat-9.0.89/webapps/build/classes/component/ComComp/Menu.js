...
if (!nexacro.Menu)
{
	...
	{
		var is_accessibility = this._isAccessibilityEnable();
		...
		switch (keycode)
		{
			...
			case E.KEY_LEFT:
				...
				{
					// 접근성에서는 popupmenu 내부에서 키액션 처리, popup 확장 상태에서는 다른 메뉴 이동 불가
					if (is_accessibility === false)
					{
						this._fire_on_Popupmenu(keycode, alt_key, ctrl_key, shift_key, fire_comp, refer_comp, meta_key);
					}
				}
	...
	_pMenu._select_menuitem = function (nextitemindex, previtemindex, popupvisible, direction)
	{
		...
		// 접근성 상태에서는 popup을 이동중에 자동으로 활성화 안됨, 사용자가 enter를 입력해야 열림
		if (this._is_item_selected && this._isAccessibilityEnable() === false)
		{
