...
if (!nexacro.Calendar)
{
	...
	_pCalendar.datepickerchangetype = "spin";
	...
	_pCalendar.useautocalibration = true;
	...
	_pCalendar.on_created_contents = function (win)
	{
		...
		switch (this._type)
		{
			...
			case "monthonly":
				...
				//    this._setAccessibilityActiveDescendant(this.datepicker.body.getElement());
				break;
			case "system":
	...
	{
		return this.calendaredit ? this.calendaredit._getInputElement() : null;
	};
    ...
    */
	...
	_pCalendar.set_useautocalibration = function(v)
	{
		...
		if(this._useautocalibration != v || this.useautocalibration != v)
		{
			this.useautocalibration = v;
	...
	_pCalendar.on_apply_useautocalibration = function()
	{
		if(this.calendaredit)
		{
			this._useautocalibration = (this._useautocalibration !== undefined) ? this.useautocalibration : nexacro._usedateautocalibration;
		}
	};
	...
	_pCalendar._on_apply_text = function (text)
	{
		...
		if (this.expr.substring(0, 4) == "Date")
		{
	...
	_pCalendar.set_datepickerchangetype = function (v)
	{
		...
		if (this.datepickerchangetype != v)
		{
			this.datepickerchangetype = v;
		...
		switch (this.type)
		{
			case "normal":
				...
				else if (keycode == nexacro.Event.KEY_ENTER)
				{
					if (this._isPopupVisible())
					{
						if (this.datepickerchangetype != "button" || datepicker.head.monthstatic.visible)
						{
	...
	_pCalendar._on_spin_onspinup = function (/*obj, e*/)
	{
		...
		if (calendaredit)
		{
			...
			var pre_text;
			var pre_value = calendaredit.value;
			...
			if (maskobj)
			{
				...
				{
					var cur_date;
					if (calendaredit.value)
					{
						var packeddate = maskobj.changeNormalizeValue(calendaredit.value);
						cur_date = packeddate;
					} else
					{
	...
	_pCalendar.on_focus_basic_action = function (self_flag, evt_name, lose_focus, refer_lose_focus, new_focus, refer_new_focus)
	{
		...
		if (calendaredit)
		{
			...
			{
				if ((this.type != "spin" && this.popuptype != "none" && (evt_name == "lbuttondown" || evt_name == "tap")) || this._type == "system")
				{
					var control_elem = this.getElement();
					if (control_elem)
					{
		...
		if (input_elem)
		{
			...
			if (keycode == nexacro.KeyCode_ImeInput)
			{
				...
				{					
					...
					// blur focus 강제 처리(조합취소)하면 caret이 마지막에 위치하는 현상 수정
	...
	_pCalendarEditControl._beforeinput_process_with_HTMLEvent = function (value, status, begin, end, inputType)
    {
		...
		if (maskobj)
		{
			...
			var front_text, rear_text, update_value, append_value, result, input_pos, end_sel;
				...
				case "insertCompositionText":
					...
					else
					{
					    ...
					    else
					    {
						    ...
						    {
								append_value = value + "".padLeft(end - value.length, " ");
							    update_value = input_value.substring(0, begin) + append_value + input_value.substring(end);
						    ...
						    else
						    {
								...
								if (this.autoskip && input_pos == -1 && result.pos == result.text.length)
								{
						...
						else
						{
							...
							else
							{
								if (_date.ret)
								{
									...
									if (this.autoskip && _pos == _text.length && maskobj._is_editing)
									{
										maskobj._is_filled = true;
									}
								}
