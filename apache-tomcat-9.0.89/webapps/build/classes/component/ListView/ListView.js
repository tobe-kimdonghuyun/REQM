...
if (!nexacro._ListViewBandControl)
{
    ...
    */
    ...
    /* accessibility */
    _pListViewBandControl.accessibilityrole = "listviewband";
	...
	_pListViewBandControl._onResetUserExpand = function ()
    {
        ...
        if (expandbar)
        {
            ...
            if (listview && listview.getBandExpandStatus(this._itemindex))
			{
                ...
                if (nexacro._enableaccessibility && nexacro._isArray(listview._getItem(this._itemindex)))
                {
...
if (!nexacro.ListView)
{
    ...
    delete _pListViewInputEventInfo;
    ...
    */
    ...
    _pListView.readonly = false;
    _pListView.createrowstype = "auto";

    /* accessibility */
    _pListView.accessibilityrole = "listview";
        ...
        if (band)
        {
            ...
            if (rowidx >= 0)
            {
                ...
                else
                {
                    ...
                    if (!nextcell)
                    {
                        if (nexacro._enableaccessibility && band.expandbartype == true && band._band == "body" && !this._is_expandbar_focus)
                        {
        ...
        if (band)
        {
            ...
            if (rowidx >= 0)
            {
                ...
                if (!prevcell)
                {
                    ...
                    if (band = this._getCurrentBand(rowidx, bandidx))
                    {
                        if (nexacro._enableaccessibility && band.expandbartype == true && band._band == "body")
                        {
                            expandbar = band.expandbar;
                        }
                        else
    ...
    _pListView._selectItemKeyInfo = function (keycode, alt_key, ctrl_key, shift_key)
    {
    	switch (keycode)
    	{
            ...
            case nexacro.Event.KEY_PAGE_DOWN:
                if (!nexacro._enableaccessibility || (nexacro._enableaccessibility && this._isComponentKeydownAction()))
                {
    ...
    */
            ...
            for (var propid in cellsett)
            {
                ...
                else if (cellinfo.hasOwnProperty(propid) == false)    // userdata
                {
                    ...
                    switch (propid)
                    {
                        ...
                        //----------------
                        default:
                            cellinfo[propid] = new nexacro.BindableValue(undefined);
    ...
    _pListView.onInitProperties = function ()
    {
        ...
        */
        ...
        this.cellmulticombobuttonsize = undefined;
        this.locale = "";
    ...
    {
        if (v != this.cellmulticombobuttonsize)
        {
            this.cellmulticombobuttonsize = v;
    ...
    {
        if (v != this.cellmulticombopopuptype)
        {
            this.cellmulticombopopuptype = v;
    ...
    {
        if (v != this.cellmulticombopopupsize)
        {
            this.cellmulticombopopupsize = v;
    ...
    _pListView.set_createrowstype = function (type)
    {
        ...
        if (type !== this.createrowstype)
        {
            switch (type)
            {
                case "auto":
                case "all":
                    this.createrowstype = type;
                    break;
                default:
                    break;
            }
...
//      return this._formats ? this._formats._getBandChildProperty(bandid, cellid, propid) : undefined;       ??
    ...
    _pListView.dropdown = function ()
    {
        ...
        if (editor && editor.visible == true && editor.dropdown)
        {
            editor.dropdown();
            return true;
        }
    ...
    _pListView.isDropdown = function ()
    {
        ...
        if (editor && editor.visible == true && editor.isDropdown)
        {
            return editor.isDropdown();
        }
        return false;
    ...
    */
    ...
    _pListView.on_fire_sys_onlbuttondown = function (button, alt_key, ctrl_key, shift_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, meta_key)
	{
		...
		if (cell)
		{
			...
			if (autoenter != "select") // autoenter가 "select"가 아닌 경우
			{
                ...
                {
                    if (cell.selected == false && nexacro._enableaccessibility)
                    {
                        cell._actionEditCell(cell, "lbuttondown");
                    }
                }
	...
	/*
     * is_editable : find editable cell
     * is_accessibility : find accessibility obj
	*/
    _pListView._getFirstCell = function (is_editable, is_accessibility)
    {
        var rowcnt = this._getBindCount();
        if (rowcnt > 0)
        {
            ...
            for (var i = 0; i < rowcnt; i++)
            {
                ...
                if (item)
                {
                    ...
                    if (item[0])
                    {
                        // firstcell index 찾고 없으면 탐색 (body)
                        first_cell = this._findNextCell(item[0], -1, undefined, 0, is_editable, is_accessibility);
                        first_cell = (!first_cell) ? this._findNextCell(item[0], -1, undefined, 1, is_editable, is_accessibility) : first_cell;
                    }

                    if (!first_cell && item[1] && this._isExistSubSlot(item[1]._rowidx))
                    {
                        // firstcell index 찾고 없으면 탐색 (detail)
                        first_cell = this._findNextCell(item[1], -1, undefined, 0, is_editable, is_accessibility);
                        first_cell = (!first_cell) ? this._findNextCell(item[1], -1, undefined, 1, is_editable, is_accessibility) : first_cell;
                    }
     ...
     * is_accessibility : find accessibility obj
    */
    _pListView._getLastCell = function (is_editable, is_accessibility)
    {
        var rowcnt = this._getBindCount();
        if (rowcnt > 0)
        {
            ...
            for (var i = rowcnt - 1; i >= 0; i--)
            {
                ...
                {
                    var last_cell = null;
                    var cellidx = 0, cells = null;

                    if (item[1] && this._isExistSubSlot(item[1]._rowidx))
                    {
                        cells = this._getBandCells(item[1]);
                        cellidx = cells ? cells.length : 0;

                        // lastcell index 찾고 없으면 탐색 (detail)
                        last_cell = this._findNextCell(item[1], cellidx, undefined, 0, is_editable, is_accessibility);
                        last_cell = (!last_cell) ? this._findNextCell(item[1], cellidx, undefined, 0, is_editable, is_accessibility) : last_cell;
                    }

                    if (!last_cell && item[0])
                    {
                        cells = this._getBandCells(item[0]);
                        cellidx = cells ? cells.length : 0;

                        // lastcell index 찾고 없으면 탐색 (body)
                        last_cell = this._findNextCell(item[0], cellidx, undefined, 0, is_editable, is_accessibility);
                        last_cell = (!last_cell) ? this._findNextCell(item[0], cellidx, undefined, 0, is_editable, is_accessibility) : last_cell;
                    }
    ...
    /*
     * item : band or cell
     * from_cellidx : start cellindex
     * from_bandidx : start bandidx
     * dir : current 0, next 1, prev -1
     * is_editable : find editable cell
     * is_accessibility : find accessibilty obj
     */
    _pListView._findNextCell = function (item, from_cellidx, from_bandidx, dir, is_editable, is_accessibility)
    {
        var cell = null;
        ...
        {
            var i, n;
            from_bandidx = (from_bandidx !== undefined) ? from_bandidx : -1;

            if (dir > 0)
            {
                i = from_bandidx ? from_bandidx : 0;
                n = item.length;
				...
				for (; i < n; i++)
				{
					if (item[i])
					{
						// detail band collapse
						if (item[i]._band === "detail" && !this._isExistSubSlot(item[i]._rowidx))
							continue;

                        // find next band
						if (i !== from_bandidx)
                        {
                            // body band lastcell to expand control focus
						    if (is_accessibility && item[i]._band === "detail" && item[i - 1].expandbar && this._is_expandbar_focus === false && item[i - 1].expandbartype == true)
							{
								cell = item[i - 1].expandbar;
							}
							else
							{
								from_cellidx = (dir > 0) ? -1 : item[i]._getLastChild() ? item[i]._getLastChild()._cellidx + 1 : 0;
								cell = this._findNextCell(item[i], from_cellidx, undefined, 0, is_editable, is_accessibility);
							}
						}

						cell = (!cell) ? this._findNextCell(item[i], from_cellidx, undefined, dir, is_editable, is_accessibility) : cell;
						if (cell)
						{
							break;
						}
            ...
            else if (dir < 0)
            {
                ...
                for (; i > n; i--)
                {
                    if (item[i])
                    {
                        ...
                        if (i !== from_bandidx)
                        {
                            // detail band to expand control focus
                            if (is_accessibility && item[i]._band === "body" && item[i].expandbar && this._is_expandbar_focus === false && item[i].expandbartype == true)
                            {
                                cell = item[i].expandbar;
                            }
                            else
                            {
                                from_cellidx = (dir > 0) ? -1 : item[i]._getLastChild() ? item[i]._getLastChild()._cellidx + 1 : 0;
            ...
            else
            {
                if (from_bandidx !== -1)
                {
                    cell = this._findNextCell(item[from_bandidx], from_cellidx, undefined, dir, is_editable, is_accessibility);
                }
            }
        }
        else
        {            
            ...
            if (cells)
            {
                // find cell (dir => next : 1, prev : -1, cur : 0)
                from_cellidx = (from_cellidx < 0) ? 0 : (from_cellidx > cells.length - 1) ? cells.length - 1 : from_cellidx;

                if (dir !== 0)
                {
                    // next cell focus
                    while (cells[from_cellidx])
                    {
                        from_cellidx += dir;

                        if (cells[from_cellidx])
                        {
                            cell = cells[from_cellidx];
                        }
                        ...
                        if (cell)
                        {
                            if (is_editable)
                            {
                                // check editable (editable 아니면 다음 index로 진행)
                                if (cell._hasEditor() === false || cell._refinfo._getEdittype(cell._getDataRow()) !== "checkbox")
                                {
                                    cell = null;
                                    continue;
                                }
                            ...
                            {
                                // check accessibility
                                if (cell._isAccessibilityEnable() === false)
                                {
                                    cell = null;
                                    continue;
                                }
                ...
                {
                    // current cell focus
                    cell = cells[from_cellidx] ? cells[from_cellidx] : null;
                    if (cell)
                    {
                        if (is_editable)
                        {
            ...
            if (cell && cell instanceof nexacro._ListViewCellControl)
            {
                this._is_expandbar_focus = false; // init expand control focus flag
            }
        }
        ...
        if (rowcnt > 0)
        {
            ...
            for (var i = rowcnt - 1; i >= 0; i--)
            {
                ...
                if (items)
                {
                    ...
                    {
                        if (items && (lastcell = items._getLastChild()))
                        {
